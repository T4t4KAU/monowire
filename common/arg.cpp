...
#define MONOWIRE_MAX_URL_LENGTH 2084 // Maximum URL Length in Chrome: 2083
...
static std::initializer_list<enum monowire_example> mmproj_examples = {
    MONOWIRE_EXAMPLE_MTMD,
    MONOWIRE_EXAMPLE_SERVER,
    MONOWIRE_EXAMPLE_CLI,
};
...
static const std::vector<common_arg> &get_common_arg_defs() {
    static const std::vector<common_arg> options = [] {
        ...
        auto ctx = common_params_parser_init(params, MONOWIRE_EXAMPLE_SERVER, nullptr);
        return ctx.options;
    }();
    return options;
}

common_arg &common_arg::set_examples(std::initializer_list<enum monowire_example> examples) {
    this->examples = examples;
    return *this;
}

common_arg &common_arg::set_excludes(std::initializer_list<enum monowire_example> excludes) {
...
bool common_arg::in_example(enum monowire_example ex) { return examples.find(ex) != examples.end(); }

bool common_arg::is_exclude(enum monowire_example ex) { return excludes.find(ex) != excludes.end(); }
...
bool common_arg::get_value_from_env(std::string &output) const {
    ...
    if (!args_neg.empty()) {
        // for compatibility, we need to check MONOWIRE_ARG_NO_ env as well
        std::string neg_env = env;
        string_replace_all(neg_env, "MONOWIRE_ARG_", "MONOWIRE_ARG_NO_");
...
static std::vector<std::string> break_str_into_lines(std::string input, size_t max_char_per_line) {
    ...
    while (std::getline(iss, line)) { add_line(line); }
    return result;
}
...
std::string common_arg::to_string() const {
    ...
    for (const auto &line : help_lines) { ss << (&line == &help_lines.front() ? "" : leading_spaces) << line << "\n"; }
    return ss.str();
}

std::vector<std::string> common_arg::get_args() const {
    std::vector<std::string> result;
    for (const auto &arg : args) { result.push_back(std::string(arg)); }
    for (const auto &arg : args_neg) { result.push_back(std::string(arg)); }
    return result;
}
...
std::vector<std::string> common_arg::get_env() const {
    ...
    if (!args_neg.empty() && env) {
        // for compatibility, we need to add MONOWIRE_ARG_NO_ variant
        std::string neg_env = env;
        string_replace_all(neg_env, "MONOWIRE_ARG_", "MONOWIRE_ARG_NO_");
...
static void parse_tensor_buffer_overrides(const std::string &value,
                                          std::vector<monowire_model_tensor_buft_override> &overrides) {
...
static bool common_params_handle_remote_preset(common_params &params, monowire_example ex) {
...
static bool common_params_parse_ex(int argc, char **argv, common_params_context &ctx_arg) {
    ...
    for (auto &opt : ctx_arg.options) {
        for (const auto &arg : opt.args) { arg_to_options[arg] = {&opt, /* is_positive */ true}; }
        for (const auto &arg : opt.args_neg) { arg_to_options[arg] = {&opt, /* is_positive */ false}; }
    }
    ...
    auto parse_cli_args = [&]() {
        ...
        for (int i = 1; i < argc; i++) {
            ...
            if (!seen_args.insert(arg).second) {
                LOG_WRN("DEPRECATED: argument '%s' specified multiple times, use comma-separated values instead (only "
    ...
    } catch (const std::exception &e) { LOG_WRN("HF cache migration failed: %s\n", e.what()); }
    // export_graph_ops loads only metadata
    const bool skip_model_download = ctx_arg.ex == MONOWIRE_EXAMPLE_EXPORT_GRAPH_OPS;
    ...
    if (params.model.path.empty() && ctx_arg.ex != MONOWIRE_EXAMPLE_SERVER && !skip_model_download && !params.usage
        && !params.completion) {
    ...
    if (params.escape) {
        ...
        for (auto &antiprompt : params.antiprompt) { string_process_escapes(antiprompt); }
        for (auto &seq_breaker : params.sampling.dry_sequence_breakers) { string_process_escapes(seq_breaker); }
    ...
    // pad tensor_buft_overrides for monowire_params_fit:
    const size_t ntbo = monowire_max_tensor_buft_overrides();
    while (params.tensor_buft_overrides.size() < ntbo) { params.tensor_buft_overrides.push_back({nullptr, nullptr}); }
    ...
    if (!params.chat_template.empty() && !common_chat_verify_template(params.chat_template, params.use_jinja)) {
        throw std::runtime_error(string_format(
            ...
            params.use_jinja
                ? ""
                : "\nnote: Monowire was started without --jinja, we only support commonly used templates"));
    }
...
static void common_params_print_usage(common_params_context &ctx_arg) {
    auto print_options = [](std::vector<common_arg *> &options) {
        for (common_arg *opt : options) { printf("%s", opt->to_string().c_str()); }
    };
    ...
    for (auto &opt : ctx_arg.options) {
        // in case multiple MONOWIRE_EXAMPLE_* are set, we prioritize the MONOWIRE_EXAMPLE_* matching current example
        if (opt.is_sparam) {
    ...
    // TODO: maybe convert enum monowire_example to string
...
static void common_params_print_completion(common_params_context &ctx_arg) {
    ...
    auto print_options = [](const std::vector<common_arg *> &options) {
        for (const common_arg *opt : options) {
            for (const char *arg : opt->args) { printf("%s ", arg); }
        }
    };
    ...
    std::set<std::string> executables = {"monowire-bench", "monowire-cli", "monowire-quantize"};

    for (const auto &exe : executables) { printf("complete -F _monowire_completions %s\n", exe.c_str()); }
}
...
bool common_params_to_map(int argc, char **argv, monowire_example ex, std::map<common_arg, std::string> &out_map) {
    common_params dummy_params;
    common_params_context ctx_arg = common_params_parser_init(dummy_params, ex, nullptr);

    std::unordered_map<std::string, common_arg *> arg_to_options;
    for (auto &opt : ctx_arg.options) {
        for (const auto &arg : opt.args) { arg_to_options[arg] = &opt; }
        for (const auto &arg : opt.args_neg) { arg_to_options[arg] = &opt; }
    }
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!seen_args.insert(arg).second) {
            LOG_WRN("DEPRECATED: argument '%s' specified multiple times, use comma-separated values instead (only last "
                    "value will be used)\n",
                    arg.c_str());
        }
...
bool common_params_parse(int argc, char **argv, common_params &params, monowire_example ex,
...
static std::string list_builtin_chat_templates() {
    ...
    int32_t res = monowire_chat_builtin_templates(nullptr, 0);
    supported_tmpl.resize(res);
    res = monowire_chat_builtin_templates(supported_tmpl.data(), supported_tmpl.size());
    std::ostringstream msg;
    for (auto &tmpl : supported_tmpl) { msg << tmpl << (&tmpl == &supported_tmpl.back() ? "" : ", "); }
    return msg.str();
}
...
bool common_arg_utils::is_autoy(const std::string &value) { return value == "auto" || value == "-1"; }
...
common_params_context common_params_parser_init(common_params &params, monowire_example ex,
                                                void (*print_usage)(int, char **)) {
    // per-example default params
    // Define this here so Monowire documentation generation includes it.
    if (ex == MONOWIRE_EXAMPLE_COMPLETION) {
        params.use_jinja = false; // disable jinja by default

    } else if (ex == MONOWIRE_EXAMPLE_MTMD) {
        params.use_jinja = false;   // disable jinja by default
        params.sampling.temp = 0.2; // lower temp by default for better quality

    } else if (ex == MONOWIRE_EXAMPLE_SERVER) {
     ...
     * rules:
     * - all examples inherit options from MONOWIRE_EXAMPLE_COMMON
     * - if MONOWIRE_EXAMPLE_* is set (other than COMMON), we only show the option in the corresponding example
     * - if both {MONOWIRE_EXAMPLE_COMMON, MONOWIRE_EXAMPLE_*,} are set, we will prioritize the MONOWIRE_EXAMPLE_*
     * matching current example
     */
    auto add_opt = [&](common_arg arg) {
        if ((arg.in_example(ex) || arg.in_example(MONOWIRE_EXAMPLE_COMMON)) && !arg.is_exclude(ex)) {
    ...
    add_opt(common_arg({"--version"}, "show version and build info", [](common_params &) {
        fprintf(stderr, "version: %d (%s)\n", monowire_build_number(), monowire_commit());
        fprintf(stderr, "built with %s for %s\n", monowire_compiler(), monowire_build_target());
        exit(0);
    }));
    add_opt(common_arg({"--license"}, "show source code license and dependencies", [](common_params &) {
        for (int i = 0; LICENSES[i]; ++i) { printf("%s\n", LICENSES[i]); }
        exit(0);
    }));
    add_opt(common_arg({"-cl", "--cache-list"}, "show list of models in cache", [](common_params &) {
        auto models = common_list_cached_models();
        printf("number of models in cache: %zu\n", models.size());
        for (size_t i = 0; i < models.size(); i++) { printf("%4zu. %s\n", i + 1, models[i].to_string().c_str()); }
        exit(0);
    }));
    add_opt(common_arg({"--completion-bash"}, "print source-able bash completion script for Monowire",
    ...
    add_opt(common_arg({"--verbose-prompt"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_EMBEDDING,
                               MONOWIRE_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg({"--display-prompt"}, {"--no-display-prompt"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI}));
    add_opt(common_arg({"-co", "--color"}, "[on|off|auto]",
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SPECULATIVE,
                               MONOWIRE_EXAMPLE_LOOKUP}));
    ...
    add_opt(common_arg(
                ...
                [](common_params &params, const std::string &value) { params.speculative.lookup_cache_static = value; })
                .set_examples({MONOWIRE_EXAMPLE_LOOKUP, MONOWIRE_EXAMPLE_SERVER}));
    ...
    add_opt(common_arg({"-c", "--ctx-size"}, "N",
                       ...
                       [](common_params &params, int value) {
                           params.n_ctx = value;
                           if (value == 0) {
                               // disable context reduction in monowire_params_fit if the user explicitly requests the
                               // full context size:
                               params.fit_params_min_ctx = UINT32_MAX;
                           }
                       })
                .set_env("MONOWIRE_ARG_CTX_SIZE"));
    add_opt(common_arg({"-n", "--predict", "--n-predict"}, "N",
                       string_format(
                           ex == MONOWIRE_EXAMPLE_COMPLETION
                               ? "number of tokens to predict (default: %d, -1 = infinity, -2 = until context filled)"
                               : "number of tokens to predict (default: %d, -1 = infinity)",
                           params.n_predict),
                       [](common_params &params, int value) { params.n_predict = value; })
                .set_env("MONOWIRE_ARG_N_PREDICT"));
    ...
    add_opt(common_arg({"--swa-full"},
                       string_format("use full-size SWA cache (default: %s)\n"
                                     "more info: upstream SWA cache reference",
                                     params.swa_full ? "true" : "false"),
                       [](common_params &params) { params.swa_full = true; })
                .set_env("MONOWIRE_ARG_SWA_FULL"));
    add_opt(common_arg({"-ctxcp", "--ctx-checkpoints", "--swa-checkpoints"}, "N",
                       string_format("max number of context checkpoints to create per slot (default: %d)\n"
                                     "more info: upstream context checkpoint reference",
                                     params.n_ctx_checkpoints),
                       [](common_params &params, int value) { params.n_ctx_checkpoints = value; })
                .set_env("MONOWIRE_ARG_CTX_CHECKPOINTS")
                .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(
        common_arg(
            {"-cpent", "--checkpoint-every-n-tokens"}, "N",
            string_format("create a checkpoint every n tokens during prefill (processing), -1 to disable (default: %d)",
                          params.checkpoint_every_nt),
            [](common_params &params, int value) { params.checkpoint_every_nt = value; })
            .set_env("MONOWIRE_ARG_CHECKPOINT_EVERY_NT")
            .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(common_arg({"-cram", "--cache-ram"}, "N",
                       string_format("set the maximum cache size in MiB (default: %d, -1 - no limit, 0 - disable)\n"
                                     "more info: upstream cache sizing reference",
                                     params.cache_ram_mib),
                       [](common_params &params, int value) { params.cache_ram_mib = value; })
                .set_env("MONOWIRE_ARG_CACHE_RAM")
                .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(
        common_arg(
            ...
            .set_env("MONOWIRE_ARG_KV_UNIFIED")
            .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_PERPLEXITY, MONOWIRE_EXAMPLE_BATCHED,
                           MONOWIRE_EXAMPLE_BENCH, MONOWIRE_EXAMPLE_PARALLEL}));
    add_opt(common_arg({"--cache-idle-slots"}, {"--no-cache-idle-slots"},
                ...
                .set_env("MONOWIRE_ARG_CACHE_IDLE_SLOTS")
                .set_examples({MONOWIRE_EXAMPLE_SERVER}));
    add_opt(common_arg({"--context-shift"}, {"--no-context-shift"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER,
                               MONOWIRE_EXAMPLE_IMATRIX, MONOWIRE_EXAMPLE_PERPLEXITY})
                .set_env("MONOWIRE_ARG_CONTEXT_SHIFT"));
    add_opt(common_arg({"--chunks"}, "N",
                       string_format("max number of chunks to process (default: %d, -1 = all)", params.n_chunks),
                       [](common_params &params, int value) { params.n_chunks = value; })
                .set_examples({MONOWIRE_EXAMPLE_IMATRIX, MONOWIRE_EXAMPLE_PERPLEXITY, MONOWIRE_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg({"-fa", "--flash-attn"}, "[on|off|auto]",
                       string_format("set Flash Attention use ('on', 'off', or 'auto', default: '%s')",
                                     monowire_flash_attn_type_name(params.flash_attn_type)),
                       [](common_params &params, const std::string &value) {
                           if (is_truthy(value)) {
                               ...
                               params.flash_attn_type = MONOWIRE_FLASH_ATTN_TYPE_AUTO;
                           } else {
                               throw std::runtime_error(
                                   string_format("error: unknown value for --flash-attn: '%s'\n", value.c_str()));
                           }
                       })
                .set_env("MONOWIRE_ARG_FLASH_ATTN"));
    add_opt(common_arg({"-p", "--prompt"}, "PROMPT", "prompt to start generation with; for system message, use -sys",
                ...
                .set_excludes({MONOWIRE_EXAMPLE_SERVER}));
    add_opt(common_arg({"-sys", "--system-prompt"}, "PROMPT",
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_DIFFUSION,
                               MONOWIRE_EXAMPLE_MTMD}));
    add_opt(common_arg({"--perf"}, {"--no-perf"},
                       string_format("whether to enable internal runtime performance timings (default: %s)",
                                     params.no_perf ? "true" : "false"),
                       [](common_params &params, bool value) {
                           params.no_perf = !value;
                           params.sampling.no_perf = !value;
                       })
                .set_env("MONOWIRE_ARG_PERF"));
    add_opt(common_arg({"--show-timings"}, {"--no-show-timings"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_SHOW_TIMINGS"));
    ...
    add_opt(common_arg({"--in-file"}, "FNAME", "an input file (use comma-separated values to specify multiple files)",
                ...
                .set_examples({MONOWIRE_EXAMPLE_IMATRIX}));
    ...
    add_opt(common_arg({"-ptc", "--print-token-count"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION}));
    ...
    add_opt(common_arg({"--warmup"}, {"--no-warmup"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER,
                               MONOWIRE_EXAMPLE_MTMD, MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_RETRIEVAL,
                               MONOWIRE_EXAMPLE_PERPLEXITY, MONOWIRE_EXAMPLE_DEBUG}));
    ...
    add_opt(common_arg({"--adaptive-target"}, "N",
                       string_format("adaptive-p: select tokens near this probability (valid range 0.0 "
                                     ...
                                     "more info: upstream adaptive sampling reference",
    ...
    add_opt(
        common_arg({"-j", "--json-schema"}, "SCHEMA",
                   ...
                   "object\nFor schemas w/ external $refs, use --grammar + scripts/json_schema_to_grammar.py instead",
                   [](common_params &params, const std::string &value) {
                       params.sampling.grammar
                           = {COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT, json_schema_to_grammar(json::parse(value))};
                   })
            .set_sparam());
    add_opt(common_arg(
                ...
                [](common_params &params, const std::string &value) {
                    ...
                    params.sampling.grammar
                        = {COMMON_GRAMMAR_TYPE_OUTPUT_FORMAT, json_schema_to_grammar(json::parse(schema))};
                })
                .set_sparam());
    add_opt(common_arg({"-bs", "--backend-sampling"}, "enable backend sampling (experimental) (default: disabled)",
                ...
                .set_sparam()
                .set_env("MONOWIRE_ARG_BACKEND_SAMPLING"));
    add_opt(common_arg({"--pooling"}, "{none,mean,cls,last,rank}",
                       "pooling type for embeddings, use model default if unspecified",
                       [](common_params &params, const std::string &value) {
                           /**/ if (value == "none") {
                               params.pooling_type = MONOWIRE_POOLING_TYPE_NONE;
                           } else if (value == "mean") {
                               params.pooling_type = MONOWIRE_POOLING_TYPE_MEAN;
                           } else if (value == "cls") {
                               params.pooling_type = MONOWIRE_POOLING_TYPE_CLS;
                           } else if (value == "last") {
                               params.pooling_type = MONOWIRE_POOLING_TYPE_LAST;
                           } else if (value == "rank") {
                               params.pooling_type = MONOWIRE_POOLING_TYPE_RANK;
                           } else {
                               throw std::invalid_argument("invalid value");
                           }
                       })
                .set_examples({MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_RETRIEVAL, MONOWIRE_EXAMPLE_SERVER,
                               MONOWIRE_EXAMPLE_DEBUG})
                .set_env("MONOWIRE_ARG_POOLING"));
    add_opt(common_arg({"--attention"}, "{causal,non-causal}",
                       "attention type for embeddings, use model default if unspecified",
                       [](common_params &params, const std::string &value) {
                           /**/ if (value == "causal") {
                               params.attention_type = MONOWIRE_ATTENTION_TYPE_CAUSAL;
                           } else if (value == "non-causal") {
                               params.attention_type = MONOWIRE_ATTENTION_TYPE_NON_CAUSAL;
                           } else {
                               throw std::invalid_argument("invalid value");
                           }
                       })
                .set_examples({MONOWIRE_EXAMPLE_EMBEDDING}));
    add_opt(common_arg({"--rope-scaling"}, "{none,linear,yarn}",
                       "RoPE frequency scaling method, defaults to linear unless specified by the model",
                       [](common_params &params, const std::string &value) {
                           /**/ if (value == "none") {
                               params.rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_NONE;
                           } else if (value == "linear") {
                               params.rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_LINEAR;
                           } else if (value == "yarn") {
                               params.rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_YARN;
                           } else {
                               throw std::invalid_argument("invalid value");
                           }
                       })
                .set_env("MONOWIRE_ARG_ROPE_SCALING_TYPE"));
    add_opt(common_arg({"--rope-scale"}, "N", "RoPE context scaling factor, expands context by a factor of N",
                       [](common_params &params, const std::string &value) {
                           params.rope_freq_scale = 1.0f / std::stof(value);
                       })
                .set_env("MONOWIRE_ARG_ROPE_SCALE"));
    add_opt(
        common_arg({"--rope-freq-base"}, "N",
            ...
            .set_env("MONOWIRE_ARG_ROPE_FREQ_BASE"));
    add_opt(
        common_arg({"--rope-freq-scale"}, "N", "RoPE frequency scaling factor, expands context by a factor of 1/N",
            ...
            .set_env("MONOWIRE_ARG_ROPE_FREQ_SCALE"));
    add_opt(common_arg({"--yarn-orig-ctx"}, "N",
                ...
                .set_env("MONOWIRE_ARG_YARN_ORIG_CTX"));
    add_opt(
        common_arg({"--yarn-ext-factor"}, "N",
            ...
            .set_env("MONOWIRE_ARG_YARN_EXT_FACTOR"));
    add_opt(
        common_arg({"--yarn-attn-factor"}, "N",
            ...
            .set_env("MONOWIRE_ARG_YARN_ATTN_FACTOR"));
    add_opt(
        common_arg({"--yarn-beta-slow"}, "N",
            ...
            .set_env("MONOWIRE_ARG_YARN_BETA_SLOW"));
    add_opt(
        common_arg({"--yarn-beta-fast"}, "N",
            ...
            .set_env("MONOWIRE_ARG_YARN_BETA_FAST"));
    add_opt(common_arg({"-gan", "--grp-attn-n"}, "N",
                ...
                .set_env("MONOWIRE_ARG_GRP_ATTN_N")
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_PASSKEY}));
    add_opt(common_arg({"-gaw", "--grp-attn-w"}, "N",
                ...
                .set_env("MONOWIRE_ARG_GRP_ATTN_W")
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION}));
    add_opt(common_arg({"-kvo", "--kv-offload"}, {"-nkvo", "--no-kv-offload"},
                ...
                .set_env("MONOWIRE_ARG_KV_OFFLOAD"));
    ...
    add_opt(common_arg({"-ctk", "--cache-type-k"}, "TYPE",
                ...
                .set_env("MONOWIRE_ARG_CACHE_TYPE_K"));
    add_opt(common_arg({"-ctv", "--cache-type-v"}, "TYPE",
                ...
                .set_env("MONOWIRE_ARG_CACHE_TYPE_V"));
    add_opt(common_arg({"--hellaswag"}, "compute HellaSwag score over random tasks from datafile supplied with -f",
                ...
                .set_examples({MONOWIRE_EXAMPLE_PERPLEXITY}));
    ...
    add_opt(common_arg({"--ppl-output-type"}, "<0|1>",
                ...
                .set_examples({MONOWIRE_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg({"-dt", "--defrag-thold"}, "N", string_format("KV cache defragmentation threshold (DEPRECATED)"),
                       [](common_params &params, const std::string &value) {
                           GGML_UNUSED(params);
                           GGML_UNUSED(value);
                           LOG_WRN("DEPRECATED: --defrag-thold is deprecated and no longer necessary to specify\n");
                       })
                .set_env("MONOWIRE_ARG_DEFRAG_THOLD"));
    if (ex == MONOWIRE_EXAMPLE_SERVER) {
        ...
        add_opt(common_arg({"-np", "--parallel"}, "N",
                    ...
                    .set_env("MONOWIRE_ARG_N_PARALLEL")
                    .set_examples({MONOWIRE_EXAMPLE_SERVER}));
    } else {
        add_opt(common_arg({"-np", "--parallel"}, "N",
    ...
    add_opt(common_arg({"-ns", "--sequences"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_PARALLEL}));
    add_opt(common_arg({"-cb", "--cont-batching"}, {"-nocb", "--no-cont-batching"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_CONT_BATCHING"));
    add_opt(common_arg({"-mm", "--mmproj"}, "FILE",
                ...
                .set_env("MONOWIRE_ARG_MMPROJ"));
    add_opt(common_arg({"-mmu", "--mmproj-url"}, "URL", "URL to a multimodal projector file. see tools/mtmd/README.md",
                       [](common_params &params, const std::string &value) { params.mmproj.url = value; })
                .set_examples(mmproj_examples)
                .set_env("MONOWIRE_ARG_MMPROJ_URL"));
    add_opt(
        common_arg({"--mmproj-auto"}, {"--no-mmproj", "--no-mmproj-auto"},
            ...
            .set_examples(mmproj_examples)
            .set_env("MONOWIRE_ARG_MMPROJ_AUTO"));
    add_opt(common_arg({"--mmproj-offload"}, {"--no-mmproj-offload"},
                ...
                .set_examples(mmproj_examples)
                .set_env("MONOWIRE_ARG_MMPROJ_OFFLOAD"));
    add_opt(common_arg({"--image", "--audio"}, "FILE",
                       ...
                       "multiple files\n",
                       [](common_params &params, const std::string &value) {
                           for (const auto &item : parse_csv_row(value)) { params.image.emplace_back(item); }
                       })
                .set_examples({MONOWIRE_EXAMPLE_MTMD, MONOWIRE_EXAMPLE_CLI}));
    add_opt(common_arg({"--image-min-tokens"}, "N",
                ...
                .set_examples(mmproj_examples)
                .set_env("MONOWIRE_ARG_IMAGE_MIN_TOKENS"));
    add_opt(common_arg({"--image-max-tokens"}, "N",
                ...
                .set_examples(mmproj_examples)
                .set_env("MONOWIRE_ARG_IMAGE_MAX_TOKENS"));
    ...
    add_opt(common_arg({"--mlock"}, "force system to keep model in RAM rather than swapping or compressing",
                ...
                .set_env("MONOWIRE_ARG_MLOCK"));
    add_opt(common_arg({"--streaming-budget-mib"}, "N",
                       "preserve up to N MiB of mmap-backed model weights in RAM using balanced memory-aware streaming "
                       "tensor selection",
                       [](common_params &params, int value) {
                           if (value < 0) {
                               throw std::invalid_argument("value must be non-negative");
                           }
                           params.streaming_budget_bytes = uint64_t(value) * 1024ULL * 1024ULL;
                       })
                .set_env("MONOWIRE_ARG_STREAMING_BUDGET_MIB"));
    add_opt(common_arg({"--streaming-window"}, "N", "set the memory streaming runtime prefetch window to N layers",
                       [](common_params &params, int value) {
                           if (value < 0) {
                               throw std::invalid_argument("value must be non-negative");
                           }
                           params.streaming_window = value;
                       })
                .set_env("MONOWIRE_ARG_STREAMING_WINDOW"));
    add_opt(common_arg({"--streaming-io-threads"}, "N",
                       "set the number of background I/O hint workers for memory streaming (0 = synchronous)",
                       [](common_params &params, int value) {
                           if (value < 0) {
                               throw std::invalid_argument("value must be non-negative");
                           }
                           params.streaming_io_threads = value;
                       })
                .set_env("MONOWIRE_ARG_STREAMING_IO_THREADS"));
    add_opt(common_arg(
                {"--streaming-prefetch"}, {"--no-streaming-prefetch"},
                string_format(
                    "enable memory streaming preserved-range prefetch and runtime layer-window streaming (default: %s)",
                    params.streaming_prefetch ? "enabled" : "disabled"),
                [](common_params &params, bool value) { params.streaming_prefetch = value; })
                .set_env("MONOWIRE_ARG_STREAMING_PREFETCH"));
    add_opt(common_arg({"--streaming-evict"}, {"--no-streaming-evict"},
                       string_format("evict dynamic streaming ranges after each layer (default: %s)",
                                     params.streaming_evict ? "enabled" : "disabled"),
                       [](common_params &params, bool value) { params.streaming_evict = value; })
                .set_env("MONOWIRE_ARG_STREAMING_EVICT"));
    add_opt(common_arg({"--streaming-read-prefetch"}, {"--no-streaming-read-prefetch"},
                       string_format("actively pread runtime streaming ranges into the page cache (default: %s)",
                                     params.streaming_read_prefetch ? "enabled" : "disabled"),
                       [](common_params &params, bool value) { params.streaming_read_prefetch = value; })
                .set_env("MONOWIRE_ARG_STREAMING_READ_PREFETCH"));
    add_opt(common_arg({"--streaming-direct-buffer"}, {"--no-streaming-direct-buffer"},
                       string_format("experimentally pread streamed CPU weights into owned buffers and bind them at "
                                     "layer boundaries (default: %s)",
                                     params.streaming_direct_buffer ? "enabled" : "disabled"),
                       [](common_params &params, bool value) { params.streaming_direct_buffer = value; })
                .set_env("MONOWIRE_ARG_STREAMING_DIRECT_BUFFER"));
    ...
    add_opt(common_arg({"-ot", "--override-tensor"}, "<tensor name pattern>=<buffer type>,...",
                ...
                .set_env("MONOWIRE_ARG_OVERRIDE_TENSOR"));
    add_opt(common_arg({"-otd", "--override-tensor-draft"}, "<tensor name pattern>=<buffer type>,...",
                       "override tensor buffer type for draft model",
                       [](common_params &params, const std::string &value) {
                           parse_tensor_buffer_overrides(value, params.speculative.tensor_buft_overrides);
                       })
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(
    ...
    add_opt(common_arg({"-ncmoe", "--n-cpu-moe"}, "N",
                ...
                .set_env("MONOWIRE_ARG_N_CPU_MOE"));
    add_opt(common_arg({"-cmoed", "--cpu-moe-draft"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_CPU_MOE_DRAFT"));
    add_opt(common_arg({"-ncmoed", "--n-cpu-moe-draft"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_N_CPU_MOE_DRAFT"));
        ...
        common_arg({"-ngl", "--gpu-layers", "--n-gpu-layers"}, "N",
                   ...
                   [](common_params &params, const std::string &value) {
                       ...
                       if (!monowire_supports_gpu_offload()) {
                           ...
                           fprintf(stderr,
                                   "warning: one possible reason is that Monowire was compiled without GPU support\n");
                           fprintf(stderr, "warning: consult docs/build.md for compilation instructions\n");
                       }
                   })
            .set_env("MONOWIRE_ARG_N_GPU_LAYERS"));
    add_opt(common_arg({"-sm", "--split-mode"}, "{none,layer,row,tensor}",
                       ...
                       [](common_params &params, const std::string &value) {
                           if (value == "none") {
                               params.split_mode = MONOWIRE_SPLIT_MODE_NONE;
                           } else if (value == "layer") {
                               params.split_mode = MONOWIRE_SPLIT_MODE_LAYER;
                           } else if (value == "row") {
                               params.split_mode = MONOWIRE_SPLIT_MODE_ROW;
                           } else if (value == "tensor") {
                               params.split_mode = MONOWIRE_SPLIT_MODE_TENSOR;
                           } else {
                               throw std::invalid_argument("invalid value");
                           }
                           if (!monowire_supports_gpu_offload()) {
                               fprintf(stderr, "warning: Monowire was compiled without support for GPU offload. "
                                               "Setting the split mode has no effect.\n");
                           }
                       })
                .set_env("MONOWIRE_ARG_SPLIT_MODE"));
    add_opt(common_arg({"-ts", "--tensor-split"}, "N0,N1,N2,...",
                       ...
                       [](common_params &params, const std::string &value) {
                           ...
                           if (split_arg.size() >= monowire_max_devices()) {
                               throw std::invalid_argument(
                                   string_format("got %zu input configs, but system only has %zu devices",
                                                 split_arg.size(), monowire_max_devices()));
                           }
                ...
                .set_env("MONOWIRE_ARG_TENSOR_SPLIT"));
    ...
    add_opt(common_arg({"-fitp", "--fit-print"}, "[on|off]",
                ...
                .set_examples({MONOWIRE_EXAMPLE_FIT_PARAMS})
                .set_env("MONOWIRE_ARG_FIT_ESTIMATE"));
    add_opt(common_arg({"-fitt", "--fit-target"}, "MiB0,MiB1,MiB2,...",
                ...
                .set_env("MONOWIRE_ARG_FIT_TARGET"));
    ...
    add_opt(common_arg({"--lora"}, "FNAME",
                       ...
                       // we define this arg on both COMMON and EXPORT_LORA, so when showing help message of
                       // export-lora, it will be categorized as "example-specific" arg
                       )
                .set_examples({MONOWIRE_EXAMPLE_COMMON, MONOWIRE_EXAMPLE_EXPORT_LORA}));
    add_opt(common_arg({"--lora-scaled"}, "FNAME:SCALE,...",
    ...
    add_opt(common_arg({"--tags"}, "STRING", "set model tags, comma-separated (informational, not used for routing)",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_TAGS"));
    add_opt(common_arg({"-m", "--model"}, "FNAME",
                       ex == MONOWIRE_EXAMPLE_EXPORT_LORA ? "model path from which to load base model"
                                                          : "model path to load",
                       [](common_params &params, const std::string &value) { params.model.path = value; })
                .set_examples({MONOWIRE_EXAMPLE_COMMON, MONOWIRE_EXAMPLE_EXPORT_LORA})
                .set_env("MONOWIRE_ARG_MODEL"));
    add_opt(common_arg({"-mu", "--model-url"}, "MODEL_URL", "model download url (default: unused)",
                ...
                .set_env("MONOWIRE_ARG_MODEL_URL"));
    add_opt(
        common_arg(
            ...
            .set_env("MONOWIRE_ARG_DOCKER_REPO"));
    add_opt(common_arg({"-hf", "-hfr", "--hf-repo"}, "<user>/<model>[:quant]",
                ...
                .set_env("MONOWIRE_ARG_HF_REPO"));
    add_opt(common_arg(
                {"-hfd", "-hfrd", "--hf-repo-draft"}, "<user>/<model>[:quant]",
                "Same as --hf-repo, but for the draft model (default: unused)",
                [](common_params &params, const std::string &value) { params.speculative.mparams_dft.hf_repo = value; })
                .set_env("MONOWIRE_ARG_HFD_REPO"));
    add_opt(
        common_arg({"-hff", "--hf-file"}, "FILE",
                   "Hugging Face model file. If specified, it will override the quant in --hf-repo (default: unused)",
                   [](common_params &params, const std::string &value) { params.model.hf_file = value; })
            .set_env("MONOWIRE_ARG_HF_FILE"));
    add_opt(common_arg({"-hfv", "-hfrv", "--hf-repo-v"}, "<user>/<model>[:quant]",
                ...
                .set_env("MONOWIRE_ARG_HF_REPO_V"));
    add_opt(common_arg({"-hffv", "--hf-file-v"}, "FILE",
                ...
                .set_env("MONOWIRE_ARG_HF_FILE_V"));
    ...
    add_opt(common_arg({"--context-file"}, "FNAME",
                ...
                .set_examples({MONOWIRE_EXAMPLE_RETRIEVAL}));
    ...
    add_opt(common_arg({"--junk"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_PASSKEY, MONOWIRE_EXAMPLE_PARALLEL}));
    add_opt(common_arg({"--pos"}, "N",
                       string_format("position of the passkey in the junk text (default: %d)", params.i_pos),
                       [](common_params &params, int value) { params.i_pos = value; })
                .set_examples({MONOWIRE_EXAMPLE_PASSKEY}));
    add_opt(common_arg({"-o", "--output", "--output-file"}, "FNAME",
                ...
                .set_examples({MONOWIRE_EXAMPLE_IMATRIX, MONOWIRE_EXAMPLE_CVECTOR_GENERATOR,
                               MONOWIRE_EXAMPLE_EXPORT_LORA, MONOWIRE_EXAMPLE_TTS, MONOWIRE_EXAMPLE_FINETUNE,
                               MONOWIRE_EXAMPLE_RESULTS, MONOWIRE_EXAMPLE_EXPORT_GRAPH_OPS}));
    ...
    add_opt(common_arg({"-pps"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_BENCH, MONOWIRE_EXAMPLE_PARALLEL}));
    ...
    add_opt(common_arg({"-npp"}, "n0,n1,...", "number of prompt tokens",
                ...
                .set_examples({MONOWIRE_EXAMPLE_BENCH}));
    ...
    add_opt(common_arg({"--embd-normalize"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_DEBUG}));
    ...
    add_opt(common_arg({"--api-prefix"}, "PREFIX",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_API_PREFIX"));
    add_opt(common_arg({"--webui-config"}, "JSON",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_WEBUI_CONFIG"));
    add_opt(
        common_arg({"--webui-config-file"}, "PATH",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SERVER})
            .set_env("MONOWIRE_ARG_WEBUI_CONFIG_FILE"));
    add_opt(common_arg({"--webui-mcp-proxy"}, {"--no-webui-mcp-proxy"},
                       string_format("experimental: whether to enable MCP CORS proxy - do not enable in untrusted "
                                     "environments (default: %s)",
                                     params.webui_mcp_proxy ? "enabled" : "disabled"),
                       [](common_params &params, bool value) { params.webui_mcp_proxy = value; })
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_WEBUI_MCP_PROXY"));
    add_opt(
        common_arg({"--tools"}, "TOOL1,TOOL2,...",
                   ...
                   "available tools: read_file, file_glob_search, grep_search, exec_shell_command, write_file, "
                   "edit_file, apply_diff",
                   [](common_params &params, const std::string &value) { params.server_tools = parse_csv_row(value); })
            .set_examples({MONOWIRE_EXAMPLE_SERVER})
            .set_env("MONOWIRE_ARG_TOOLS"));
    add_opt(
    ...
    add_opt(
        common_arg(
            ...
            .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_DEBUG})
            .set_env("MONOWIRE_ARG_EMBEDDINGS"));
    add_opt(common_arg({"--rerank", "--reranking"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_RERANKING"));
    ...
    add_opt(common_arg({"--ssl-key-file"}, "FNAME", "path to file a PEM-encoded SSL private key",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_SSL_KEY_FILE"));
    add_opt(common_arg({"--ssl-cert-file"}, "FNAME", "path to file a PEM-encoded SSL certificate",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_SSL_CERT_FILE"));
    add_opt(common_arg({"--chat-template-kwargs"}, "STRING",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_CHAT_TEMPLATE_KWARGS"));
    ...
    add_opt(common_arg({"--cache-prompt"}, {"--no-cache-prompt"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_CACHE_PROMPT"));
    add_opt(common_arg({"--cache-reuse"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_CACHE_REUSE"));
    add_opt(common_arg({"--metrics"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_ENDPOINT_METRICS"));
    add_opt(common_arg({"--props"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_ENDPOINT_PROPS"));
    add_opt(common_arg({"--slots"}, {"--no-slots"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_ENDPOINT_SLOTS"));
    ...
    add_opt(common_arg({"--models-dir"}, "PATH",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_MODELS_DIR"));
    add_opt(common_arg({"--models-preset"}, "PATH",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_MODELS_PRESET"));
    add_opt(common_arg(
                {"--models-max"}, "N",
                string_format(
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_MODELS_MAX"));
    add_opt(common_arg({"--models-autoload"}, {"--no-models-autoload"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_MODELS_AUTOLOAD"));
    add_opt(common_arg({"--jinja"}, {"--no-jinja"},
                ...
                .set_examples(
                    {MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_MTMD})
                .set_env("MONOWIRE_ARG_JINJA"));
    add_opt(common_arg({"--reasoning-format"}, "FORMAT",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_THINK"));
    add_opt(common_arg(
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_REASONING"));
    add_opt(
        common_arg(
            {"--reasoning-budget"}, "N",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI})
            .set_env("MONOWIRE_ARG_THINK_BUDGET"));
    add_opt(
        common_arg(
            ...
            .set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI})
            .set_env("MONOWIRE_ARG_THINK_BUDGET_MESSAGE"));
    add_opt(
        common_arg({"--chat-template"}, "JINJA_TEMPLATE",
            ...
            .set_examples(
                {MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_MTMD})
            .set_env("MONOWIRE_ARG_CHAT_TEMPLATE"));
    add_opt(
        common_arg({"--chat-template-file"}, "JINJA_TEMPLATE_FILE",
            ...
            .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER})
            .set_env("MONOWIRE_ARG_CHAT_TEMPLATE_FILE"));
    add_opt(common_arg(
                ...
                .set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_SKIP_CHAT_PARSING"));
    add_opt(common_arg({"--prefill-assistant"}, {"--no-prefill-assistant"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_SERVER})
                .set_env("MONOWIRE_ARG_PREFILL_ASSISTANT"));
    ...
    add_opt(common_arg({"--positive-file"}, "FNAME",
                ...
                .set_examples({MONOWIRE_EXAMPLE_CVECTOR_GENERATOR}));
    ...
    add_opt(common_arg({"--output-format"}, "{md,jsonl}", "output format for batched-bench results (default: md)",
                ...
                .set_examples({MONOWIRE_EXAMPLE_BENCH}));
    add_opt(common_arg({"--log-disable"}, "Log disable", [](common_params &) { common_log_pause(common_log_main()); }));
    add_opt(common_arg({"--log-file"}, "FNAME", "Log to file", [](common_params &, const std::string &value) {
                common_log_set_file(common_log_main(), value.c_str());
            }).set_env("MONOWIRE_LOG_FILE"));
    ...
    add_opt(common_arg({"-td", "--threads-draft"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER}));
    ...
    add_opt(common_arg({"-Cd", "--cpu-mask-draft"}, "M",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE}));
    ...
    add_opt(common_arg({"--prio-draft"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg({"--poll-draft"}, "<0|1>", "Use polling to wait for draft model work (default: same as --poll])",
    ...
    add_opt(common_arg({"--poll-batch-draft"}, "<0|1>",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg({"--draft", "--draft-n", "--draft-max"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_LOOKUP, MONOWIRE_EXAMPLE_SERVER,
                               MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_DRAFT_MAX"));
    add_opt(common_arg({"--draft-min", "--draft-n-min"}, "N",
                ...
                .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_LOOKUP, MONOWIRE_EXAMPLE_SERVER,
                               MONOWIRE_EXAMPLE_CLI})
                .set_env("MONOWIRE_ARG_DRAFT_MIN"));
    add_opt(
        common_arg(
            {"--draft-p-split"}, "P",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE})
            .set_env("MONOWIRE_ARG_DRAFT_P_SPLIT"));
    add_opt(
        common_arg({"--draft-p-min"}, "P",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
            .set_env("MONOWIRE_ARG_DRAFT_P_MIN"));
    add_opt(
        common_arg({"-cd", "--ctx-size-draft"}, "N",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
            .set_env("MONOWIRE_ARG_CTX_SIZE_DRAFT"));
    ...
    add_opt(
        common_arg({"-ngld", "--gpu-layers-draft", "--n-gpu-layers-draft"}, "N",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
            .set_env("MONOWIRE_ARG_N_GPU_LAYERS_DRAFT"));
    add_opt(
        common_arg({"-md", "--model-draft"}, "FNAME", "draft model for speculative decoding (default: unused)",
                   [](common_params &params, const std::string &value) { params.speculative.mparams_dft.path = value; })
            .set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI})
            .set_env("MONOWIRE_ARG_MODEL_DRAFT"));
    ...
    add_opt(
        common_arg({"--spec-type"}, "[none|ngram-cache|ngram-simple|ngram-map-k|ngram-map-k4v|ngram-mod]",
            ...
            .set_examples({MONOWIRE_EXAMPLE_SERVER})
            .set_env("MONOWIRE_ARG_SPEC_TYPE"));
    add_opt(
        common_arg(
            {"--spec-ngram-size-n"}, "N",
            string_format(
    ...
    add_opt(common_arg({"-ctkd", "--cache-type-k-draft"}, "TYPE",
                ...
                .set_env("MONOWIRE_ARG_CACHE_TYPE_K_DRAFT"));
    add_opt(common_arg({"-ctvd", "--cache-type-v-draft"}, "TYPE",
                ...
                .set_env("MONOWIRE_ARG_CACHE_TYPE_V_DRAFT"));
    ...
    add_opt(common_arg({"-mv", "--model-vocoder"}, "FNAME", "vocoder model for audio generation (default: unused)",
                ...
                .set_examples({MONOWIRE_EXAMPLE_TTS, MONOWIRE_EXAMPLE_SERVER}));
    ...
    add_opt(
        common_arg({"-lr", "--learning-rate"}, "ALPHA",
            ...
            .set_examples({MONOWIRE_EXAMPLE_FINETUNE}));
    ...
    add_opt(
        common_arg({"--check"},
            ...
            .set_examples({MONOWIRE_EXAMPLE_RESULTS}));
    add_opt(common_arg({"--save-logits"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_DEBUG}));
    ...
    add_opt(common_arg({"--embd-gemma-default"},
                ...
                .set_examples({MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_SERVER}));

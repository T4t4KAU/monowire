...
bool common_arg::in_example(enum monowire_example ex) {
    return examples.find(ex) != examples.end();
}

bool common_arg::is_exclude(enum monowire_example ex) {
...
static void parse_tensor_buffer_overrides(const std::string & value, std::vector<monowire_model_tensor_buft_override> & overrides) {
...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    if (params.model.path.empty() && ctx_arg.ex != MONOWIRE_EXAMPLE_SERVER && !skip_model_download && !params.usage && !params.completion) {
...
static void common_params_print_completion(common_params_context & ctx_arg) {
    ...
    for (const auto& exe : executables) {
        printf("complete -F _monowire_completions %s\n", exe.c_str());
    }
}
...
bool common_params_parse(int argc, char ** argv, common_params & params, monowire_example ex, void(*print_usage)(int, char **)) {
...
common_params_context common_params_parser_init(common_params & params, monowire_example ex, void(*print_usage)(int, char **)) {
     ...
     * - if both {MONOWIRE_EXAMPLE_COMMON, MONOWIRE_EXAMPLE_*,} are set, we will prioritize the MONOWIRE_EXAMPLE_* matching current example
     */
    ...
    add_opt(common_arg(
        {"--completion-bash"},
        "print source-able bash completion script for Monowire",
        [](common_params & params) {
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg(
        {"--display-prompt"},
        {"--no-display-prompt"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_LOOKUP}));
    add_opt(common_arg(
        {"-t", "--threads"}, "N",
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            params.n_ctx = value;
            if (value == 0) {
                // disable context reduction in monowire_params_fit if the user explicitly requests the full context size:
    ...
    add_opt(common_arg(
        ...
        string_format("use full-size SWA cache (default: %s)\n"
            "more info: upstream SWA cache reference", params.swa_full ? "true" : "false"),
        [](common_params & params) {
    ...
    add_opt(common_arg(
        ...
        string_format("max number of context checkpoints to create per slot (default: %d)\n"
            "more info: upstream context checkpoint reference", params.n_ctx_checkpoints),
        [](common_params & params, int value) {
            params.n_ctx_checkpoints = value;
        }
    ).set_env("MONOWIRE_ARG_CTX_CHECKPOINTS").set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_env("MONOWIRE_ARG_CHECKPOINT_EVERY_NT").set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-cram", "--cache-ram"}, "N",
        string_format("set the maximum cache size in MiB (default: %d, -1 - no limit, 0 - disable)\n"
            "more info: upstream cache sizing reference", params.cache_ram_mib),
        [](common_params & params, int value) {
            params.cache_ram_mib = value;
        }
    ).set_env("MONOWIRE_ARG_CACHE_RAM").set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-kvu", "--kv-unified"},
    ...
    ).set_env("MONOWIRE_ARG_KV_UNIFIED").set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_PERPLEXITY, MONOWIRE_EXAMPLE_BATCHED, MONOWIRE_EXAMPLE_BENCH, MONOWIRE_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--cache-idle-slots"},
        {"--no-cache-idle-slots"},
    ...
    ).set_env("MONOWIRE_ARG_CACHE_IDLE_SLOTS").set_examples({MONOWIRE_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--context-shift"},
        {"--no-context-shift"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_IMATRIX, MONOWIRE_EXAMPLE_PERPLEXITY}).set_env("MONOWIRE_ARG_CONTEXT_SHIFT"));
    add_opt(common_arg(
        {"--chunks"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_DIFFUSION, MONOWIRE_EXAMPLE_MTMD}));
    add_opt(common_arg(
        {"--perf"},
        {"--no-perf"},
        string_format("whether to enable internal runtime performance timings (default: %s)", params.no_perf ? "true" : "false"),
        [](common_params & params, bool value) {
    ...
    ).set_examples({MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_SHOW_TIMINGS"));
    add_opt(common_arg(
        {"-f", "--file"}, "FNAME",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_MTMD, MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_RETRIEVAL, MONOWIRE_EXAMPLE_PERPLEXITY, MONOWIRE_EXAMPLE_DEBUG}));
    add_opt(common_arg(
        {"--spm-infill"},
        string_format(
    ...
    add_opt(common_arg(
        {"-s", "--seed"}, "SEED",
        string_format("RNG seed (default: %d, use random seed for %d)", params.sampling.seed, MONOWIRE_DEFAULT_SEED),
        [](common_params & params, const std::string & value) {
    ...
    ).set_sparam().set_env("MONOWIRE_ARG_TOP_K"));
    add_opt(common_arg(
        {"--top-p"}, "N",
    ...
    ).set_sparam().set_env("MONOWIRE_ARG_BACKEND_SAMPLING"));
    add_opt(common_arg(
        {"--pooling"}, "{none,mean,cls,last,rank}",
        "pooling type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.pooling_type = MONOWIRE_POOLING_TYPE_NONE; }
            else if (value == "mean") { params.pooling_type = MONOWIRE_POOLING_TYPE_MEAN; }
            else if (value == "cls")  { params.pooling_type = MONOWIRE_POOLING_TYPE_CLS;  }
            else if (value == "last") { params.pooling_type = MONOWIRE_POOLING_TYPE_LAST; }
            else if (value == "rank") { params.pooling_type = MONOWIRE_POOLING_TYPE_RANK; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({MONOWIRE_EXAMPLE_EMBEDDING, MONOWIRE_EXAMPLE_RETRIEVAL, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_DEBUG}).set_env("MONOWIRE_ARG_POOLING"));
    add_opt(common_arg(
        {"--attention"}, "{causal,non-causal}",
        "attention type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "causal") { params.attention_type = MONOWIRE_ATTENTION_TYPE_CAUSAL; }
            else if (value == "non-causal") { params.attention_type = MONOWIRE_ATTENTION_TYPE_NON_CAUSAL; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_NONE; }
            else if (value == "linear") { params.rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_LINEAR; }
            else if (value == "yarn") { params.rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_YARN; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ...
    ).set_env("MONOWIRE_ARG_GRP_ATTN_N").set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
        {"-gaw", "--grp-attn-w"}, "N",
    ...
    ).set_env("MONOWIRE_ARG_GRP_ATTN_W").set_examples({MONOWIRE_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"-kvo", "--kv-offload"},
        {"-nkvo", "--no-kv-offload"},
    ...
    if (ex == MONOWIRE_EXAMPLE_SERVER) {
        ...
        ).set_env("MONOWIRE_ARG_N_PARALLEL").set_examples({MONOWIRE_EXAMPLE_SERVER}));
    } else {
        add_opt(common_arg(
            {"-np", "--parallel"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_CONT_BATCHING"));
    add_opt(common_arg(
        {"-mm", "--mmproj"}, "FILE",
    ...
    ).set_examples(mmproj_examples).set_env("MONOWIRE_ARG_MMPROJ"));
    add_opt(common_arg(
        {"-mmu", "--mmproj-url"}, "URL",
    ...
    ).set_examples(mmproj_examples).set_env("MONOWIRE_ARG_MMPROJ_URL"));
    add_opt(common_arg(
        {"--mmproj-auto"},
    ...
    ).set_examples(mmproj_examples).set_env("MONOWIRE_ARG_MMPROJ_AUTO"));
    add_opt(common_arg(
        {"--mmproj-offload"},
        {"--no-mmproj-offload"},
    ...
    ).set_examples(mmproj_examples).set_env("MONOWIRE_ARG_MMPROJ_OFFLOAD"));
    add_opt(common_arg(
        {"--image", "--audio"}, "FILE",
    ...
    ).set_examples(mmproj_examples).set_env("MONOWIRE_ARG_IMAGE_MIN_TOKENS"));
    add_opt(common_arg(
        {"--image-max-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("MONOWIRE_ARG_IMAGE_MAX_TOKENS"));
    ...
    add_opt(common_arg(
        {"--streaming-budget-mib"}, "N",
        "preserve up to N MiB of mmap-backed model weights in RAM using balanced memory-aware streaming tensor selection",
        [](common_params & params, int value) {
            if (value < 0) {
    ...
    add_opt(common_arg(
        {"--streaming-window"}, "N",
        "set the memory streaming runtime prefetch window to N layers",
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("value must be non-negative");
            }
    ...
    add_opt(common_arg(
        ...
        string_format("enable memory streaming preserved-range prefetch and runtime layer-window streaming (default: %s)", params.streaming_prefetch ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.streaming_prefetch = value;
        }
    ).set_env("MONOWIRE_ARG_STREAMING_PREFETCH"));
    add_opt(common_arg(
        {"--streaming-evict"},
        {"--no-streaming-evict"},
        string_format("evict dynamic streaming ranges after each layer (default: %s)", params.streaming_evict ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.streaming_evict = value;
        }
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_CPU_MOE_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_N_CPU_MOE_DRAFT"));
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (!monowire_supports_gpu_offload()) {
                ...
                fprintf(stderr, "warning: one possible reason is that Monowire was compiled without GPU support\n");
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (!monowire_supports_gpu_offload()) {
                fprintf(stderr, "warning: Monowire was compiled without support for GPU offload. Setting the split mode has no effect.\n");
            }
        }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (split_arg.size() >= monowire_max_devices()) {
                throw std::invalid_argument(
                    string_format("got %zu input configs, but system only has %zu devices", split_arg.size(), monowire_max_devices())
                );
            }
            ...
            if (!monowire_supports_gpu_offload()) {
                fprintf(stderr, "warning: Monowire was compiled without support for GPU offload. Setting a tensor split has no effect.\n");
            }
        }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            ...
            if (!monowire_supports_gpu_offload()) {
                fprintf(stderr, "warning: Monowire was compiled without support for GPU offload. Setting the main GPU has no effect.\n");
            }
        }
    ...
    ).set_examples({MONOWIRE_EXAMPLE_FIT_PARAMS}).set_env("MONOWIRE_ARG_FIT_ESTIMATE"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_ALIAS"));
    add_opt(common_arg(
        {"--tags"}, "STRING",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_TAGS"));
    add_opt(common_arg(
        {"-m", "--model"}, "FNAME",
        ex == MONOWIRE_EXAMPLE_EXPORT_LORA
            ? "model path from which to load base model"
            : "model path to load",
        [](common_params & params, const std::string & value) {
            params.model.path = value;
        }
    ).set_examples({MONOWIRE_EXAMPLE_COMMON, MONOWIRE_EXAMPLE_EXPORT_LORA}).set_env("MONOWIRE_ARG_MODEL"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_IMATRIX, MONOWIRE_EXAMPLE_CVECTOR_GENERATOR, MONOWIRE_EXAMPLE_EXPORT_LORA, MONOWIRE_EXAMPLE_TTS, MONOWIRE_EXAMPLE_FINETUNE,
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_HOST"));
    add_opt(common_arg(
        {"--port"}, "PORT",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_PORT"));
    add_opt(common_arg(
        {"--reuse-port"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_REUSE_PORT"));
    add_opt(common_arg(
        {"--path"}, "PATH",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_STATIC_PATH"));
    add_opt(common_arg(
        {"--api-prefix"}, "PREFIX",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_API_PREFIX"));
    add_opt(common_arg(
        {"--webui-config"}, "JSON",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_WEBUI_CONFIG"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_WEBUI_CONFIG_FILE"));
    add_opt(common_arg(
        {"--webui-mcp-proxy"},
        {"--no-webui-mcp-proxy"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_WEBUI_MCP_PROXY"));
    add_opt(common_arg(
        {"--tools"}, "TOOL1,TOOL2,...",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_TOOLS"));
    add_opt(common_arg(
        {"--webui"},
        {"--no-webui"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_WEBUI"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_DEBUG}).set_env("MONOWIRE_ARG_EMBEDDINGS"));
    add_opt(common_arg(
        {"--rerank", "--reranking"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_RERANKING"));
    add_opt(common_arg(
        {"--api-key"}, "KEY",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_API_KEY"));
    add_opt(common_arg(
        {"--api-key-file"}, "FNAME",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_SSL_KEY_FILE"));
    add_opt(common_arg(
        {"--ssl-cert-file"}, "FNAME",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_SSL_CERT_FILE"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_CHAT_TEMPLATE_KWARGS"));
    add_opt(common_arg(
        {"-to", "--timeout"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_TIMEOUT"));
    add_opt(common_arg(
        {"--threads-http"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_THREADS_HTTP"));
    add_opt(common_arg(
        {"--cache-prompt"},
        {"--no-cache-prompt"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_CACHE_PROMPT"));
    add_opt(common_arg(
        {"--cache-reuse"}, "N",
        string_format(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_CACHE_REUSE"));
    add_opt(common_arg(
        {"--metrics"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_ENDPOINT_METRICS"));
    add_opt(common_arg(
        {"--props"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_ENDPOINT_PROPS"));
    add_opt(common_arg(
        {"--slots"},
        {"--no-slots"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_ENDPOINT_SLOTS"));
    add_opt(common_arg(
        {"--slot-save-path"}, "PATH",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_MODELS_DIR"));
    add_opt(common_arg(
        {"--models-preset"}, "PATH",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_MODELS_PRESET"));
    add_opt(common_arg(
        {"--models-max"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_MODELS_MAX"));
    add_opt(common_arg(
        {"--models-autoload"},
        {"--no-models-autoload"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_MODELS_AUTOLOAD"));
    add_opt(common_arg(
        {"--jinja"},
        {"--no-jinja"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_MTMD}).set_env("MONOWIRE_ARG_JINJA"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_THINK"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_REASONING"));
    add_opt(common_arg(
        {"--reasoning-budget"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_THINK_BUDGET"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_THINK_BUDGET_MESSAGE"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_MTMD}).set_env("MONOWIRE_ARG_CHAT_TEMPLATE"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_CHAT_TEMPLATE_FILE"));
    add_opt(common_arg(
        {"--skip-chat-parsing"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_COMPLETION, MONOWIRE_EXAMPLE_CLI, MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_SKIP_CHAT_PARSING"));
    add_opt(common_arg(
        {"--prefill-assistant"},
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_PREFILL_ASSISTANT"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_LOOKUP, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_DRAFT_MAX"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_LOOKUP, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_DRAFT_MIN"));
    add_opt(common_arg(
        {"--draft-p-split"}, "P",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE}).set_env("MONOWIRE_ARG_DRAFT_P_SPLIT"));
    add_opt(common_arg(
        {"--draft-p-min"}, "P",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_DRAFT_P_MIN"));
    add_opt(common_arg(
        {"-cd", "--ctx-size-draft"}, "N",
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_CTX_SIZE_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_N_GPU_LAYERS_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SPECULATIVE, MONOWIRE_EXAMPLE_SERVER, MONOWIRE_EXAMPLE_CLI}).set_env("MONOWIRE_ARG_MODEL_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({MONOWIRE_EXAMPLE_SERVER}).set_env("MONOWIRE_ARG_SPEC_TYPE"));
    add_opt(common_arg(
        {"--spec-ngram-size-n"}, "N",

...
static bool common_speculative_are_compatible(
    const monowire_model * model_tgt,
    const monowire_model * model_dft) {
    const monowire_vocab * vocab_tgt = monowire_model_get_vocab(model_tgt);
    const monowire_vocab * vocab_dft = monowire_model_get_vocab(model_dft);

    const bool vocab_type_tgt = monowire_vocab_type(vocab_tgt);
    LOG_DBG("%s: vocab_type tgt: %d\n", __func__, vocab_type_tgt);

    const bool vocab_type_dft = monowire_vocab_type(vocab_dft);
    ...
    if (
        monowire_vocab_get_add_bos(vocab_tgt) != monowire_vocab_get_add_bos(vocab_dft) ||
        monowire_vocab_get_add_eos(vocab_tgt) != monowire_vocab_get_add_eos(vocab_dft) ||
        monowire_vocab_bos(vocab_tgt) != monowire_vocab_bos(vocab_dft) ||
        monowire_vocab_eos(vocab_tgt) != monowire_vocab_eos(vocab_dft)
    ) {
    ...
    {
        const int n_vocab_tgt = monowire_vocab_n_tokens(vocab_tgt);
        const int n_vocab_dft = monowire_vocab_n_tokens(vocab_dft);
        ...
        if (vocab_diff > SPEC_VOCAB_MAX_SIZE_DIFFERENCE) {
            ...
            LOG_DBG("target vocab size %d does not match draft vocab size %d - difference %d, max allowed %d\n",
                    n_vocab_tgt, monowire_vocab_n_tokens(vocab_dft), vocab_diff, SPEC_VOCAB_MAX_SIZE_DIFFERENCE);
            return false;
        }

        for (int i = SPEC_VOCAB_CHECK_START_TOKEN_ID; i < std::min(n_vocab_tgt, n_vocab_dft); ++i) {
            const char * token_text_tgt = monowire_vocab_get_text(vocab_tgt, i);
            const char * token_text_dft = monowire_vocab_get_text(vocab_dft, i);
...
struct common_speculative_state {
    ...
    virtual void begin(const monowire_tokens & prompt) = 0;

    virtual void draft(
            const common_params_speculative & params,
            const monowire_tokens & prompt_tgt,
            monowire_token id_last,
            monowire_tokens & result) = 0;

    virtual void accept(uint16_t n_accepted) = 0;
};

struct common_speculative_checkpoint {
    monowire_pos pos_min  = 0;
    monowire_pos pos_max  = 0;
...
struct common_speculative_state_draft : public common_speculative_state {
    monowire_context * ctx_tgt; // only used for retokenizing from ctx_dft
    monowire_context * ctx_dft;
    ...
    common_speculative_state_draft(
        ...
        , use_ckpt(use_ckpt)
    {
        batch = monowire_batch_init(monowire_n_batch(ctx_dft), 0, 1);
        smpl = nullptr;
        ...
        //     };
        //
        //     result->smpl = common_sampler_init(monowire_get_model(ctx_dft), params);
        // }
        {
            common_params_sampling params;
            params.no_perf = false;
            ...
            smpl = common_sampler_init(monowire_get_model(ctx_dft), params);
        }

        vocab_cmpt = common_speculative_are_compatible(monowire_get_model(ctx_tgt), monowire_get_model(ctx_dft));
    ...
    ~common_speculative_state_draft() override {
        monowire_perf_context_print(ctx_dft);

        monowire_free(ctx_dft);
    ...
    void begin(const monowire_tokens & prompt) override {
    ...
    size_t draft_create_checkpoint(int n_tokens_prompt, int n_tokens_batch) {
        int slot_id = 0;
        const size_t checkpoint_size = monowire_state_seq_get_size_ext(ctx_dft, slot_id, MONOWIRE_STATE_SEQ_FLAGS_PARTIAL_ONLY);

        ckpt.pos_min  = monowire_memory_seq_pos_min(monowire_get_memory(ctx_dft), slot_id);
        ckpt.pos_max  = monowire_memory_seq_pos_max(monowire_get_memory(ctx_dft), slot_id);
        ckpt.n_tokens = n_tokens_prompt - n_tokens_batch;
        ckpt.data.resize(checkpoint_size);

        const size_t n = monowire_state_seq_get_data_ext(ctx_dft, ckpt.data.data(), checkpoint_size, slot_id, MONOWIRE_STATE_SEQ_FLAGS_PARTIAL_ONLY);
    ...
    size_t draft_restore_checkpoint(size_t ckpt_size_part_expected) {
        ...
        const size_t n = monowire_state_seq_set_data_ext(ctx_dft, ckpt.data.data(), ckpt.size(), slot_id, MONOWIRE_STATE_SEQ_FLAGS_PARTIAL_ONLY);
        if (n != ckpt_size_part_expected) {
            GGML_ABORT("%s: failed to restore context checkpoint (pos_min=%d, pos_max=%d, size=%zu, get_data_ext->%zu, set_data_ext->%zu",
                        __func__, ckpt.pos_min, ckpt.pos_max, ckpt.size(), ckpt_size_part_expected, n);
        }
        monowire_memory_seq_rm(monowire_get_memory(ctx_dft), slot_id, ckpt.pos_max + 1, -1);
    ...
    void draft(
            ...
            const monowire_tokens & prompt_tgt,
            monowire_token id_last,
            monowire_tokens & result) override {
        auto * spec = this;
        ...
        auto * mem_dft = monowire_get_memory(ctx_dft);

        int reuse_i = 0; // index of part to be reused in prompt_dft
        int reuse_n = 0; // length of part to be reused in prompt_dft

        const int n_ctx = monowire_n_ctx(ctx_dft) - params.n_max;

        monowire_tokens prompt_cnv;
        if (!spec->vocab_cmpt) {
            std::string text;
            ...
            // convert id_last to draft vocab. monowire_detokenize is called directly to avoid an allocation
            const auto * model_tgt = monowire_get_model(ctx_tgt);
            const auto * vocab_tgt = monowire_model_get_vocab(model_tgt);

            int32_t n_chars = monowire_detokenize(vocab_tgt, &id_last, 1, nullptr, 0, false, false);
            GGML_ASSERT(n_chars < 0 && "failed to detokenize id_last");

            text.resize(-n_chars);
            monowire_detokenize(vocab_tgt, &id_last, 1, text.data(), text.size(), false, false);
            text = replace_to_dft(text);
        ...
        const monowire_tokens & prompt_cur = spec->vocab_cmpt ? prompt_tgt : prompt_cnv;
        ...
        if (reuse_n == 0 || (use_ckpt && reuse_i > 0)) {
            monowire_memory_clear(mem_dft, false);
            prompt_dft.clear();
        } else {
            ...
            if (reuse_i > 0) {
                bool is_removed = monowire_memory_seq_rm (mem_dft, 0, 0, reuse_i);
                if (!is_removed) {
                    LOG_ERR("%s: monowire_memory_seq_rm failed, reuse_i=%d\n", __func__, reuse_i);
                }
                monowire_memory_seq_add(mem_dft, 0, reuse_i, -1, -reuse_i);
            ...
            if (reuse_n < (int) prompt_dft.size() || do_restore) {
                if (use_ckpt) {
                    ...
                    bool is_removed = monowire_memory_seq_rm (mem_dft, 0, reuse_n, -1);
                    if (!is_removed) {
                        LOG_ERR("%s: monowire_memory_seq_rm failed, reuse_n=%d, prompt_dft.size=%zu\n",
        ...
        if (batch.n_tokens > 0) {
            ...
            int ret = monowire_decode(ctx_dft, batch);
            if (ret != 0 && ret != 1) {
                LOG_WRN("%s: monowire_decode returned %d, prompt_cur.size=%zu\n",
                        __func__, ret, prompt_cur.size());
            }
        }

        const monowire_pos n_past = prompt_dft.size();
        ...
        int ret = monowire_decode(ctx_dft, batch);
        if (ret != 0 && ret != 1) {
            LOG_WRN("%s: monowire_decode returned %d, prompt_cur.size=%zu, prompt_dft.size=%zu\n",
        ...
        for (int i = 0; i < params.n_max; ++i) {
            ...
            const monowire_token id = cur_p->data[0].id;
            ...
            ret = monowire_decode(ctx_dft, batch);
            if (ret != 0) {
                LOG_WRN("%s: monowire_decode[%d] returned %d, prompt_cur.size=%zu, prompt_dft.size=%zu\n",
...
struct common_speculative_state_eagle3 : public common_speculative_state {
    ...
    void draft(
            ...
            const monowire_tokens & prompt_tgt,
            monowire_token id_last,
            monowire_tokens & draft_tokens) override {
        // TODO: implement
        GGML_UNUSED(params);
...
struct common_speculative_state_ngram_mod : public common_speculative_state {
    ...
    // enable trace logging if MONOWIRE_TRACE is set
    const bool verbose;

    common_speculative_state_ngram_mod(enum common_speculative_type type, common_ngram_mod & mod)
        : common_speculative_state(type), mod(mod), verbose(std::getenv("MONOWIRE_TRACE") != nullptr) {
        static_assert(sizeof(monowire_token) == sizeof(common_ngram_mod::entry_t));
    }
    ...
    void draft(
        ...
        for (int i = 0; i < params.n_max; ++i) {
            const monowire_token token = mod.get(result.data() + i);
...
struct common_speculative_state_ngram_cache : public common_speculative_state {
    ...
    void draft(
        ...
        if (cache_size < prompt_tgt.size() + 1) {
            ...
            common_ngram_cache_update(ngram_cache_context, MONOWIRE_NGRAM_MIN, MONOWIRE_NGRAM_MAX,
        ...
        common_ngram_cache_draft(inp, result, n_draft, MONOWIRE_NGRAM_MIN, MONOWIRE_NGRAM_MAX,
                ngram_cache_context,
                ngram_cache_dynamic,
                ngram_cache_static);
...
common_speculative * common_speculative_init(
        common_params_speculative & params,
        monowire_context             * ctx_tgt) {
    monowire_context * ctx_dft = nullptr;
    if (params.model_dft) {
        ctx_dft = monowire_init_from_model(params.model_dft, params.cparams_dft);
        if (ctx_dft == nullptr) {
        ...
        if (has_ngram_mod) {
            ...
            if (!params.ngram_mod) {
                ...
                if (params.ngram_size_n < 16) {
                    LOG_WRN("%s: ngram_mod n=%d is too small - poor quality is possible, see: upstream-reference", __func__, params.ngram_size_n);
                }
            }
...
void common_speculative_begin(common_speculative * spec, const monowire_tokens & prompt) {
    if (spec == nullptr) {
        return;
    }
...
monowire_tokens common_speculative_draft(
        common_speculative * spec,
        const common_params_speculative & params,
        const monowire_tokens & prompt_tgt, // specified in target model vocab
        monowire_token id_last) {
    monowire_tokens result;

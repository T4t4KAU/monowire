...
static uint32_t common_ngram_map_hash(const monowire_tokens & tokens, size_t start, size_t len) {
    uint32_t hash = 0;
...
// Print the values of a sublist of `monowire_tokens & inp` to a string in the form [v0, v1, v2, ...].
static std::string common_tokens_to_str(const monowire_tokens & inp, size_t start, size_t length) {
 ...
 */
monowire_tokens common_ngram_simple_draft(
        const common_ngram_simple_config & config,
        const monowire_tokens & tokens, monowire_token sampled) {
...
void common_ngram_map_begin(
    common_ngram_map & map, const monowire_tokens & tokens) {
...
void common_ngram_map_draft(common_ngram_map & map,
        const monowire_tokens & inp, monowire_token sampled,
        monowire_tokens & draft) {
    ...
    std::vector<monowire_token> key_tokens;

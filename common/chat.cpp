        ...
        for (const auto & message : messages) {
            ...
            if (!has_content && !has_tool_calls) {
                throw std::invalid_argument(
                    "Expected 'content' or 'tool_calls' (ref: upstream-reference & "
                    "upstream-reference)");
            }
...
bool common_chat_verify_template(const std::string & tmpl, bool use_jinja) {
    ...
    monowire_chat_message chat[] = {
        { "user", "test" }
    };
    const int res = monowire_chat_apply_template(tmpl.c_str(), chat, 1, true, nullptr, 0);
    return res >= 0;
}
...
common_chat_templates_ptr common_chat_templates_init(const struct monowire_model * model,
    ...
    if (chat_template_override.empty()) {
        ...
        const auto * str = monowire_model_chat_template(model, /* name */ nullptr);
        if (str) {
            default_template_src  = str;
            has_explicit_template = true;
        }
        str = monowire_model_chat_template(model, /* name */ "tool_use");
        if (str) {
    ...
    if (model) {
        ...
        const auto   get_token = [&](monowire_token token, const char * name, const char * jinja_variable_name) {
        ...
        token_bos = get_token(monowire_vocab_bos(vocab), "BOS", "bos_token");
        token_eos = get_token(monowire_vocab_eos(vocab), "EOS", "eos_token");
        add_bos   = monowire_vocab_get_add_bos(vocab);
        add_eos   = monowire_vocab_get_add_eos(vocab);
    }
...
// Legacy template route (adhoc C++ implementation of known templates), forward to monowire_chat_apply_template.
static common_chat_params common_chat_templates_apply_legacy(const struct common_chat_templates *        tmpls,
                                                             const struct common_chat_templates_inputs & inputs) {
    size_t                          alloc_size = 0;
    std::vector<monowire_chat_message> chat;
    ...
    int32_t      res = monowire_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt,
                                                 buf.data(), buf.size());

    // error: chat template is not supported
    if (res < 0) {
        // if the custom "tmpl" is not supported, we throw an error
        // this is a bit redundant (for good), since we're not sure if user validated the custom template with monowire_chat_verify_template()
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = monowire_chat_apply_template(src.c_str(), chat.data(), chat.size(), inputs.add_generation_prompt, buf.data(),
                                        buf.size());
    }

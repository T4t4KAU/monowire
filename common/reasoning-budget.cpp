...
struct token_matcher {
    ...
    bool advance(monowire_token token) {
        if (tokens.empty()) {
            return false;
        }
...
struct common_reasoning_budget_ctx {
    ...
    std::vector<monowire_token> forced_tokens;
...
static const char * common_reasoning_budget_name(const struct monowire_sampler * /*smpl*/) {
    return "reasoning-budget";
}

static void common_reasoning_budget_accept(struct monowire_sampler * smpl, monowire_token token) {
...
static void common_reasoning_budget_apply(struct monowire_sampler * smpl, monowire_token_data_array * cur_p) {
    auto * ctx = (common_reasoning_budget_ctx *) smpl->ctx;
    ...
    const monowire_token forced = ctx->forced_tokens[ctx->force_pos];
...
static void common_reasoning_budget_reset(struct monowire_sampler * smpl) {
    auto * ctx = (common_reasoning_budget_ctx *) smpl->ctx;
...
static struct monowire_sampler * common_reasoning_budget_init_state(
        const struct monowire_vocab * vocab, const std::vector<monowire_token> & start_tokens,
        const std::vector<monowire_token> & end_tokens, const std::vector<monowire_token> & forced_tokens,
        int32_t budget, common_reasoning_budget_state initial_state);

static struct monowire_sampler * common_reasoning_budget_clone(const struct monowire_sampler * smpl) {
...
static void common_reasoning_budget_free(struct monowire_sampler * smpl) {
    delete (common_reasoning_budget_ctx *) smpl->ctx;
}

static struct monowire_sampler_i common_reasoning_budget_i = {
...
static struct monowire_sampler * common_reasoning_budget_init_state(
        ...
        const std::vector<monowire_token>       & start_tokens,
        const std::vector<monowire_token>       & end_tokens,
        const std::vector<monowire_token>       & forced_tokens,
        int32_t                                budget,
...
struct monowire_sampler * common_reasoning_budget_init(
        const struct monowire_vocab       * vocab,
        const std::vector<monowire_token> & start_tokens,
        const std::vector<monowire_token> & end_tokens,
        const std::vector<monowire_token> & forced_tokens,
        int32_t                          budget,
        const std::vector<monowire_token> & prefill_tokens) {
...
common_reasoning_budget_state common_reasoning_budget_get_state(const struct monowire_sampler * smpl) {
    if (!smpl) {

...
#define MONOWIRE_NGRAM_MIN    1
#define MONOWIRE_NGRAM_MAX    4
#define MONOWIRE_NGRAM_STATIC 2

// Data structures to map n-grams to empirical token probabilities:

struct common_ngram {
    monowire_token tokens[MONOWIRE_NGRAM_MAX];

    common_ngram() {
        for (int i = 0; i < MONOWIRE_NGRAM_MAX; ++i) {
            tokens[i] = MONOWIRE_TOKEN_NULL;
        }
    }

    common_ngram(const monowire_token * input, const int ngram_size) {
        for (int i = 0; i < MONOWIRE_NGRAM_MAX; ++i) {
            tokens[i] = i < ngram_size ? input[i] : MONOWIRE_TOKEN_NULL;
        }
    }
...
struct common_token_hash_function {
    size_t operator()(const monowire_token token) const {
...
struct common_ngram_hash_function {
    size_t operator()(const common_ngram & ngram) const {
        ...
        for (int i = 1; i < MONOWIRE_NGRAM_MAX; ++i) {
...
typedef std::unordered_map<monowire_token, int32_t> common_ngram_cache_part;
...
void common_ngram_cache_update(
    common_ngram_cache & ngram_cache, int ngram_min, int ngram_max, std::vector<monowire_token> & inp_data, int nnew, bool print_progress);

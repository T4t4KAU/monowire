...
struct common_adapter_lora_info {
    ...
    struct monowire_adapter_lora *ptr;
};

using monowire_tokens = std::vector<monowire_token>;
...
enum monowire_example {
    MONOWIRE_EXAMPLE_BATCHED,
    MONOWIRE_EXAMPLE_DEBUG,
    MONOWIRE_EXAMPLE_COMMON,
    MONOWIRE_EXAMPLE_SPECULATIVE,
    MONOWIRE_EXAMPLE_COMPLETION,
    MONOWIRE_EXAMPLE_CLI,
    MONOWIRE_EXAMPLE_EMBEDDING,
    MONOWIRE_EXAMPLE_PERPLEXITY,
    MONOWIRE_EXAMPLE_RETRIEVAL,
    MONOWIRE_EXAMPLE_PASSKEY,
    MONOWIRE_EXAMPLE_IMATRIX,
    MONOWIRE_EXAMPLE_BENCH,
    MONOWIRE_EXAMPLE_SERVER,
    MONOWIRE_EXAMPLE_CVECTOR_GENERATOR,
    MONOWIRE_EXAMPLE_EXPORT_LORA,
    MONOWIRE_EXAMPLE_MTMD,
    MONOWIRE_EXAMPLE_LOOKUP,
    MONOWIRE_EXAMPLE_PARALLEL,
    MONOWIRE_EXAMPLE_TTS,
    MONOWIRE_EXAMPLE_DIFFUSION,
    MONOWIRE_EXAMPLE_FINETUNE,
    MONOWIRE_EXAMPLE_FIT_PARAMS,
    MONOWIRE_EXAMPLE_RESULTS,
    MONOWIRE_EXAMPLE_EXPORT_GRAPH_OPS,

    MONOWIRE_EXAMPLE_COUNT,
};
...
struct common_grammar_trigger {
    ...
    std::string value;
    monowire_token token = MONOWIRE_TOKEN_NULL;
};
...
inline const std::string &common_grammar_value(const common_grammar &g) { return g.grammar; }
...
struct common_params_sampling {
    uint32_t seed = MONOWIRE_DEFAULT_SEED; // the seed used to initialize monowire_sampler
    ...
    std::set<monowire_token> preserved_tokens;

    std::vector<monowire_logit_bias> logit_bias;     // logit biases to apply
    std::vector<monowire_logit_bias> logit_bias_eog; // pre-calculated logit biases for EOG tokens
    ...
    std::vector<monowire_token> reasoning_budget_start;  // start tag token sequence
    std::vector<monowire_token> reasoning_budget_end;    // end tag token sequence
    std::vector<monowire_token> reasoning_budget_forced; // forced sequence (message + end tag)
...
struct common_params_speculative {
    ...
    monowire_model *model_dft = nullptr; // a monowire_model that can be shared by multiple speculative contexts

    monowire_context_params cparams_dft; // these are the parameters for the draft monowire_context
...
enum common_reasoning_format {
    ...
    COMMON_REASONING_FORMAT_DEEPSEEK_LEGACY, // Extract thinking tag contents and return as `message.reasoning_content`,
...
struct common_params {
    ...
    std::vector<size_t> fit_params_target = std::vector<size_t>(monowire_max_devices(), 1024 * 1024 * 1024);

    enum monowire_split_mode split_mode = MONOWIRE_SPLIT_MODE_LAYER; // how to split the model across GPUs
    ...
    enum monowire_rope_scaling_type rope_scaling_type = MONOWIRE_ROPE_SCALING_TYPE_UNSPECIFIED;
    enum monowire_pooling_type pooling_type = MONOWIRE_POOLING_TYPE_UNSPECIFIED;       // pooling type for embeddings
    enum monowire_attention_type attention_type = MONOWIRE_ATTENTION_TYPE_UNSPECIFIED; // attention type for embeddings
    enum monowire_flash_attn_type flash_attn_type = MONOWIRE_FLASH_ATTN_TYPE_AUTO;     // whether to use Flash Attention
    ...
    // Monowire debug-specific options
    ...
    bool check = false; // check rather than generate results for Monowire result tools
    ...
    bool swa_full = false;        // use full-size SWA cache (upstream-reference)
    ...
    uint64_t streaming_budget_bytes = 0;  // preserve selected mmap-backed tensors in RAM
    uint32_t streaming_window = 3;        // number of layers in the runtime memory streaming prefetch window
    uint32_t streaming_io_threads = 1;    // background I/O hint workers for runtime memory streaming
    bool streaming_prefetch = true;       // prefetch preserved mmap-backed tensors during model load
    bool streaming_evict = true;          // evict dynamic streaming ranges after each layer
    bool streaming_read_prefetch = false; // actively pread runtime streaming ranges into page cache
    bool streaming_direct_buffer = false; // bind streamed CPU weights from owned runtime buffers
    ...
    monowire_progress_callback load_progress_callback = NULL;
...
#define MONOWIRE_COMMON_ATTRIBUTE_FORMAT(...) __attribute__((format(gnu_printf, __VA_ARGS__)))
#else
#define MONOWIRE_COMMON_ATTRIBUTE_FORMAT(...) __attribute__((format(printf, __VA_ARGS__)))
#endif
#else
#define MONOWIRE_COMMON_ATTRIBUTE_FORMAT(...)
#endif

MONOWIRE_COMMON_ATTRIBUTE_FORMAT(1, 2)
...
struct common_init_result {
    ...
    monowire_model *model();
    monowire_context *context();

    common_sampler *sampler(monowire_seq_id seq_id);
    void reset_samplers();
...
// check if the monowire_context can remove sequences
...
bool common_prompt_batch_decode(struct monowire_context *ctx, const std::vector<monowire_token> &embd, int &n_past,
...
std::string common_token_to_piece(const struct monowire_context *ctx, monowire_token token, bool special = true);

std::string common_token_to_piece(const struct monowire_vocab *vocab, monowire_token token, bool special = true);
...
inline std::string llm_ffn_exps_block_regex(int idx) { return string_format("blk\\.%d%s", idx, LLM_FFN_EXPS_REGEX); }

inline monowire_model_tensor_buft_override llm_ffn_exps_cpu_override() {

...
// this enum is only used in monowire_params_fit_impl but needs to be defined outside of it to fix a Windows compilation issue
...
static std::vector<monowire_device_memory_data> common_get_device_memory_data(
        const char * path_model,
        const monowire_model_params * mparams,
        const monowire_context_params * cparams,
    ...
    user_data_t ud;
    monowire_log_get(&ud.original_logger.callback, &ud.original_logger.user_data);
    ud.min_level = log_level;

    monowire_log_set([](ggml_log_level level, const char * text, void * user_data) {
        const user_data_t * ud = (const user_data_t *) user_data;
        const ggml_log_level level_eff = level >= ud->min_level ? level : GGML_LOG_LEVEL_DEBUG;
        ud->original_logger.callback(level_eff, text, ud->original_logger.user_data);
    }, &ud);

    monowire_model_params mparams_copy = *mparams;
    mparams_copy.no_alloc  = true;
    mparams_copy.use_mmap  = false;
    mparams_copy.use_mlock = false;

    monowire_model * model = monowire_model_load_from_file(path_model, mparams_copy);
    if (model == nullptr) {
        monowire_log_set(ud.original_logger.callback, ud.original_logger.user_data);
        throw std::runtime_error("failed to load model");
    }

    monowire_context * ctx = monowire_init_from_model(model, *cparams);
    if (ctx == nullptr) {
        monowire_model_free(model);
        monowire_log_set(ud.original_logger.callback, ud.original_logger.user_data);
        throw std::runtime_error("failed to create monowire_context from model");
    }

    const size_t nd = monowire_model_n_devices(model);
    std::vector<monowire_device_memory_data> ret(nd + 1);

    monowire_memory_breakdown memory_breakdown = monowire_get_memory_breakdown(ctx);
    ...
    for (const auto & [buft, mb] : memory_breakdown) {
        ...
        for (size_t i = 0; i < nd; i++) {
            if (dev == monowire_model_get_device(model, i)) {
    ...
    for (size_t i = 0; i < nd; i++) {
        size_t free;
        size_t total;
        ggml_backend_dev_memory(monowire_model_get_device(model, i), &free, &total);

        // devices can return 0 bytes for free and total memory if they do not
        // have any to report. in this case, we will use the host memory as a fallback
        // fixes: upstream-reference
        if (free == 0 && total == 0) {
            free  = ret.back().free;
    ...
    devs.clear();
    for (int i = 0; i < monowire_model_n_devices(model); i++) {
        devs.push_back(monowire_model_get_device(model, i));
    }

    hp_ngl         = monowire_model_n_layer(model);
    hp_n_ctx_train = monowire_model_n_ctx_train(model);
    hp_n_expert    = monowire_model_n_expert(model);
...
static void common_params_fit_impl(
        const char * path_model, struct monowire_model_params * mparams, struct monowire_context_params * cparams,
        float * tensor_split, struct monowire_model_tensor_buft_override * tensor_buft_overrides,
        size_t * margins_s, uint32_t n_ctx_min, enum ggml_log_level log_level) {
    if (mparams->split_mode == MONOWIRE_SPLIT_MODE_TENSOR) {
        throw common_params_fit_exception("monowire_params_fit is not implemented for SPLIT_MODE_TENSOR, abort");
    }
    constexpr int64_t MiB = 1024*1024;
    typedef std::vector<monowire_device_memory_data> dmds_t;
    const monowire_model_params default_mparams = monowire_model_default_params();
    ...
    if (nd == 0) {
        ...
        for (size_t id = 0; id < nd; id++) {
            const monowire_device_memory_data & dmd = dmds_full[id];
    ...
    if (nd > 1) {
        ...
        if (mparams->split_mode == MONOWIRE_SPLIT_MODE_ROW) {
            throw common_params_fit_exception("changing weight allocation for MONOWIRE_SPLIT_MODE_ROW not implemented, abort");
        }
    }
        ...
        for (size_t id = 0; id < nd; id++) {
            ...
            for (uint32_t il = il0; il < il0 + ngl_per_device[id].n_part; il++) {
                if (itbo + 1 >= ntbo) {
                    ...
                    throw common_params_fit_exception("monowire_max_tensor_buft_overrides() == "
...
enum common_params_fit_status common_fit_params(
        const char * path_model,
        monowire_model_params * mparams,
        monowire_context_params * cparams,
        float * tensor_split,
        monowire_model_tensor_buft_override * tensor_buft_overrides,
        size_t * margins,
        uint32_t n_ctx_min,
        ggml_log_level log_level) {
    const int64_t t0_us = monowire_time_us();
    ...
    const int64_t t1_us = monowire_time_us();
    LOG_INF("%s: fitting params to free memory took %.2f seconds\n", __func__, (t1_us - t0_us) * 1e-6);
    return status;
}

void common_memory_breakdown_print(const struct monowire_context * ctx) {
    //const auto & devices = ctx->get_model().devices;
    const auto * model = monowire_get_model(ctx);

    std::vector<ggml_backend_dev_t> devices;
    for (int i = 0; i < monowire_model_n_devices(model); i++) {
        devices.push_back(monowire_model_get_device(model, i));
    }
    ...
    std::vector<monowire_memory_breakdown_data> mb_dev(devices.size());
    monowire_memory_breakdown_data              mb_host;

    for (const auto & buft_mb : memory_breakdown) {
        ggml_backend_buffer_type_t          buft = buft_mb.first;
        const monowire_memory_breakdown_data & mb   = buft_mb.second;
        if (ggml_backend_buft_is_host(buft)) {
    ...
    for (size_t i = 0; i < devices.size(); i++) {
        ...
        monowire_memory_breakdown_data mb = mb_dev[i];

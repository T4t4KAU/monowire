...
std::string common_params_get_system_info(const common_params &params) {
    ...
    os << " / " << logicalProcessorCount << " | " << monowire_print_system_info();
#else
    os << " / " << std::thread::hardware_concurrency() << " | " << monowire_print_system_info();
#endif
...
bool string_parse_kv_override(const char *data, std::vector<monowire_model_kv_override> &overrides) {
    ...
    if (strncmp(sep, "int:", 4) == 0) {
        ...
        sep += 6;
        kvo.tag = MONOWIRE_KV_OVERRIDE_TYPE_FLOAT;
        kvo.val_f64 = std::atof(sep);
    } else if (strncmp(sep, "bool:", 5) == 0) {
        sep += 5;
        kvo.tag = MONOWIRE_KV_OVERRIDE_TYPE_BOOL;
...
common_init_result::common_init_result(common_params &params) : pimpl(new impl{}) {
    ...
    for (auto &la : params.lora_adapters) {
        ...
        char buf[1024];
        la.ptr = lora.get();
        monowire_adapter_meta_val_str(la.ptr, "adapter.lora.task_name", buf, sizeof(buf));
        la.task_name = buf;
        monowire_adapter_meta_val_str(la.ptr, "adapter.lora.prompt_prefix", buf, sizeof(buf));
    ...
    if (params.sampling.ignore_eos && monowire_vocab_eos(vocab) == MONOWIRE_TOKEN_NULL) {
        LOG_WRN("%s: warning: vocab does not have an EOS token, ignoring --ignore-eos\n", __func__);
        params.sampling.ignore_eos = false;
    }

    // initialize once
    for (monowire_token i = 0; i < monowire_vocab_n_tokens(vocab); i++) {
        if (monowire_vocab_is_eog(vocab, i)) {
...
common_init_result_ptr common_init_from_params(common_params &params) {
    ...
    if (monowire_pooling_type(lctx) == MONOWIRE_POOLING_TYPE_RANK) {
        bool ok = true;

        if (monowire_vocab_bos(vocab) == MONOWIRE_TOKEN_NULL) {
            LOG_WRN("%s: warning: vocab does not have a  BOS token, reranking will not work\n", __func__);
            ok = false;
        }

        bool has_eos = monowire_vocab_eos(vocab) != MONOWIRE_TOKEN_NULL;
        bool has_sep = monowire_vocab_sep(vocab) != MONOWIRE_TOKEN_NULL;
    ...
    if (params.warmup) {
        ...
        monowire_set_warmup(lctx, true);

        std::vector<monowire_token> tmp;
        monowire_token bos = monowire_vocab_bos(vocab);
        monowire_token eos = monowire_vocab_eos(vocab);

        // some models (e.g. T5) don't have a BOS token
        if (bos != MONOWIRE_TOKEN_NULL) {
            tmp.push_back(bos);
        }
        if (eos != MONOWIRE_TOKEN_NULL) {
            tmp.push_back(eos);
        }
        if (tmp.empty()) {
            tmp.push_back(0);
        ...
        if (monowire_model_has_encoder(model)) {
            monowire_encode(lctx, monowire_batch_get_one(tmp.data(), tmp.size()));
        ...
        monowire_perf_context_reset(lctx);
        monowire_set_warmup(lctx, false);
...
common_context_seq_rm_type common_context_can_seq_rm(monowire_context *ctx) {
    ...
    int ret = monowire_decode(ctx, monowire_batch_get_one(tmp.data(), tmp.size()));
    if (ret != 0) {
        LOG_ERR("%s: monowire_decode() failed: %d\n", __func__, ret);
        res = COMMON_CONTEXT_SEQ_RM_TYPE_NO;
        goto done;
    }

    // try to remove the last tokens
    if (!monowire_memory_seq_rm(mem, 0, 1, -1)) {
...
bool common_prompt_batch_decode(struct monowire_context *ctx, const std::vector<monowire_token> &tokens, int &n_past,
    ...
    if (save_state && n_eval > 1) {
        ...
        monowire_state_save_file(ctx, state_path.data(), tokens.data(), n_tokens_before_last);
        LOG_INF("saved session before last token to %s, n_tokens = %d\n", state_path.data(), n_tokens_before_last);

        monowire_token last_token = tokens.back();
        monowire_batch batch = monowire_batch_get_one(&last_token, 1);
        int32_t pos = n_past;
        batch.pos = &pos;

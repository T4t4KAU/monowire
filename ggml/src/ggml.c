...
void ggml_print_backtrace(void) {
    ...
    if (!GGML_BACKTRACE_LLDB) {
        ...
        fprintf(stderr, "See: upstream-reference");
...
struct ggml_tensor * ggml_set_rows(
    ...
    result->src[0] = b;
    result->src[1] = c;
    result->src[2] = a; // note: order is weird due to legacy reasons (upstream-reference)
        ...
        case GGML_OP_MUL_MAT: {
            ...
            if (src1_needs_grads) {
                ggml_add_or_set(ctx, cgraph, isrc1,
                        ...
                        // when src0 is bigger than tensor->grad (this is common in decoder runtimes),

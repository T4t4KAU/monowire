...
OutputVector translate_view(const NodeContext & context) {
    ...
    if (context.get_op_case() == 3) {
        ...
        // if the input ov shape size is different from the input runtime shape size, it means the input is already reshaped and we need to reshape it back to the original shape before slicing

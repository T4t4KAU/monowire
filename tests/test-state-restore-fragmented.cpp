...
// This tests the fix for: upstream-reference
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, MONOWIRE_EXAMPLE_COMMON)) {
        return 1;
    }

    // init
    common_init_result_ptr monowire_init = common_init_from_params(params);

    monowire_model * model = monowire_init->model();
    monowire_context * ctx = monowire_init->context();
    ...
    // tokenize prompt
    std::vector<monowire_token> tokens(70, 1);

    // interleave the 3 sequences:
    // 01201230123...
    monowire_batch batch = monowire_batch_init(params.n_parallel*tokens.size(), 0, 1);
    ...
    // Save state of seq 1
    std::vector<uint8_t> seq_state(monowire_state_seq_get_size(ctx, 1));
    const size_t ncopy = monowire_state_seq_get_data(ctx, seq_state.data(), seq_state.size(), 1);
    ...
    monowire_memory_seq_rm(mem, 1, -1, -1);
    ...
    const size_t nset = monowire_state_seq_set_data(ctx, seq_state.data(), seq_state.size(), 1);
    ...
    monowire_sampler_chain_add(smpl, monowire_sampler_init_dist(params.sampling.seed));

    auto next_token = monowire_sampler_sample(smpl, ctx, -1);

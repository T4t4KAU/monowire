...
int main(void) {
    ...
    for (int ex = 0; ex < MONOWIRE_EXAMPLE_COUNT; ex++) {
        try {
            auto ctx_arg = common_params_parser_init(params, (enum monowire_example)ex);
    ...
    // missing value
    argv = {"binary_name", "-m"};
    assert(false == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_COMMON));
    ...
    // non-existence arg in specific example (--draft cannot be used outside monowire-speculative)
    argv = {"binary_name", "--draft", "123"};
    assert(false == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_EMBEDDING));
    ...
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_COMMON));
    ...
    // --draft cannot be used outside monowire-speculative
    argv = {"binary_name", "--draft", "123"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_SPECULATIVE));
...
// skip this part on windows, because setenv is not supported
    ...
    setenv("MONOWIRE_ARG_THREADS", "blah", true);
    argv = {"binary_name"};
    assert(false == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_COMMON));

    setenv("MONOWIRE_ARG_MODEL", "blah.gguf", true);
    setenv("MONOWIRE_ARG_THREADS", "1010", true);
    argv = {"binary_name"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_COMMON));
    ...
    setenv("MONOWIRE_ARG_MMAP", "0", true);
    setenv("MONOWIRE_ARG_NO_PERF", "1", true); // negated env format
    argv = {"binary_name"};
    assert(true == common_params_parse(argv.size(), list_str_to_char(argv).data(), params, MONOWIRE_EXAMPLE_COMMON));

...
static monowire_model_ptr load_model(const test_args & args) {
    ...
    monowire_model_ptr res;

    res.reset(monowire_model_load_from_file(args.model.c_str(), mparams));
...
struct test_context {
    monowire_context_ptr ctx;

    int n_vocab = 0;

    const monowire_vocab * vocab = nullptr;

    std::unordered_map<monowire_seq_id, int32_t> seq_positions;
    std::unordered_map<monowire_seq_id, int32_t> last_batch_info;

    test_context(const test_params & params, std::vector<monowire_sampler_seq_config> & configs, int32_t n_seq_max = -1) {
        ...
        ctx.reset(monowire_init_from_model(model, cparams));
        if (!ctx) {
            throw std::runtime_error("failed to create context");
        }

        monowire_set_warmup(ctx.get(), false);

        vocab = monowire_model_get_vocab(model);
        n_vocab = monowire_vocab_n_tokens(vocab);
    }

    bool decode(const std::map<monowire_seq_id, std::string> & prompts) {
        GGML_ASSERT(ctx);

        last_batch_info.clear();
        monowire_batch batch = monowire_batch_init(512, 0, prompts.size());

        for (const auto & [seq_id, prompt] : prompts) {
            std::vector<monowire_token> tokens;
            tokens.push_back(monowire_vocab_bos(vocab));

            std::vector<monowire_token> prompt_tokens(32);
            int n_tokens = monowire_tokenize(vocab, prompt.c_str(), prompt.length(),
        ...
        if (monowire_decode(ctx.get(), batch) != 0) {
            fprintf(stderr, "Warning: monowire_decode failed\n");
            monowire_batch_free(batch);
            return false;
        }
        ...
        for (int i = 0; i < batch.n_tokens; i++) {
            if (batch.logits[i]) {
                monowire_seq_id seq_id = batch.seq_id[i][0];
    ...
    int32_t idx_for_seq(monowire_seq_id seq_id) {
    ...
    void update_batch_info(const monowire_batch & batch) {
        last_batch_info.clear();
        for (int i = 0; i < batch.n_tokens; i++) {
            if (batch.logits[i]) {
                monowire_seq_id cur_seq = batch.seq_id[i][0];
                last_batch_info[cur_seq] = i;
            }
        }
    }

    bool decode_token(monowire_token token, monowire_seq_id seq_id = 0) {
        GGML_ASSERT(ctx);

        monowire_batch batch = monowire_batch_init(1, 0, 1);
        int32_t pos = seq_positions[seq_id];
        common_batch_add(batch, token, pos, { seq_id }, true);

        if (monowire_decode(ctx.get(), batch) != 0) {
            fprintf(stderr, "Warning: monowire_decode failed for token %d in seq %d\n", token, seq_id);
            monowire_batch_free(batch);
            return false;
        }
    ...
    bool decode_tokens(const std::map<monowire_seq_id, monowire_token> & seq_tokens) {
        GGML_ASSERT(ctx);

        monowire_batch batch = monowire_batch_init(seq_tokens.size(), 0, seq_tokens.size());
        ...
        if (monowire_decode(ctx.get(), batch) != 0) {
            fprintf(stderr, "Warning: monowire_decode failed for batch tokens\n");
            monowire_batch_free(batch);
            return false;
        }
    ...
    std::string token_to_piece(monowire_token token, bool special) const {
        std::string piece;
...
static void test_backend_greedy_sampling(const test_params & params) {
    ...
    struct monowire_sampler_chain_params backend_sampler_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_sampler_params));

    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_greedy());
    std::vector<monowire_sampler_seq_config> backend_sampler_configs = {{ seq_id, backend_sampler_chain.get() }};
    ...
    monowire_token token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), batch_idx);
    printf("greedy sampled id:%d, string:'%s'\n", token, test_ctx.token_to_piece(token, false).c_str());
    GGML_ASSERT(token >= 0 && token < test_ctx.n_vocab);

    token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), -1);
    printf("greedy sampled id:%d, string:'%s'\n", token, test_ctx.token_to_piece(token, false).c_str());
    GGML_ASSERT(token >= 0 && token < test_ctx.n_vocab);

    for (int i = 0; i < 10; i++) {
        int32_t loop_idx = test_ctx.idx_for_seq(seq_id);
        monowire_token token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), loop_idx);
...
static void test_backend_top_k_sampling(const test_params & params) {
    const int seq_id = 0;
    const int32_t k = 8;
    struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_top_k(k));
    std::vector<monowire_sampler_seq_config> backend_sampler_configs = {{ seq_id, backend_sampler_chain.get() }};
    ...
    float * logits = monowire_get_sampled_logits_ith(test_ctx.ctx.get(), batch_idx);
    uint32_t n_logits = monowire_get_sampled_logits_count_ith(test_ctx.ctx.get(), batch_idx);
    for (size_t i = 0; i < n_logits; ++i) {
        printf("top_k logit[%zu] = %.6f\n", i, logits[i]);
    }

    monowire_token * candidates = monowire_get_sampled_candidates_ith(test_ctx.ctx.get(), batch_idx);
    uint32_t n_candidates = monowire_get_sampled_candidates_count_ith(test_ctx.ctx.get(), batch_idx);
    ...
    struct monowire_sampler_chain_params chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr chain(monowire_sampler_chain_init(chain_params));
    GGML_ASSERT(chain->iface->backend_apply != nullptr);

    monowire_sampler_chain_add(chain.get(), monowire_sampler_init_dist(18));
    monowire_token token = monowire_sampler_sample(chain.get(), test_ctx.ctx.get(), batch_idx);
    GGML_ASSERT(token >= 0 && token < test_ctx.n_vocab);
...
static void test_backend_temp_sampling(const test_params & params) {
    {
        ...
        struct monowire_sampler_chain_params backend_chain_params_0 = monowire_sampler_chain_default_params();
        monowire_sampler_ptr backend_sampler_chain_0(monowire_sampler_chain_init(backend_chain_params_0));
        monowire_sampler_chain_add(backend_sampler_chain_0.get(), monowire_sampler_init_temp(temp_0));

        const float temp_1 = 0.1f;
        struct monowire_sampler_chain_params backend_chain_params_1 = monowire_sampler_chain_default_params();
        monowire_sampler_ptr backend_sampler_chain_1(monowire_sampler_chain_init(backend_chain_params_1));
        monowire_sampler_chain_add(backend_sampler_chain_1.get(), monowire_sampler_init_temp(temp_1));

        std::vector<monowire_sampler_seq_config> backend_sampler_configs = {
        ...
        // Verify sequence 0
        {
            ...
            int n_logits = monowire_get_sampled_logits_count_ith(test_ctx.ctx.get(), batch_idx);
    ...
    auto test_argmax_temp = [&](float temp) {
        ...
        struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
        monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
        monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_temp(temp));
...
static void test_backend_temp_ext_sampling(const test_params & params) {
    {
        ...
        struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
        monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
        monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_temp_ext(temp, delta, exponent));
...
static void test_backend_min_p_sampling(const test_params & params) {
    ...
    struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_min_p(p, 0));
    std::vector<monowire_sampler_seq_config> backend_sampler_configs = {{ seq_id, backend_sampler_chain.get() }};
    ...
    struct monowire_sampler_chain_params chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr chain(monowire_sampler_chain_init(chain_params));
    monowire_sampler_chain_add(chain.get(), monowire_sampler_init_dist(88));
    ...
    for (int i = 0; i < 10; i++) {
        int32_t loop_idx = test_ctx.idx_for_seq(seq_id);
        monowire_token token = monowire_sampler_sample(chain.get(), test_ctx.ctx.get(), loop_idx);
...
static void test_backend_top_p_sampling(const test_params & params) {
    ...
    struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_top_p(p, 0));
    std::vector<monowire_sampler_seq_config> backend_sampler_configs = {{ seq_id, backend_sampler_chain.get() }};
...
static void test_backend_multi_sequence_sampling(const test_params & params) {
    struct monowire_sampler_chain_params chain_params_0 = monowire_sampler_chain_default_params();
    monowire_sampler_ptr sampler_chain_0(monowire_sampler_chain_init(chain_params_0));
    monowire_sampler_chain_add(sampler_chain_0.get(), monowire_sampler_init_greedy());

    struct monowire_sampler_chain_params chain_params_1 = monowire_sampler_chain_default_params();
    monowire_sampler_ptr sampler_chain_1(monowire_sampler_chain_init(chain_params_1));
    monowire_sampler_chain_add(sampler_chain_1.get(), monowire_sampler_init_temp(0.8f));
    monowire_sampler_chain_add(sampler_chain_1.get(), monowire_sampler_init_greedy());
    ...
    std::map<monowire_seq_id, std::string> prompts = {
        {0, "Hello"},
        {1, "Some"}
    };
    ...
    for (int step = 0; step < 4; step++) {
        std::map<monowire_seq_id, monowire_token> tokens;

        for (monowire_seq_id seq_id : {0, 1}) {
            int32_t idx = test_ctx.idx_for_seq(seq_id);
            monowire_token token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), idx);
            const std::string token_str = test_ctx.token_to_piece(token, false);
...
static void test_backend_dist_sampling(const test_params & params) {
    ...
    struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_dist(seed));
    std::vector<monowire_sampler_seq_config> backend_sampler_configs = {{ seq_id, backend_sampler_chain.get() }};
    ...
    GGML_ASSERT(token >= 0 && token < test_ctx.n_vocab);
    //GGML_ASSERT(monowire_get_sampled_logits_ith(test_ctx.ctx.get(), batch_idx) == nullptr);
...
static void test_backend_dist_sampling_and_cpu(const test_params & params) {
    ...
    monowire_token backend_token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), batch_idx);
    monowire_token cpu_token = monowire_sampler_sample(chain.get(), test_ctx.ctx.get(), batch_idx);
...
static void test_backend_logit_bias_sampling(const test_params & params) {
    ...
    const auto * vocab = monowire_model_get_vocab(model);

    const int seq_id = 0;

    std::vector<monowire_logit_bias> logit_bias;

    // Get the token for the piece "World".
    const std::string piece = "World";
    std::vector<monowire_token> tokens(16);
    monowire_tokenize(vocab, piece.c_str(), piece.size(), tokens.data(), tokens.size(), false, false);

    monowire_token bias_token = tokens[0];
    ...
    struct monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_logit_bias(
                monowire_vocab_n_tokens(vocab),
                logit_bias.size(),
                logit_bias.data()));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_dist(88));
    ...
    monowire_token backend_token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), test_ctx.idx_for_seq(seq_id));
...
static void test_backend_mixed_sampling(const test_params & params) {
    struct monowire_sampler_chain_params chain_params_0 = monowire_sampler_chain_default_params();
    monowire_sampler_ptr sampler_chain_0(monowire_sampler_chain_init(chain_params_0));
    monowire_sampler_chain_add(sampler_chain_0.get(), monowire_sampler_init_dist(88));

    int k = 40;
    struct monowire_sampler_chain_params chain_params_1 = monowire_sampler_chain_default_params();
    monowire_sampler_ptr sampler_chain_1(monowire_sampler_chain_init(chain_params_1));
    monowire_sampler_chain_add(sampler_chain_1.get(), monowire_sampler_init_top_k(k));
    ...
    // Verify sequence 0 that used the dist backend sampler.
    {
        ...
        GGML_ASSERT(token >= 0 && token < test_ctx.n_vocab);
        //GGML_ASSERT(monowire_get_sampled_logits_ith(test_ctx.ctx.get(), batch_idx) == nullptr);
        //GGML_ASSERT(monowire_get_sampled_logits_count_ith(test_ctx.ctx.get(), batch_idx) == 0);
    }
    ...
    // Verify sequence 1 that used the top-k backend sampler.
    {
        ...
        size_t n_logits = monowire_get_sampled_logits_count_ith(test_ctx.ctx.get(), batch_idx);
        GGML_ASSERT(n_logits == (size_t) k);
        GGML_ASSERT(monowire_get_sampled_token_ith(test_ctx.ctx.get(), batch_idx) == MONOWIRE_TOKEN_NULL);
    }
...
static void test_backend_set_sampler(const test_params & params) {
    ...
    monowire_set_sampler(test_ctx.ctx.get(), seq_id, nullptr);
    ...
    std::map<monowire_seq_id, monowire_token> tokens = { { seq_id, backend_token}, };
    if (!test_ctx.decode_tokens(tokens)) {
        GGML_ASSERT(false && "Failed to decode token");
    }
    ...
    GGML_ASSERT(monowire_get_sampled_token_ith(test_ctx.ctx.get(), idx) == MONOWIRE_TOKEN_NULL);
    GGML_ASSERT(monowire_get_sampled_probs_ith(test_ctx.ctx.get(), idx) == nullptr);

    // Sample the token using the CPU sampler chain.
    monowire_token token2 = monowire_sampler_sample(chain.get(), test_ctx.ctx.get(), seq_id);
    ...
    std::map<monowire_seq_id, monowire_token> tokens2 = { { seq_id, token2}, };

    // Set a new backend sampler for the sequence.
    struct monowire_sampler_chain_params new_backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr new_backend_sampler_chain(monowire_sampler_chain_init(new_backend_chain_params));
    monowire_sampler_chain_add(new_backend_sampler_chain.get(), monowire_sampler_init_top_k(20));
    monowire_sampler_chain_add(new_backend_sampler_chain.get(), monowire_sampler_init_dist(seed));
    monowire_set_sampler(test_ctx.ctx.get(), seq_id, new_backend_sampler_chain.get());

    if (!test_ctx.decode_tokens(tokens2)) {
        GGML_ASSERT(false && "Failed to decode token");
    }

    monowire_token new_backend_token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), test_ctx.idx_for_seq(seq_id));
...
static void test_backend_cpu_mixed_batch(const test_params & params) {
    ...
    // Verify sequence 1 (CPU sampled)
    {
        ...
        monowire_token backend_token = monowire_get_sampled_token_ith(test_ctx.ctx.get(), batch_idx);
        GGML_ASSERT(backend_token == MONOWIRE_TOKEN_NULL);

        struct monowire_sampler_chain_params chain_params = monowire_sampler_chain_default_params();
        monowire_sampler_ptr chain(monowire_sampler_chain_init(chain_params));
        monowire_sampler_chain_add(chain.get(), monowire_sampler_init_greedy());
    ...
    // Clear/remove the backend sampler, and sample again
    {
        ...
        // samplers.
        monowire_set_sampler(test_ctx.ctx.get(), 0, nullptr);
    ...
    {
        struct monowire_sampler_chain_params chain_params = monowire_sampler_chain_default_params();
        monowire_sampler_ptr sampler_chain(monowire_sampler_chain_init(chain_params));
        monowire_sampler_chain_add(sampler_chain.get(), monowire_sampler_init_dist(88));

        monowire_set_sampler(test_ctx.ctx.get(), 0, sampler_chain.get());
...
static void test_backend_max_outputs(const test_params & params) {
    ...
    monowire_sampler_chain_params backend_chain_params = monowire_sampler_chain_default_params();
    monowire_sampler_ptr backend_sampler_chain(monowire_sampler_chain_init(backend_chain_params));
    monowire_sampler_chain_add(backend_sampler_chain.get(), monowire_sampler_init_dist(seed));
    std::vector<monowire_sampler_seq_config> backend_sampler_configs = {{ seq_id, backend_sampler_chain.get() }};
    ...
    monowire_batch batch = monowire_batch_init(512, 0, 1);
    std::string prompt = "Hello";

    std::vector<monowire_token> tokens;
    tokens.push_back(monowire_vocab_bos(test_ctx.vocab));

    std::vector<monowire_token> prompt_tokens(32);
    int n_tokens = monowire_tokenize(test_ctx.vocab, prompt.c_str(), prompt.length(),
                                   prompt_tokens.data(), prompt_tokens.size(),
                                   false, false);
    for (int i = 0; i < n_tokens; i++) {
        tokens.push_back(prompt_tokens[i]);
    ...
    const int ret = monowire_decode(test_ctx.ctx.get(), batch);
    GGML_ASSERT(ret != 0 && "monowire_decode should not succeed multiple outputs per sequence");

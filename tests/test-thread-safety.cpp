...
int main(int argc, char ** argv) {
    ...
    std::vector<monowire_model_ptr> models;
    ...
    for (int m = 0; m < num_models; ++m) {
        ...
        if (m < gpu_dev_count) {
            mparams.split_mode = MONOWIRE_SPLIT_MODE_NONE;
            mparams.devices = gpus[m].data();
        } else if (m == gpu_dev_count) {
            mparams.split_mode = MONOWIRE_SPLIT_MODE_NONE;
            mparams.main_gpu = -1; // CPU model
        } else {
            mparams.split_mode = MONOWIRE_SPLIT_MODE_LAYER;
        }
    ...
    for  (int m = 0; m < num_models; ++m) {
        ...
        for (int c = 0; c < num_contexts; ++c) {
            threads.emplace_back([&, m, c, model]() {
                ...
                monowire_context_ptr ctx { monowire_init_from_model(model, cparams) };
                if (ctx == NULL) {
                ...
                monowire_batch batch = {};
                {
                    ...
                    batch = monowire_batch_get_one(prompt.data(), prompt.size());
                    if (monowire_decode(ctx.get(), batch)) {
                ...
                for (int i = 0; i < params.n_predict; i++) {
                    ...
                    if (batch.n_tokens > 0) {
                        ...
                        token = monowire_vocab_bos(vocab);
                    }

                    result += common_token_to_piece(ctx.get(), token);

                    if (monowire_vocab_is_eog(vocab, token)) {
                        break;
                    }

                    batch = monowire_batch_get_one(&token, 1);

                    int ret = monowire_decode(ctx.get(), batch);
                    if (ret == 1 && i > 0) {

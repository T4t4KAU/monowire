...
static monowire_grammar * build_grammar_with_root(const std::string & grammar_str, const char * grammar_root) {
    return monowire_grammar_init_impl(nullptr, grammar_str.c_str(), grammar_root, false, nullptr, 0, nullptr, 0);
}

static monowire_grammar * build_grammar(const std::string & grammar_str) {
...
static bool test_build_grammar_fails(const std::string & grammar_str) {
    ...
    monowire_grammar * grammar = build_grammar(grammar_str);
    if (grammar != nullptr) {
...
static std::string token(monowire_token id) {
    return std::string{
    ...
    while (offset < input.size()) {
        try {
            if (static_cast<unsigned char>(input[offset]) == 0xff) {
                ...
                result.push_back({static_cast<monowire_token>(val), piece});
                offset += 5;
            } else {
            ...
            // Silently ignore invalid UTF-8 input to avoid leaking the exception beyond monowire_tokenize
            ++offset;
...
static bool match_string(const std::string & input, monowire_grammar * grammar) {
    const auto parsed = parse_tokens(input);

    auto & stacks_cur = monowire_grammar_get_stacks(grammar);

    for (const auto & in : parsed) {
        try {
            monowire_grammar_accept_token(*grammar, in.token, in.piece);
        } catch (const std::runtime_error & /*e*/) {
            // normally this shouldn't get hit because of monowire_grammar_apply
            return false;
        }
...
static void test(const std::string & test_desc, const std::string & grammar_str, const std::vector<std::string> & passing_strings, const std::vector<std::string> & failing_strings) {
    ...
    const monowire_grammar_stacks stacks_org = monowire_grammar_get_stacks(grammar); // copy

    monowire_grammar_stacks & stacks_cur = monowire_grammar_get_stacks(grammar);
    ...
    for (const auto & test_string : passing_strings) {
        ...
        if (!matched) {
            ...
            fprintf(stderr, "\n NOTE: Debug grammar file generated. To analyze this failure in detail, run the following command:     ./monowire-gbnf-validator test-grammar-integration.grammar.gbnf test-grammar-integration.string.txt\n\n");
        } else {
            fprintf(stdout, "✅︎\n");
        }
...
static void test_failure_missing_root_symbol() {
    ...
    monowire_grammar * failure_result = build_grammar_with_root(grammar_str, "nonexistent");
...
static void test_custom_root_symbol_check() {
    ...
    monowire_grammar * failure_result = build_grammar_with_root(custom_root_grammar_str, "root");
    assert(failure_result == nullptr);

    monowire_grammar * success_result = build_grammar_with_root(custom_root_grammar_str, "foobar");
    assert(success_result != nullptr);
    monowire_grammar_free_impl(success_result);

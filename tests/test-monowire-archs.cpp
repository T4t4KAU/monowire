...
// TODO: replace with #include "monowire-ext.h" in the future
#include "../src/monowire-arch.h"
#include "../src/monowire-model-saver.h"
...
static std::vector<monowire_token> get_tokens(const uint32_t n_tokens, const uint32_t n_vocab, const size_t seed){
    std::mt19937 gen(seed);
    std::uniform_int_distribution<> dis(0, n_vocab - 1);
    std::vector<monowire_token> ret;
...
static gguf_context_ptr get_gguf_ctx(const llm_arch arch, const bool moe) {
    ...
    monowire_model_saver ms(arch, ret.get());
...
static std::pair<monowire_model_ptr, monowire_context_ptr> get_model_and_ctx(
        struct gguf_context * gguf_ctx, FILE * file, const size_t seed, const std::vector<ggml_backend_dev_t> & devs,
        const monowire_split_mode split_mode = MONOWIRE_SPLIT_MODE_LAYER, bool encode = false) {
    GGML_ASSERT((gguf_ctx == nullptr) != (file == nullptr));
    monowire_model_params model_params = monowire_model_default_params();
    ...
    monowire_context_params ctx_params = monowire_context_default_params();
    ctx_params.n_ctx = 0;
    ...
    size_t tmp = seed;
    monowire_model_ptr model(gguf_ctx != nullptr ?
        monowire_model_init_from_user(gguf_ctx, set_tensor_data, &tmp, model_params) :
        monowire_model_load_from_file_ptr(file, model_params));
    if (!model) {
        throw std::runtime_error("failed to create Monowire model");
    }
    monowire_context_ptr lctx(monowire_init_from_model(model.get(), ctx_params));
    if (!lctx) {
        throw std::runtime_error("failed to create Monowire context");
    }
    return std::make_pair(std::move(model), std::move(lctx));
}

static std::vector<float> get_logits(
        monowire_model * model, monowire_context * lctx, const std::vector<monowire_token> & tokens, bool encode = false) {
    const uint32_t n_vocab  = monowire_vocab_n_tokens(monowire_model_get_vocab(model));
    const uint32_t n_ctx    = monowire_n_ctx(lctx);
    const uint32_t n_tokens = tokens.size();
    monowire_batch batch = monowire_batch_init(n_ctx, 0, 1);
    ...
    for (uint32_t i = 0; i < n_tokens; i++) {
        const float * logits_ith = monowire_get_logits_ith(lctx, i);
...
static int save_models(const llm_arch target_arch, const size_t seed, const ggml_log_level log_level, const std::string & dir) {
    ...
    for (const llm_arch & arch : llm_arch_all()) {
        ...
        for (bool moe : {false, true}) {
            ...
            if (!monowire_model_saver_supports_arch(arch)) {
            ...
            monowire_model_save_to_file(model_and_ctx.first.get(), path.c_str());
        }
    }
...
static int test_backends(const llm_arch target_arch, const size_t seed, const ggml_log_level log_level) {
    ...
    const std::vector<monowire_token> tokens = get_tokens(128, 128, seed);
    ...
    struct device_config {
        ...
        device_config(std::vector<ggml_backend_dev_t> devs, std::string name, monowire_split_mode split_mode)
    ...
    std::vector<device_config> dev_configs;
    {
        std::vector<ggml_backend_dev_t> devices_meta;
        {
            ...
            for (size_t i = 0; i < device_count; i++) {
                ...
                dev_configs.emplace_back(std::vector<ggml_backend_dev_t>{dev}, ggml_backend_dev_description(dev), MONOWIRE_SPLIT_MODE_LAYER);
        ...
        dev_configs.emplace_back(devices_meta, "Meta", MONOWIRE_SPLIT_MODE_TENSOR);
    }
    ...
    for (const llm_arch & arch : llm_arch_all()) {
        ...
        for (bool moe : {false, true}) {
            ...
            gguf_context_ptr gguf_ctx = get_gguf_ctx(arch, moe);
            std::pair<monowire_model_ptr, monowire_context_ptr> model_and_ctx_cpu;
            std::vector<float> logits_cpu;
            for (device_config & dc : dev_configs) {
                std::pair<monowire_model_ptr, monowire_context_ptr> model_and_ctx_dev;
                std::vector<float> logits_dev;
                std::string status_nmse      = "\033[1;33mSKIP\033[0m";
                std::string status_roundtrip = "\033[1;33mSKIP\033[0m";
                char nmse_str[12] = {0};
                bool skip = !arch_supported(arch) || (dc.split_mode == MONOWIRE_SPLIT_MODE_TENSOR && dc.devs.empty());
#if defined(GGML_USE_WEBGPU)
                skip = true; // FIXME
#endif // GGML_USE_WEBGPU
                if (!skip) {
                    if (logits_cpu.empty()) {
                        model_and_ctx_cpu = get_model_and_ctx(gguf_ctx.get(), nullptr, seed, {}, MONOWIRE_SPLIT_MODE_LAYER, encode);
                        logits_cpu = get_logits(model_and_ctx_cpu.first.get(), model_and_ctx_cpu.second.get(), tokens, encode);
                    }
                    if (dc.split_mode != MONOWIRE_SPLIT_MODE_TENSOR || llm_arch_supports_sm_tensor(arch)) {
                    ...
                    if (file != nullptr && monowire_model_saver_supports_arch(arch) && dc.split_mode != MONOWIRE_SPLIT_MODE_TENSOR) {
                        GGML_ASSERT(model_and_ctx_dev.first && model_and_ctx_dev.second);
                        monowire_model_saver ms = monowire_model_saver(model_and_ctx_dev.first.get());

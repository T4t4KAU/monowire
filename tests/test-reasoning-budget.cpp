...
static void test_reasoning_budget(
    ...
    const std::vector<monowire_token> & sequence,
    ...
    for (size_t i = 0; i < n_vocab; i++) {
        cur.emplace_back(monowire_token_data{(monowire_token)i, logf((float)(i+1)), 0.0f});
    }
    ...
    for (size_t i = 0; i < sequence.size(); i++) {
        ...
        monowire_sampler_accept(sampler, sequence[i]);
...
int main(void) {
    ...
    {
        const std::vector<monowire_token> start = {100};  // start token
        const std::vector<monowire_token> end = {101};    // end token
        const std::vector<monowire_token> forced = {102}; // forced token (not used in this test)
        const std::vector<monowire_token> sequence = {100, 50, 51, 101, 52}; // start, two tokens, end, one more
    ...
    {
        const std::vector<monowire_token> start = {100};
        const std::vector<monowire_token> end = {101};
        const std::vector<monowire_token> forced = {102, 101}; // forced message + end
        const std::vector<monowire_token> sequence = {100, 50, 51, 52, 53}; // start + 4 tokens (budget=2)
    ...
    // Flow: init promotes COUNTING+budget=0 to FORCING, so apply() sees FORCING at i=0
    {
        const std::vector<monowire_token> start = {100};
        const std::vector<monowire_token> end = {101};
        const std::vector<monowire_token> forced = {102, 101};
        const std::vector<monowire_token> sequence = {100, 50, 51, 52}; // start token first, then 3 tokens
    ...
    {
        const std::vector<monowire_token> start = {};
        const std::vector<monowire_token> end = {};
        const std::vector<monowire_token> forced = {102};
        const std::vector<monowire_token> sequence = {50, 51, 52, 53};

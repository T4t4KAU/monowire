...
extern struct monowire_sampler * monowire_sampler_init_dry_testing(int32_t context_size, float dry_multiplier, float dry_base, int32_t dry_allowed_length, int32_t dry_penalty_last_n, const std::vector<std::vector<monowire_token>>& seq_breakers);

static void dump(const monowire_token_data_array * cur_p) {
...
struct sampler_tester {
    sampler_tester(size_t n_vocab) {
        cur.reserve(n_vocab);
        for (monowire_token token_id = 0; token_id < (monowire_token)n_vocab; token_id++) {
            const float logit = logf(token_id);
            cur.emplace_back(monowire_token_data{token_id, logit, 0.0f});
        }

        cur_p = monowire_token_data_array { cur.data(), cur.size(), -1, false };
    }
    ...
    sampler_tester(const std::vector<float> & probs, const std::vector<float> & probs_expected) : probs_expected(probs_expected) {
        ...
        for (monowire_token token_id = 0; token_id < (monowire_token)probs.size(); token_id++) {
            const float logit = logf(probs[token_id]);
            cur.emplace_back(monowire_token_data{token_id, logit, probs[token_id]});
        }

        cur_p = monowire_token_data_array { cur.data(), cur.size(), -1, false };
    }

    void apply(monowire_sampler * sampler) {
        monowire_sampler_apply(sampler, &cur_p);
        monowire_sampler_free(sampler);
    }
...
static void test_temp(const std::vector<float> & probs, const std::vector<float> & probs_expected, float temp) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_temp(temp));
    tester.apply(monowire_sampler_init_dist(0));
    DUMP(&tester.cur_p);
...
static void test_temp_ext(const std::vector<float> & probs, const std::vector<float> & probs_expected, float temp, float delta, float exponent) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_temp_ext(temp, delta, exponent));
    tester.apply(monowire_sampler_init_dist (0));
    DUMP(&tester.cur_p);
...
static void test_top_k(const std::vector<float> & probs, const std::vector<float> & probs_expected, int k) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_top_k(k));
    tester.apply(monowire_sampler_init_dist (0));
    DUMP(&tester.cur_p);
...
static void test_top_p(const std::vector<float> & probs, const std::vector<float> & probs_expected, float p) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_top_p(p, 0));
    tester.apply(monowire_sampler_init_dist (0));
    DUMP(&tester.cur_p);
...
static void test_min_p(const std::vector<float> & probs, const std::vector<float> & probs_expected, float p) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_min_p(p, 0));
    tester.apply(monowire_sampler_init_dist (0));
    DUMP(&tester.cur_p);
...
static void test_xtc(const std::vector<float> & probs, const std::vector<float> & probs_expected, float p, float t) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_xtc(p, t, 0, 0));
    DUMP(&tester.cur_p);
...
static void test_typical(const std::vector<float> & probs, const std::vector<float> & probs_expected, float p) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_typical(p, 0));
    DUMP(&tester.cur_p);
...
static void test_penalties(
    const std::vector<float> & probs, const std::vector<monowire_token> & last_tokens,
    ...
    auto * sampler = monowire_sampler_init_penalties(last_tokens.size(), repeat_penalty, alpha_frequency, alpha_presence);

    for (size_t i = 0; i < last_tokens.size(); i++) {
        monowire_sampler_accept(sampler, last_tokens[i]);
    }
...
static void test_dry(
    ...
    auto * sampler = monowire_sampler_init_dry_testing(1024, dry_multiplier, dry_base, dry_allowed_length, dry_penalty_last_n, seq_breakers);
...
static void test_top_n_sigma(const std::vector<float> & probs, const std::vector<float> & probs_expected, int n) {
    ...
    DUMP(&tester.cur_p);
    tester.apply(monowire_sampler_init_top_n_sigma(n));
    tester.apply(monowire_sampler_init_dist (0));
    DUMP(&tester.cur_p);
...
static void test_sampler_queue(const size_t n_vocab, const std::string & samplers_sequence, const int top_k, const float top_p, const float min_p
    ...
    sampler_tester tester(n_vocab);
          ...
          monowire_token min_token_id = 0;
    const monowire_token max_token_id = n_vocab - 1;

    for (auto s : samplers_sequence) {
        switch (s) {
            case 'k': tester.apply(monowire_sampler_init_top_k(top_k)); break;
            case 'y': GGML_ABORT("typical test not implemented");
            case 'p': tester.apply(monowire_sampler_init_top_p(top_p, 1)); break;
            case 'm': tester.apply(monowire_sampler_init_min_p(min_p, 1)); break;
        ...
        if (s == 'k') {
            ...
            min_token_id = std::max(min_token_id, (monowire_token)(n_vocab - top_k));
            ...
            min_token_id = std::max(min_token_id, (monowire_token)(n_vocab - size));
            min_token_id = std::min(min_token_id, (monowire_token)(n_vocab - 1));
...
static void bench(monowire_sampler * cnstr, const char * cnstr_name, const std::vector<monowire_token_data> & data, int n_iter) {
    std::vector<monowire_token_data> cur(data.size());
    std::copy(data.begin(), data.end(), cur.begin());
    monowire_token_data_array cur_p = { cur.data(), cur.size(), -1, false };
    monowire_sampler_apply(cnstr, &cur_p);
    monowire_sampler_reset(cnstr);
...
static void test_perf() {
    ...
    std::vector<monowire_token_data> data;

    data.reserve(n_vocab);
    for (int i = 0; i < n_vocab; i++) {
        const float logit = 2.0f*((double)(rand())/RAND_MAX - 0.5);
        data.emplace_back(monowire_token_data{i, logit, 0.0f});
    }

    BENCH(monowire_sampler_init_top_k  (40),                     data, 32);
    BENCH(monowire_sampler_init_top_p  (0.8f, 1),                data, 32);
    BENCH(monowire_sampler_init_min_p  (0.2f, 1),                data, 32);
    BENCH(monowire_sampler_init_typical(0.5f, 1),                data, 32);
    BENCH(monowire_sampler_init_xtc    (1.0f, 0.1f, 1, 1),       data, 32);
}

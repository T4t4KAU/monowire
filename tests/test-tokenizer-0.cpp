...
//static const std::map<std::string, std::vector<monowire_token>> & k_tests() {
//    static std::map<std::string, std::vector<monowire_token>> _k_tests = {
//        { ""                      , {  }, },
//        { " "                     , {     220, }, },
//        { "  "                    , {     256, }, },
//        { "   "                   , {     262, }, },
...
using monowire_tests = std::map<std::string, std::vector<monowire_token>>;

static monowire_tests read_tests(const std::string & fname_inp, const std::string & fname_out) {
    monowire_tests tests;
    ...
    for (size_t i = 0; i < sinp.size(); ++i) {
        ...
        std::vector<monowire_token> toks;
...
int main(int argc, char **argv) {
    ...
    // load the vocab
    {
        ...
        model = monowire_model_load_from_file(fname.c_str(), mparams);
        ...
        ctx = monowire_init_from_model(model, cparams);
    ...
    const auto k_tests = [&]() -> monowire_tests {
        if (!fname_text.empty()) {
            return {};
        }
    ...
    for (int i = 0; i < nthread; i++) {
        threads[i] = std::thread([&, i]() {
            for (const auto & test_kv : k_tests) {
                const std::vector<monowire_token> res = common_tokenize(ctx, test_kv.first, add_special, false);
    ...
    if (!fname_text.empty()) {
...

        {
            ...
            for (const auto & tok : res) {
                //ofs << tok << " '" << string_strip(monowire_detokenize(ctx, std::vector<int>{tok})) << "'" << std::endl;
                ofs << tok << "\n";
            }
        }

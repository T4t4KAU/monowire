...
int main_automated_tests(void) {
    ...
    std::vector<monowire_chat_message> conversation {
    ...
    struct TestCase {
        ...
        std::vector<monowire_chat_message> extra_conversation = {};
    };
    ...
    res = monowire_chat_builtin_templates(nullptr, 0);
    assert(res > 0);
    ...
    res = monowire_chat_apply_template("INVALID TEMPLATE", conversation.data(), conversation.size(), true, formatted_chat.data(), formatted_chat.size());
    assert(res < 0);
    ...
    for (const auto & test_case : test_cases) {
        ...
        res = monowire_chat_apply_template(

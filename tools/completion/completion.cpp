...
static monowire_context           ** g_ctx;
static monowire_model             ** g_model;
static common_sampler          ** g_smpl;
static common_params            * g_params;
static std::vector<monowire_token> * g_input_tokens;
static std::ostringstream       * g_output_ss;
static std::vector<monowire_token> * g_output_tokens;
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, MONOWIRE_EXAMPLE_COMPLETION, print_usage)) {
        return 1;
    }
    ...
    LOG_INF("%s: Monowire backend init\n", __func__);

    monowire_backend_init();
    monowire_numa_init(params.numa);

    monowire_model * model = nullptr;
    monowire_context * ctx = nullptr;
    ...
    auto monowire_init = common_init_from_params(params);

    ctx   = monowire_init->context();
    model = monowire_init->model();
    smpl  = monowire_init->sampler(0);
    ...
    monowire_memory_t mem = monowire_get_memory(ctx);
    ...
    monowire_perf_context_reset(ctx);

    LOG_INF("%s: Monowire threadpool init, n_threads = %d\n", __func__, (int) params.cpuparams.n_threads);
    ...
    monowire_attach_threadpool(ctx, threadpool, threadpool_batch);

    const int n_ctx_train = monowire_model_n_ctx_train(model);
    const int n_ctx = monowire_n_ctx(ctx);
    ...
    std::vector<monowire_token> session_tokens;
    ...
    if (!path_session.empty()) {
        ...
        if (!file_exists(path_session)) {
            ...
            if (!monowire_state_load_file(ctx, path_session.c_str(), session_tokens.data(), session_tokens.capacity(), &n_token_count_out)) {
    ...
    const bool add_bos = monowire_vocab_get_add_bos(vocab) && !params.use_jinja;
    if (!monowire_model_has_encoder(model)) {
        GGML_ASSERT(!monowire_vocab_get_add_eos(vocab));
    }

    LOG_DBG("n_ctx: %d, add_bos: %d\n", n_ctx, add_bos);

    std::vector<monowire_token> embd_inp;
    ...
    if (!waiting_for_first_input && embd_inp.empty()) {
        if (add_bos) {
            embd_inp.push_back(monowire_vocab_bos(vocab));
...

    {
        ...
        if (!session_tokens.empty()) {
            for (monowire_token id : session_tokens) {
    ...
    std::vector<monowire_token> embd;

    // single-token antiprompts
    std::vector<monowire_token> antiprompt_token;
    ...
    if (monowire_model_has_encoder(model)) {
        ...
        monowire_token * enc_input_buf = embd_inp.data();

        if (monowire_encode(ctx, monowire_batch_get_one(enc_input_buf, enc_input_size))) {
            LOG_ERR("%s : failed to eval\n", __func__);
            return 1;
        }

        monowire_token decoder_start_token_id = monowire_model_decoder_start_token(model);
        if (decoder_start_token_id == MONOWIRE_TOKEN_NULL) {
            decoder_start_token_id = monowire_vocab_bos(vocab);
        }
    ...
    while ((n_remain != 0 && !is_antiprompt) || params.interactive) {
        ...
        if (!embd.empty()) {
            ...
            if (ga_n == 1) {
                ...
                if (n_past + (int) embd.size() >= n_ctx) {
                    ...
                    monowire_memory_seq_rm (mem, 0, params.n_keep            , params.n_keep + n_discard);
                    monowire_memory_seq_add(mem, 0, params.n_keep + n_discard, n_past, -n_discard);
                ...
                while (n_past >= ga_i + ga_w) {
                    ...
                    monowire_memory_seq_add(mem, 0, ga_i,                n_past,              ib*bd);
                    monowire_memory_seq_div(mem, 0, ga_i + ib*bd,        ga_i + ib*bd + ga_w, ga_n);
                    monowire_memory_seq_add(mem, 0, ga_i + ib*bd + ga_w, n_past + ib*bd,      dd);
        ...
        if ((int) embd_inp.size() <= n_consumed && !is_interacting) {
            ...
            const monowire_token id = common_sampler_sample(smpl, ctx, -1);
            ...
            if (params.conversation_mode && !waiting_for_first_input && !monowire_vocab_is_eog(vocab, id)) {
        ...
        if ((int) embd_inp.size() <= n_consumed) {
            ...
            if (!params.antiprompt.empty()) {
                ...
                if (!last_output.empty()) {
                    monowire_token last_token = common_sampler_last(smpl);
            ...
            if (!waiting_for_first_input && monowire_vocab_is_eog(vocab, common_sampler_last(smpl))) {
            ...
            if ((n_past > 0 || waiting_for_first_input) && is_interacting) {
                ...
                if (buffer.empty()) { // Enter key on empty line lets the user pass control back
                    ...
                    if (need_insert_eot && format_chat) {
                        monowire_token eot = monowire_vocab_eot(vocab);
                        embd_inp.push_back(eot == MONOWIRE_TOKEN_NULL ? monowire_vocab_eos(vocab) : eot);
                    ...
                    for (size_t i = original_size; i < embd_inp.size(); ++i) {
                        const monowire_token token = embd_inp[i];
        ...
        // end of generation
        if (!embd.empty() && monowire_vocab_is_eog(vocab, embd.back()) && !(params.interactive)) {
            LOG(" [end of text]\n");
            break;
        }
    ...
    if (!path_session.empty() && params.prompt_cache_all && !params.prompt_cache_ro) {
        ...
        monowire_state_save_file(ctx, path_session.c_str(), session_tokens.data(), session_tokens.size());
    }

...
```./monowire-quantize [options] input-model-f32.gguf [output-model-quant.gguf] type [threads]```
...
./monowire-quantize ./models/mymodel/ggml-model-f16.gguf ./models/mymodel/ggml-model-Q4_K_M.gguf Q4_K_M

# update the gguf filetype to current version if older version is now unsupported
./monowire-quantize ./models/mymodel/ggml-model-Q4_K_M.gguf ./models/mymodel/ggml-model-Q4_K_M-v2.gguf COPY
```
...
./monowire-cli -m ./models/mymodel/ggml-model-Q4_K_M.gguf -cnv -p "You are a helpful assistant"
```
...
* `--imatrix` uses data in file generated by an importance-matrix tool as importance matrix for quant optimizations (highly recommended)
...
./monowire-quantize input-model-f32.gguf q4_k_m 8
```
...
./monowire-quantize --allow-requantize --leave-output-tensor --pure input-model-f32.gguf q4_k_m 8
```

```bash
# quantize model using an importance matrix for specified tensors only (attn_v and ffn_down)
./monowire-quantize --imatrix imatrix.gguf --include-weights attn_v --include-weights ffn_down input-model-f32.gguf q4_k_m 8
```

```bash
# quantize model setting output tensor to Q5_K_M, token embeddings to Q3_K_M, and keeping the input file's shards
./monowire-quantize --imatrix imatrix.gguf --output-tensor-type q5_k --token-embedding-type q3_k --keep-split input-model-f32.gguf q4_k_m 8
```
...
./monowire-quantize --imatrix imatrix.gguf --tensor-type "\.(\d*[13579])\.attn_k=q5_k" --tensor-type "\.(\d*[02468])\.attn_q=q3_k" input-model-f32.gguf q4_k_m 8
```

```bash
# quantize model setting tensors attn_v and ffn_down to Q5_K_M and pruning layers 20, 21, and 22
./monowire-quantize --imatrix imatrix.gguf --tensor-type attn_v=q5_k --tensor-type ffn_down=q5_k --prune-layers 20,21,22 input-model-f32.gguf q4_k_m 8
```
...
./monowire-quantize --imatrix imatrix.gguf --override-kv qwen3moe.expert_used_count=int:16 --prune-layers 20,21,22 input-model-f32.gguf pruned-model-f32.gguf copy 8
```
...
## Background information on monowire-quantize

- [k-quants](upstream-reference)
- k-quants improvements and i-quants
  - [#2707](upstream-reference)
  - [#2807](upstream-reference)
  - [#4773 - 2-bit i-quants (inference)](upstream-reference)
  - [#4856 - 2-bit i-quants (inference)](upstream-reference)
  - [#4861 - importance matrix](upstream-reference)
  - [#4872 - MoE models](upstream-reference)
  - [#4897 - 2-bit quantization](upstream-reference)
  - [#4930 - imatrix for all k-quants](upstream-reference)
  - [#4951 - imatrix on the GPU](upstream-reference)
  - [#4969 - imatrix for legacy quants](upstream-reference)
  - [#4996 - k-quants tuning](upstream-reference)
  - [#5060 - Q3_K_XS](upstream-reference)
  - [#5196 - 3-bit i-quants](upstream-reference)
  - [quantization tuning](upstream-reference), [another one](upstream-reference), and [another one](upstream-reference)

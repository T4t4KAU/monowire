...
static std::string trim(const std::string &str) {
    size_t start = 0;
    size_t end = str.size();
    while (start < end && isspace(static_cast<unsigned char>(str[start]))) { start += 1; }
    while (end > start && isspace(static_cast<unsigned char>(str[end - 1]))) { end -= 1; }
...
llm_chat_template llm_chat_detect_template(const std::string &tmpl) {
    ...
    if (tmpl_contains("<|im_start|>")) {
        return tmpl_contains("<|im_sep|>")         ? LLM_CHAT_TEMPLATE_PHI_4
             : tmpl_contains("<end_of_utterance>") ? LLM_CHAT_TEMPLATE_SMOLVLM // SmolVLM uses <|im_start|> as BOS,
                                                                               // but it is NOT chatml
    ...
    } else if (tmpl_contains("message['role'] + additional_special_tokens[0] + "
...
// Simple version of "monowire_apply_chat_template" that only works with strings
// This function uses heuristic checks to determine commonly used template. It
// is not a jinja parser.
int32_t llm_chat_apply_template(llm_chat_template tmpl, const std::vector<const monowire_chat_message *> &chat,
                                std::string &dest, bool add_ass) {
    // Taken from the research: upstream-reference
    std::stringstream ss;
    if (tmpl == LLM_CHAT_TEMPLATE_CHATML) {
        ...
        // SmolVLM
        ss << "<|im_start|>"; // uses <|im_start|> as BOS, but the actual content is
                              // NOT chatml
        for (auto message : chat) {
            std::string role(message->role);
            if (role == "system") {
...
int32_t monowire_chat_builtin_templates(const char **output, size_t len) {

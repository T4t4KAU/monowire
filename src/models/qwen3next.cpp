...
llm_build_qwen3next::llm_build_qwen3next(const monowire_model &model, const llm_graph_params &params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // Residual connection for FFN - add to the tensor from before
...
ggml_tensor *llm_build_qwen3next::build_layer_attn(llm_graph_input_attn_kv *inp, ggml_tensor *cur, ggml_tensor *inp_pos,
    ...
    // Order: joint QG projection, QG split, Q norm, KV projection, K norm, RoPE,
    // attention
    ...
    // TODO: CUDA is missing non-contiguous unary ops. when implemented: remove
    // this cont
...
std::pair<ggml_tensor *, ggml_tensor *> llm_build_qwen3next::build_qkvz(ggml_tensor *input, int il) {
    ...
    if (model.layers[il].wqkv) {
        ...
        // After creating query, key, and value_reshaped, reshape each to flatten
        // the head dimensions query: [head_k_dim, num_k_heads, n_tokens, n_seqs] ->
        ...
        // key: [head_k_dim, num_k_heads, n_tokens, n_seqs] -> [head_k_dim *
        // num_k_heads, n_tokens, n_seqs]
        ggml_tensor *key_flat = ggml_cont_3d(ctx0, key, head_k_dim * num_k_heads, n_seq_tokens, n_seqs);
        cb(key_flat, "key_flat", il);

        // value_reshaped: [head_v_dim, num_v_heads, n_tokens, n_seqs] ->
        ...
        // Now concatenate along the feature dimension (dim 0) to get [conv_dim,
        // n_tokens, n_seqs]
...
ggml_tensor *llm_build_qwen3next::build_layer_attn_linear(llm_graph_input_rs *inp, ggml_tensor *cur, int il) {
    ...
    // Reshape mixed_ba: [batch, seq_len, hidden_size] -> [batch, seq_len,
    // num_k_heads, 2*num_v_heads/num_k_heads]
    ...
    // Reshape a to merge head dimensions: [batch, seq_len, num_k_heads,
    // num_v_heads/num_k_heads] -> [batch, seq_len, num_v_heads]
    ...
    // q_conv = ggml_cont_4d(ctx0, q_conv, head_k_dim, num_k_heads, n_seq_tokens,
    // n_seqs); k_conv = ggml_cont_4d(ctx0, k_conv, head_k_dim, num_k_heads,
    // n_seq_tokens, n_seqs); v_conv = ggml_cont_4d(ctx0, v_conv, head_v_dim,
    // num_v_heads, n_seq_tokens, n_seqs);

    // if head keys and value keys are different, repeat to force tensors into
    // matching shapes
    // TODO: avoid repeats for fused GDN, needs broadcast configuration for GDN op
    // [TAG_GGML_GDN_BCAST]
    if (num_k_heads != num_v_heads) {
        GGML_ASSERT(num_v_heads % num_k_heads == 0);
        int64_t repeat_factor = num_v_heads / num_k_heads;

        // repeat interleave: reshape to (repeat part, 1, remaining part...), do
        // repeat, then reshape back
    ...
    // z: [head_dim, n_heads, n_tokens, n_seqs] -> [n_heads * n_tokens * n_seqs,
    // head_dim]
    ...
    // Final reshape: [head_dim, n_heads, n_tokens, n_seqs] -> [n_tokens, n_seqs,
    // n_heads * head_dim]
    ...
    if (model.layers[il].ffn_gate_inp != nullptr) {
        ...
        ggml_tensor *moe_out
            = build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                            ...
                            MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il, nullptr, model.layers[il].ffn_gate_up_exps);
        cb(moe_out, "ffn_moe_out", il);

        // Add shared experts if present - following Qwen3Next reference
        // implementation
        if (model.layers[il].ffn_up_shexp != nullptr) {
            ...
            // Note: ffn_gate_inp_shexp is the shared expert gate (outputs 1 value per
            // token)

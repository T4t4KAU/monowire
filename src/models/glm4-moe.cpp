...
llm_build_glm4_moe::llm_build_glm4_moe(const monowire_model &model, const llm_graph_params &params)
    ...
    if (ubatch.embd && !use_mrope) {
        // unfortunately, we need to forcefully stop here, to avoid users
    ...
    for (int il = 0; il < n_transformer_layers; ++il) {
        ...
        // Check if this is a dense layer (n_layer_dense_lead=1, so layer 0 is
        // dense)
        if (static_cast<uint32_t>(il) < hparams.n_layer_dense_lead) {
            ...
            ggml_tensor *routed_out = build_moe_ffn(
                ...
                (monowire_expert_gating_func_type)hparams.expert_gating_func, il);

...
llm_build_plamo2::llm_build_plamo2(const monowire_model &model, const llm_graph_params &params)
...
ggml_tensor *llm_build_plamo2::build_plamo2_attn_layer(llm_graph_input_attn_kv *inp, ggml_tensor *inp_pos,
                                                       ggml_tensor *cur, const monowire_model &model, int il) {
    // self-attention
    {
        ...
        cur = build_attn(inp, model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, Kcur, Vcur, NULL, NULL, NULL,
...
ggml_tensor *llm_build_plamo2::build_plamo2_mamba_layer(llm_graph_input_rs *inp, ggml_tensor *cur,
    ...
    // in_proj: {n_embd, 2*d_inner} @ {n_embd, n_seq_tokens, n_seqs} =>
    // {2*d_inner, n_seq_tokens, n_seqs}
    ...
    // SSM
    {
        // bcdt_proj: {d_inner, dt_rank + 2*d_state} @ {d_inner, n_seq_tokens,
        // n_seqs} => {dt_rank + 2*d_state, n_seq_tokens, n_seqs}
        ...
        // dt_proj: {dt_rank, d_inner} @ {dt_rank, n_seq_tokens, n_seqs} =>
        ...
        // {d_inner, n_seq_tokens, n_seqs} * {d_inner} => {d_inner, n_seq_tokens,
        // n_seqs}
        ...
        // out_proj: {d_inner, n_embd} @ {d_inner, n_seq_tokens, n_seqs} => {n_embd,
        // n_seq_tokens, n_seqs}

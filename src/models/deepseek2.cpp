...
llm_build_deepseek2::llm_build_deepseek2(const monowire_model &model, const llm_graph_params &params)
    ...
    // note: these are the actual head sizes you get when treating as MHA or after
    // "decompression" using wv_b for MLA
    ...
    // correctly. See upstream-reference for detailed explanation. And also:
    // upstream-reference [TAG_DEEPSEEK2_YARN_LOG_MUL_FIX]

    // first cancel the adjustment from monowire_hparams::yarn_attn_factor_adjust
    // to get the original attn_factor
    ...
    for (int il = 0; il < effective_n_layers; ++il) {
        ...
        if (is_ocr) {
            ...
            Qcur
                = ggml_rope_ext(ctx0, Qcur, inp_pos, nullptr, n_embed_head, ocr_rope_type, 0, freq_base, 1, 0, 1, 0, 0);
            Kcur
                = ggml_rope_ext(ctx0, Kcur, inp_pos, nullptr, n_embed_head, ocr_rope_type, 0, freq_base, 1, 0, 1, 0, 0);
            cb(Qcur, "q_pe", il);
            cb(Kcur, "k_pe", il);

            cur = build_attn(inp_attn_kv, model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, Kcur, Vcur, nullptr,
            ...
            if (is_mla) {
                ...
                // note: MLA with the absorption optimization converts into MQA (ie: GQA
                // with 1 group)
                cur = build_attn(inp_attn_k, model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, Kcur, Vcur,
                                 nullptr, nullptr, model.layers[il].wv_b, kq_scale, il);
            } else {
                ...
                // note: MLA without the absorption optimization converts into MHA (ie:
                // GQA with full n_head groups)
                cur = build_attn(inp_attn_kv, model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, Kcur, Vcur,
        ...
        if ((uint32_t)il < hparams.n_layer_dense_lead) {
            ...
            ggml_tensor *moe_out = build_moe_ffn(
                ...
                (monowire_expert_gating_func_type)hparams.expert_gating_func, il, nullptr,

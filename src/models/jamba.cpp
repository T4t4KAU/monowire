...
llm_build_jamba::llm_build_jamba(const monowire_model &model, const llm_graph_params &params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (n_head_kv == 0) {
            ...
            // No RoPE :)
            cur = build_attn(inp_hybrid->get_attn(), model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, Kcur, Vcur,
                             NULL, NULL, NULL, 1.0f / sqrtf(float(n_embd_head)), il);
        }

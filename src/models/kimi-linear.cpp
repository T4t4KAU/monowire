...
static ggml_tensor *causal_conv1d(ggml_cgraph *gf, ggml_context *ctx0, ggml_tensor *conv_states_all,
    ...
    // Each conv state is [(d_conv-1) * d_inner] per sequence, need to reshape to
    // [d_conv-1, d_inner, n_seqs] Memory layout: for each seq, Q state is first
    // conv_state_size elements, then K, then V conv_state_all has stride: nb[0] =
    // element_size, nb[1] = n_embd_r_total * element_size View Q conv state:
    // offset 0, size conv_state_size per seq conv_state_all is [n_embd_r_total,
    // n_seqs] with memory layout:
    //   state[i + seq * n_embd_r_total] where i = conv_step + channel *
    //   (d_conv-1) + {0, conv_state_size, 2*conv_state_size} for Q/K/V
    ...
    // Concat Q conv state and current input: {d_conv-1 + n_seq_tokens, d_inner,
    // n_seqs}
    ...
    ggml_build_forward_expand(
        gf, ggml_cpy(ctx0, last_conv_x,
                     ggml_view_3d(ctx0, conv_states_all, d_conv - 1, d_inner, n_seqs,
                                  (d_conv - 1) * ggml_element_size(conv_states_all),   // nb1: contiguous within
                                                                                       // one channel's conv taps
                                  n_embd_r_total * ggml_element_size(conv_states_all), // nb2: stride between sequences
                                                                                       // (skip over K,V states)
                                  (kv_head * n_embd_r_total + qkv * conv_state_size)
                                      * ggml_element_size(conv_states_all)))); // offset to first seq's Q/K/V state
    // Reshape conv weight: GGUF [d_conv, 1, d_inner, 1] -> ggml_ssm_conv expects
    // [d_conv, d_inner] GGUF stores as [d_conv, 1, d_inner, 1] with memory layout
    // w[conv_step + channel * d_conv] vLLM stores as [d_inner, d_conv] with
    // memory layout w[channel * d_conv + conv_step] ggml_ssm_conv computes:
    // c[conv_step + channel * d_conv] GGUF layout: [d_conv, 1, d_inner] or
    // [d_conv, 1, d_inner, 1] -> reshape to [d_conv, d_inner] Reshape conv weight
    // from [d_conv, 1, d_inner, 1] to [d_conv, d_inner] for ggml_ssm_conv
...
llm_build_kimi_linear::llm_build_kimi_linear(const monowire_model &model, const llm_graph_params &params)
    ...
    // Confirmed from tensor shape: wkv_a_mqa [2304, 576] = [n_embd, kv_lora_rank
    // + qk_rope_head_dim]
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        if (hparams.is_recurrent(il)) {
            ...
            // A_log shape is [1, n_head] or [1, n_head, 1, 1], need to broadcast to
            // [head_dim, n_head, n_tokens]. No need to -exp(a_log) because it was
            // done in convert_hf_to_gguf.py Reshape to [1, n_head, 1] for
            // broadcasting with g1 [head_dim, n_head, n_tokens]
            ...
            // Choose between build_delta_net_chunking and build_delta_net_recurrent
            // based on n_tokens
            ...
            // Note: Kimi model uses sigmoid gating, not SiLU (despite
            // FusedRMSNormGated default being swish) Formula: output = RMSNorm(x) *
            // sigmoid(g)
            ...
            // vLLM Kimi: q = q_proj(hidden_states), then view as [n_tokens, n_head,
            // qk_head_dim] Note: Kimi MLA does NOT use RoPE (rotary_emb=None in vLLM)
            ggml_tensor *Qcur = ggml_mul_mat(ctx0, layer.wq, cur);

            // Step 2: KV compression
            // kv_cmpr_pe = kv_a_proj_with_mqa(hidden_states) -> [kv_lora_rank +
            // qk_rope_head_dim, n_tokens]
            ...
            if (layer.wk_b && layer.wv_b) { // MLA KV cache enabled
                ...
                // {n_embd_head_qk_nope, kv_lora_rank, n_head} x {n_embd_head_qk_nope,
                // n_tokens, n_head}
                ...
                // note: rope must go first for in-place context shifting in
                // build_rope_shift()
                ...
                cur = build_attn(inp_attn_k, layer.wo, NULL, layer.wo_s, Qcur, Kcur, Vcur, nullptr, nullptr, layer.wv_b,
                                 kq_scale_mla, il);
                cb(cur, "mla_out", il);
                ...
                // K = [k_nope, k_pe] where k_nope is [qk_nope_head_dim, n_head,
                // n_tokens] and k_pe is [qk_rope_head_dim, 1, n_tokens] broadcast to
                // all heads Need to broadcast k_pe from [qk_rope, 1, n_tokens] to
                // [qk_rope, n_head, n_tokens]
                ...
                cur = build_attn(inp_attn_kv, layer.wo, NULL, layer.wo_s, Qcur, Kcur, Vcur, nullptr, nullptr, nullptr,
                                 kq_scale_mla, il);
                cb(cur, "mla_out", il);
            }
        }
        ...
        if ((uint32_t)il < hparams.n_layer_dense_lead) {
            ...
            // MoE layer
            // Kimi uses moe_renormalize=True and routed_scaling_factor (stored as
            // expert_weights_scale) = 2.446
            ggml_tensor *moe_out = build_moe_ffn(
                ...
                hparams.expert_weights_scale, (monowire_expert_gating_func_type)hparams.expert_gating_func, il);

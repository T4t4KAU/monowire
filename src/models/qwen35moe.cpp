...
llm_build_qwen35moe::llm_build_qwen35moe(const monowire_model &model, const llm_graph_params &params)
...
ggml_tensor *llm_build_qwen35moe ::build_layer_attn(llm_graph_input_attn_kv *inp, ggml_tensor *cur,
    ...
    ggml_tensor *Qcur_full = build_lora_mm(model.layers[il].wq, cur,
                                           model.layers[il].wq_s); // [ (n_embd_head * 2) * n_head, n_tokens ]
...
ggml_tensor *llm_build_qwen35moe ::build_layer_attn_linear(llm_graph_input_rs *inp, ggml_tensor *cur, int il) {
    ...
    // matching shapes note: need explicit repeat only if we are not using the
    // fused GDN
...
ggml_tensor *llm_build_qwen35moe ::build_layer_ffn(ggml_tensor *cur, const int il) {
    ...
    ggml_tensor *moe_out = build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                                         ...
                                         MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il, nullptr,

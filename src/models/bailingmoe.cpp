...
llm_build_bailingmoe::llm_build_bailingmoe(const monowire_model &model, const llm_graph_params &params)
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            auto [Qcur, Kcur, Vcur] = build_qkv(model.layers[il], cur, n_embd_head_k, n_head, n_head_kv, il);
            ...
            cur = build_attn(inp_attn, model.layers[il].wo, model.layers[il].wo_b, model.layers[il].wo_s, Qcur, Kcur,
                             Vcur, nullptr, nullptr, nullptr, 1.0f / sqrtf(float(n_rot)), il);
        }
        ...
        ggml_tensor *moe_out = build_moe_ffn(
            ...
            model.layers[il].ffn_down_exps, nullptr, n_expert, n_expert_used, LLM_FFN_SILU, hparams.expert_weights_norm,
            hparams.expert_weights_scale, MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il);

...
llm_build_gemma4_iswa::llm_build_gemma4_iswa(const monowire_model &model, const llm_graph_params &params)
    : llm_graph_context(params), model(model), n_embd_per_layer(model.hparams.n_embd_per_layer) {
    ggml_tensor *cur;
    ggml_tensor *inpL;
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        ggml_tensor *Qcur;
        {
            ...
            Qcur = ggml_rope_ext(ctx0, Qcur, inp_pos, freq_factors, n_rot_l, rope_type, n_ctx_orig, freq_base_l,
        ...
        if (hparams.has_kv(il)) {
            ...
            ggml_tensor *Vcur = model.layers[il].wv ? build_lora_mm(model.layers[il].wv, cur, model.layers[il].wv_s)
            ...
            Kcur = ggml_rope_ext(ctx0, Kcur, inp_pos, freq_factors, n_rot_l, rope_type, n_ctx_orig, freq_base_l,
                                 freq_scale_l, ext_factor, attn_factor, beta_fast, beta_slow);

            cb(Kcur, "Kcur_pos", il);

            cur = build_attn(inp_attn, model.layers[il].wo, nullptr, model.layers[il].wo_s, Qcur, Kcur, Vcur, nullptr,
                             nullptr, nullptr, hparams.f_attention_scale, il);
        } else {
            // reuse KV cache of earlier layers
            cur = build_attn(inp_attn, model.layers[il].wo, nullptr, model.layers[il].wo_s, Qcur, nullptr, nullptr,
                             nullptr, nullptr, nullptr, hparams.f_attention_scale, il);
        }

        // TODO @ngxson : strip unused token right after the last KV layer to speed
        ...
        if (is_moe_layer) {
            ...
            cur_mlp
                = build_ffn(cur_mlp, model.layers[il].ffn_up, nullptr, model.layers[il].ffn_up_s,
                            model.layers[il].ffn_gate, nullptr, model.layers[il].ffn_gate_s, model.layers[il].ffn_down,
                            nullptr, model.layers[il].ffn_down_s, nullptr, LLM_FFN_GELU, LLM_FFN_PAR, il);
            ...
            ggml_tensor *logits = build_lora_mm(model.layers[il].ffn_gate_inp,
                                                tmp); // [n_expert, n_tokens]
            cb(logits, "ffn_moe_logits", il);

            cur_moe
                = build_moe_ffn(cur_moe,
                                nullptr, // gate_inp
                                nullptr, // up_exps
                                nullptr, // gate_exps
                                model.layers[il].ffn_down_exps,
                                nullptr, // exp_probs_b (not used for gemma4)
                                n_expert, n_expert_used, LLM_FFN_GELU, true, 1.0f,
                                MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il, logits, model.layers[il].ffn_gate_up_exps,
                                nullptr, // up_exps_s
                                nullptr, // gate_exps_s
            ...
            cur = build_ffn(cur, model.layers[il].ffn_up, nullptr, model.layers[il].ffn_up_s, model.layers[il].ffn_gate,
                            ...
                            model.layers[il].ffn_down_s, nullptr, LLM_FFN_GELU, LLM_FFN_PAR, il);
            cb(cur, "ffn_out", il);
        }
        ...
        if (inp_per_layer) {
            ...
            cur = build_lora_mm(model.layers[il].per_layer_inp_gate,
                                cur); // [n_embd_per_layer, n_tokens]
            ...
            cur = build_lora_mm(model.layers[il].per_layer_proj,
                                cur); // [n_embd, n_tokens]
...
ggml_tensor *llm_build_gemma4_iswa::build_inp_per_layer() {
    ...
    if (ubatch.token) {
        ...
        // TODO: verify if this is the correct behavior in transformers
        // implementation
...
// this calculates the per-layer inputs, so the final tensor shape will have
// n_layer as the last dim inp_batch     shape: [n_embd, n_tokens] inp_per_layer
// shape: [n_embd_per_layer, n_layer, n_tokens] (from build_inp_per_layer)
...
ggml_tensor *llm_build_gemma4_iswa::project_per_layer_inputs(ggml_tensor *inp_batch, ggml_tensor *inp_per_layer) {
    ...
    // note: this matrix multiplication will be performed in the input layer (i.e.
    // on the CPU)

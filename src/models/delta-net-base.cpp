...
llm_build_delta_net_base::build_delta_net_chunking(ggml_tensor *q, ggml_tensor *k, ggml_tensor *v, ggml_tensor *g,
    ...
    if (kda) {
        ...
        ggml_tensor *g_cs_i = ggml_reshape_4d(ctx0, g_cs, CS, 1, S_k,
                                              CHB); // [chunk_size, 1, S_k, CHB]
        ggml_tensor *g_cs_j = ggml_reshape_4d(ctx0, g_cs, 1, CS, S_k,
                                              CHB); // [1, chunk_size, S_k, CHB]
        ...
        // decay_mask [S_k,BT_j,BT_i,CHB] *Note* second and third chunk_sizes are
        // switched
    ...
    //   g_last = torch.clamp(g_cum[:, :, -1],
    //   max=50.0).exp().unsqueeze(-1).unsqueeze(-1) g_diff = torch.clamp(g_cum[:,
    //   :, -1:] - g_cum, max=50.0).exp() key_gdiff = key * g_diff.unsqueeze(-1)
    ...
    ggml_tensor *o = ggml_view_4d(ctx0, v, S_v, n_tokens, H_v, n_seqs, ggml_row_size(v->type, S_v),
...
std::pair<ggml_tensor *, ggml_tensor *>
llm_build_delta_net_base::build_delta_net_autoregressive(ggml_tensor *q, ggml_tensor *k, ggml_tensor *v, ggml_tensor *g,
                                                         ggml_tensor *b, // beta
                                                         ggml_tensor *s, // state
                                                         int il) {
    const int64_t S_k = q->ne[0];
...
std::pair<ggml_tensor *, ggml_tensor *> llm_build_delta_net_base::build_delta_net_fused(ggml_tensor *q, ggml_tensor *k,
    ...
    if (n_tokens == 1) {
        cb(result, MONOWIRE_TENSOR_NAME_FGDN_AR, il);
    } else {
        cb(result, MONOWIRE_TENSOR_NAME_FGDN_CH, il);
    }

...
template <>
llm_build_t5<false>::llm_build_t5(const monowire_model &model, const llm_graph_params &params)
    ...
    for (int il = 0; il < dec_n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            cur = build_attn(inp_attn_self, model.layers[il].wo, model.layers[il].wo_b, model.layers[il].wo_s, Qcur,
                             Kcur, Vcur, kq_b, nullptr, nullptr, 1.0f, il);
            cb(cur, "kqv_out", il);
        }
        ...
        // cross-attention
        {
            ...
            cur = build_attn(inp_attn_cross, model.layers[il].wo_cross, nullptr, nullptr, Qcur, Kcur, Vcur, nullptr,
                             nullptr, nullptr, 1.0f, il);
            cb(cur, "kqv_out", il);

            // ggml_tensor * q =                 ggml_permute(ctx0, Qcur, 0, 2, 1, 3);
            // ggml_tensor * k = ggml_cont(ctx0, ggml_permute(ctx0, Kcur, 0, 2, 1,
            // 3));
            ...
            // hparams.f_max_alibi_bias); cb(kq, "kq_soft_max_ext", il);

            // ggml_tensor * v = ggml_cont(ctx0, ggml_transpose(ctx0,
            // ggml_reshape_2d(ctx0, Vcur, n_embd_gqa, n_outputs_enc))); cb(v, "v",
            // il);

            // ggml_tensor * kqv = ggml_mul_mat(ctx0, ggml_reshape_3d(ctx0, v,
            // n_outputs_enc, n_embd_head, n_head_kv), kq); cb(kqv, "kqv", il);
...
template <>
llm_build_t5<true>::llm_build_t5(const monowire_model &model, const llm_graph_params &params)
    : llm_graph_context(params) {
    const int64_t n_embd_head = hparams.n_embd_head_v();
    ...
    for (int il = 0; il < n_layer; ++il) {
        ...
        // self-attention
        {
            ...
            cur = build_attn(inp_attn, model.layers[il].wo_enc, nullptr, nullptr, Qcur, Kcur, Vcur, kq_b, nullptr,
                             nullptr, 1.0f, il);
            cb(cur, "kqv_out", il);
        }

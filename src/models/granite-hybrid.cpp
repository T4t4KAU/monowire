...
llm_build_granite_hybrid::llm_build_granite_hybrid(const monowire_model &model, const llm_graph_params &params)
...
ggml_tensor *llm_build_granite_hybrid::build_attention_layer(ggml_tensor *cur, ggml_tensor *inp_pos,
                                                             ...
                                                             const monowire_model &model, const int64_t n_embd_head,
                                                             const int il) {
    auto [Qcur, Kcur, Vcur]
...
ggml_tensor *llm_build_granite_hybrid::build_layer_ffn(ggml_tensor *cur, ggml_tensor *inpSA,
                                                       const monowire_model &model, const int il) {
    ...
    if (model.layers[il].ffn_gate_inp == nullptr) {
        ...
        ggml_tensor *moe_out = build_moe_ffn(cur, model.layers[il].ffn_gate_inp, model.layers[il].ffn_up_exps,
                                             ...
                                             MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX, il);

...
llm_build_gemma3n_iswa::llm_build_gemma3n_iswa(const monowire_model &model, const llm_graph_params &params)
    ...
    // these "added" altups will be concat to the last dim of inpL
    {
        ...
        ggml_tensor *altup_added = ggml_mul_mat(ctx0, model.altup_proj,
                                                inp_repeated); // shape: [n_embd, n_tokens, n_altup - 1]
        ...
        inpL = ggml_concat(ctx0, inpL, altup_added,
                           2); // shape: [n_embd, n_tokens, n_altup]
        cb(inpL, "inp_stacked", -1);
    }
    // inpL now has shape: [n_embd, n_tokens, n_altup]

    for (int il = 0; il < n_layer; ++il) {
        // this block is made to be closely resemble Gemma3p5DecoderLayer on python
        // code
        ...
        if (hparams.has_kv(il)) {
            ...
            cur = build_attn(inp_attn, model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, Kcur, Vcur, nullptr,
            ...
            cur = build_attn(inp_attn, model.layers[il].wo, NULL, model.layers[il].wo_s, Qcur, nullptr, nullptr,
        ...
        ggml_tensor *corrected = altup_correct(predictions, attn_ffw_laurel_gated,
                                               il); // [n_embd, n_tokens, n_altup]

        ggml_tensor *first_prediction; // [n_embd, n_tokens]
        {
            first_prediction = ggml_view_2d_slice(ctx0, corrected,
                                                  i_altup_act); // [n_embd, n_tokens]
            ...
            first_prediction = ggml_mul(ctx0, first_prediction,
                                        inp_this_layer); // [n_embd_altup, n_tokens]
            cb(first_prediction, "first_prediction_scaled", il);

            first_prediction = build_lora_mm(model.layers[il].per_layer_proj,
                                             first_prediction); // [n_embd, n_tokens]
            first_prediction
        ...
        // equivalent to python code: corrected_predictions[1:] += first_prediction
        {
            ...
            ggml_tensor *tmp = ggml_add(ctx0, slice_rest,
                                        first_prediction);      // [n_embd, n_tokens, n_altup - 1]
    ...
    {
        ggml_tensor *target_magnitude
            = calc_magnitude(ggml_view_2d_slice(ctx0, cur, i_altup_act)); // [n_embd, n_tokens]
        ...
        ggml_tensor *altup_unembd = ggml_mul_mat(ctx0, model.altup_unembd_proj,
                                                 alt_slice); // shape: [n_embd, n_tokens, n_altup - 1]
        ...
        for (int i = 0; i < n_altup - 1; ++i) { cur = ggml_add(ctx0, cur, ggml_view_2d_slice(ctx0, altup_unembd, i)); }
...
// n_layer as the last dim output shape: [n_embd_altup, n_tokens, n_layer]
...
ggml_tensor *llm_build_gemma3n_iswa::altup_predict(ggml_tensor *cur, int il) {
    ...
    // first dim now having n_altup^2 elements, we reshape it to 2D (so we end up
    // with 3D tensor)
...
ggml_tensor *llm_build_gemma3n_iswa::altup_correct(ggml_tensor *predictions, ggml_tensor *activated, int il) {
    ...
    ggml_tensor *all_coefs = build_lora_mm(model.layers[il].altup_correct_coef,
                                           modalities);       // [n_altup, n_tokens]
    all_coefs = ggml_scale_bias(ctx0, all_coefs, 1.0f, 1.0f); // + 1.0
    cb(all_coefs, "all_coefs", il);
    all_coefs = ggml_transpose(ctx0, all_coefs); // [n_tokens, n_altup]
    all_coefs = ggml_cont_3d(ctx0, all_coefs, 1, n_tokens,
                             n_altup); // [1, n_tokens, n_altup]

...
ggml_tensor *llm_build_mamba_base::build_mamba_layer(llm_graph_input_rs *inp, ggml_tensor *cur,
                                                     const monowire_model &model, const monowire_ubatch &ubatch,
                                                     int il) {
    ...
    // Some variants of Mamba arch (e.g. FalconMamba do apply layer norm on B and
    // Dt layers)
    ...
    // {n_embd, 2*d_inner} @ {n_embd, n_seq_tokens, n_seqs} => {2*d_inner,
    // n_seq_tokens, n_seqs}
    ...
    // conv
    {
        ...
        // (the last two steps are a dot product over rows (also doable with
        // mul_mat)) then permute away the ne[0] dimension, and then you're left
        // with the resulting x tensor. For simultaneous sequences, all sequences
    ...
    // ssm
    {
        // {d_inner, dt_rank + 2*d_state} @ {d_inner, n_seq_tokens, n_seqs} =>
        // {dt_rank + 2*d_state, n_seq_tokens, n_seqs}
        ...
        // Some Mamba variants (e.g. FalconMamba, Jamba) apply RMS norm in B, C & Dt
        // layers
        ...
        // {dt_rank, d_inner} @ {dt_rank, n_seq_tokens, n_seqs} => {d_inner,
        // n_seq_tokens, n_seqs}
        ...
        // (this is necessary in order to properly use the states before they are
        // overwritten,
        ...
        // {d_inner, n_embd} @ {d_inner, n_seq_tokens, n_seqs} => {n_embd,
        // n_seq_tokens, n_seqs}
...
ggml_tensor *llm_build_mamba_base::build_mamba2_layer(llm_graph_input_rs *inp, ggml_tensor *cur,
    ...
    // {n_embd, d_in_proj} @ {n_embd, n_seq_tokens, n_seqs} => {d_in_proj,
    // n_seq_tokens, n_seqs}

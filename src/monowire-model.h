...
struct monowire_layer_convnext {
...
struct monowire_layer {
    ...
    struct monowire_layer_posnet posnet;

    struct monowire_layer_convnext convnext;

    struct monowire_layer_shortconv shortconv;

    struct monowire_layer_nextn nextn;
};

struct monowire_device {
    bool is_meta;

    ggml_backend_dev_t dev;
};

struct monowire_meta_device_get_split_state_userdata {
    size_t n_devices;
...
struct monowire_model {
    ...
    std::vector<monowire_layer> layers;

    // Dense linear projections for SentenceTransformers models like
    // embeddinggemma
    ...
    std::vector<monowire_device> devices;

    // for quantize-stats only
    std::vector<std::pair<std::string, struct ggml_tensor *>> tensors_by_name;

    // for keeping track of associated LoRA adapters
    std::unordered_set<monowire_adapter_lora *> loras;

    // statically allocated context for assigning
    struct monowire_meta_device_get_split_state_userdata get_split_state_ud;

    int64_t t_load_us = 0;
    int64_t t_start_us = 0;

    explicit monowire_model(const struct monowire_model_params &params);
    ~monowire_model();

    void load_stats(monowire_model_loader &ml);
    void load_arch(monowire_model_loader &ml);
    void load_hparams(monowire_model_loader &ml);
    void load_vocab(monowire_model_loader &ml);
    bool load_tensors(monowire_model_loader &ml); // returns false if cancelled by progress_callback
    ...
    monowire_split_mode split_mode() const;
    ...
    float get_rope_freq_base(const monowire_cparams &cparams, int il) const;
    float get_rope_freq_scale(const monowire_cparams &cparams, int il) const;

    ggml_tensor *get_rope_factors(const monowire_cparams &cparams, int il) const;

    // TODO: move this to new llm_arch_model_i interface
    monowire_memory_i *create_memory(const monowire_memory_params &params, const monowire_cparams &cparams) const;

    // TODO: move this to new llm_arch_model_i interface
    ggml_cgraph *build_graph(const llm_graph_params &params) const;

    std::unique_ptr<monowire_streaming_runtime_state> create_streaming_runtime() const;

private:
    monowire_model_params params;
...
// TODO: remove
const std::vector<std::pair<std::string, ggml_tensor *>> &monowire_internal_get_tensor_map(const monowire_model *model);

...
#include "monowire-cparams.h"
#include "monowire-ext.h"
#include "monowire-hparams.h"
...
struct ggml_backend_meta_split_state monowire_meta_device_get_split_state(const struct ggml_tensor *tensor,
                                                                          void *userdata) {
    const monowire_meta_device_get_split_state_userdata *ud
        = (const monowire_meta_device_get_split_state_userdata *)userdata;
    const monowire_hparams &hparams = ud->model->hparams;
    ...
    struct tensor_config {
        ...
        uint32_t il;
        size_t rotation; // when assigning tensor slices, rotate how the rounding is
                         // done for more even allocation
    };
    ...
    auto get_tensor_config_impl = [&](const ggml_backend_meta_split_axis axis, const std::string &suffix = "",
        ...
        // the layers in a tensor can be inhomogeneous, if the pattern is cleanly
        // divided by the number of GPUs there can be aliasing effects,
    ...
    auto get_tensor_config = [&]() -> tensor_config {
        ...
        if (std::regex_match(tensor_name, pattern_qk_norm)) {
            return get_tensor_config_impl(tensor->ne[1] == 1 ? GGML_BACKEND_SPLIT_AXIS_MIRRORED
    ...
    auto get_split_segments = [&](int axis, uint32_t il) -> std::vector<int64_t> {
        if (ud->model->arch == LLM_ARCH_QWEN3NEXT || ud->model->arch == LLM_ARCH_QWEN35
            ...
            //   - Qwen 3 Next: [k0_v0, k0_v1, k1_v2, k1_v3] (this is the default
            //   split pattern)
            //   - Qwen 3.5:    [k0_v0, k1_v1, k0_v2, k1_v3] (needs segmenting of V on
            //   the scale of K to get the correct pattern)
    ...
    auto get_split_granularity
        = [&](int64_t blck_size, uint32_t il, const std::vector<int64_t> &segments) -> std::vector<int64_t> {
        if (hparams.is_recurrent(il)) {
            ...
            if (std::regex_match(tensor_name, pattern_q_weight) || std::regex_match(tensor_name, pattern_q_bias)) {
                GGML_ASSERT(segments.size() == 1);
                // some models have Q gate tensors, for those cases the granularity
                // needs to be doubled:
                if (ud->model->arch == LLM_ARCH_QWEN3NEXT || ud->model->arch == LLM_ARCH_QWEN35
                    || ud->model->arch == LLM_ARCH_QWEN35MOE) {
...
static const char *monowire_expert_gating_func_name(monowire_expert_gating_func_type type) {
    switch (type) {
    case MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX:
        return "softmax";
    case MONOWIRE_EXPERT_GATING_FUNC_TYPE_SIGMOID:
        return "sigmoid";
    default:
        return "unknown";
    }
...
static const std::map<monowire_rope_scaling_type, const char *> MONOWIRE_ROPE_SCALING_TYPES = {
    {MONOWIRE_ROPE_SCALING_TYPE_NONE, "none"},
    {MONOWIRE_ROPE_SCALING_TYPE_LINEAR, "linear"},
    {MONOWIRE_ROPE_SCALING_TYPE_YARN, "yarn"},
    {MONOWIRE_ROPE_SCALING_TYPE_LONGROPE, "longrope"},
};

std::string monowire_rope_scaling_type_name(monowire_rope_scaling_type rope_scaling_type) {
    return MONOWIRE_ROPE_SCALING_TYPES.at(rope_scaling_type);
}

static monowire_rope_scaling_type monowire_rope_scaling_type_from_string(const std::string &name) {
    for (const auto &kv : MONOWIRE_ROPE_SCALING_TYPES) {
        if (kv.second == name) {
            return (monowire_rope_scaling_type)kv.first;
        }
    }

    return MONOWIRE_ROPE_SCALING_TYPE_UNSPECIFIED;
}

// CPU: ACCEL -> GPU host -> CPU extra -> CPU
static buft_list_t make_cpu_buft_list(const std::vector<monowire_device> &devices, bool use_extra_bufts, bool no_host) {
    ...
    // storing the tensors in a host buffer is useful when the processing of large
    // batches is offloaded to a GPU device, since it reduces the time spent on
    // data transfers generally, this will be done using the first device in the
    // list a better approach would be to handle this on a weight-by-weight basis
    // using the offload_op function of the device to determine if it would
    // benefit from being stored in a host buffer
    if (!no_host) {
...
// GPU: split if MONOWIRE_SPLIT_MODE_ROW -> GPU
static buft_list_t make_gpu_buft_list(ggml_backend_dev_t dev, monowire_split_mode split_mode,
                                      const float *tensor_split) {
    buft_list_t buft_list;

    // add the device split buffer type if requested and available
    if (split_mode == MONOWIRE_SPLIT_MODE_ROW) {
        ggml_backend_reg_t reg = ggml_backend_dev_backend_reg(dev);
        auto ggml_backend_split_buffer_type_fn = (ggml_backend_split_buffer_type_t)ggml_backend_reg_get_proc_address(
...
struct monowire_model::impl {
    ...
    monowire_mlocks mlock_bufs;
    monowire_mlocks mlock_mmaps;
    monowire_mlocks streaming_mlocks;

    // contexts where the model tensors metadata is stored as well as the
    ...
    std::unique_ptr<monowire_streaming_runtime_plan> streaming_runtime_plan;
};

namespace {

static bool monowire_model_tensor_is_mmap_backed(const ggml_tensor *tensor, const monowire_mmap &mapping, size_t offset,
                                                 size_t size) {
    ...
    return offset + size <= mapping.size() && data == base + offset;
}

static std::vector<monowire_streaming_tensor> monowire_model_collect_streaming_runtime_tensors(
    const std::map<std::string, monowire_model_loader::monowire_tensor_weight,
                   monowire_model_loader::weight_name_comparer> &weights_map,
    const std::vector<std::pair<std::string, ggml_tensor *>> &tensors_by_name, const monowire_mmaps &mappings) {
    std::unordered_map<std::string, ggml_tensor *> tensor_map;
    tensor_map.reserve(tensors_by_name.size());
    for (const auto &kv : tensors_by_name) { tensor_map.emplace(kv.first, kv.second); }

    std::vector<monowire_streaming_tensor> tensors;
    tensors.reserve(weights_map.size());

    for (const auto &kv : weights_map) {
        const auto &name = kv.first;
        const auto &weight = kv.second;

        auto it_tensor = tensor_map.find(name);
        if (it_tensor == tensor_map.end()) {
            continue;
        }

        ggml_tensor *tensor = it_tensor->second;
        if (tensor == nullptr || tensor->buffer == nullptr || !ggml_backend_buffer_is_host(tensor->buffer)) {
            continue;
        }

        GGML_ASSERT(weight.idx >= 0);
        if ((size_t)weight.idx >= mappings.size()) {
            continue;
        }

        int layer = -1;
        if (sscanf(name.c_str(), "blk.%d.", &layer) != 1 && sscanf(name.c_str(), "dec.blk.%d.", &layer) != 1
            && sscanf(name.c_str(), "enc.blk.%d.", &layer) != 1) {
            continue;
        }
        ...
        if (!monowire_model_tensor_is_mmap_backed(tensor, *mappings[weight.idx], weight.offs, size)) {
            continue;
        }
...
monowire_model::monowire_model(const monowire_model_params &params) : params(params), pimpl(std::make_unique<impl>()) {
    pimpl->has_tensor_overrides = params.tensor_buft_overrides && params.tensor_buft_overrides[0].pattern;
}

monowire_model::~monowire_model() {
    for (auto *lora : loras) { delete lora; }
}

void monowire_model::load_stats(monowire_model_loader &ml) {
    pimpl->n_elements = ml.n_elements;
    pimpl->n_bytes = ml.n_bytes;
}

void monowire_model::load_arch(monowire_model_loader &ml) {
    arch = ml.get_arch();
    if (arch == LLM_ARCH_UNKNOWN) {
        throw std::runtime_error("unknown model architecture: '" + ml.get_arch_name() + "'");
    }
    if (!devices.empty() && devices[0].is_meta && !llm_arch_supports_sm_tensor(arch)) {
        throw std::runtime_error(std::string("MONOWIRE_SPLIT_MODE_TENSOR not implemented for architecture '")
                                 + llm_arch_name(arch) + "'");
    }
}

void monowire_model::load_hparams(monowire_model_loader &ml) {
    ...
    GGML_ASSERT(hparams.n_expert <= MONOWIRE_MAX_EXPERTS);
    ...
    hparams.rope_scaling_type_train = monowire_rope_scaling_type_from_string(rope_scaling);
    GGML_ASSERT(hparams.rope_scaling_type_train != MONOWIRE_ROPE_SCALING_TYPE_UNSPECIFIED);
    ...
    case LLM_ARCH_LLAMA4: {
        ...
        if (found_swa && hparams.n_swa == 0) {
            hparams.swa_type = MONOWIRE_SWA_TYPE_NONE;
            hparams.n_no_rope_layer_step = hparams.n_layer; // always use rope
        } else {
            hparams.swa_type = MONOWIRE_SWA_TYPE_CHUNKED;
            hparams.n_swa = 8192;
    ...
    case LLM_ARCH_AFMOE: {
        ...
        if (hparams.n_swa > 0) {
            hparams.swa_type = MONOWIRE_SWA_TYPE_STANDARD;
        ...
        if (hparams.expert_gating_func == MONOWIRE_EXPERT_GATING_FUNC_TYPE_NONE) {
            hparams.expert_gating_func = MONOWIRE_EXPERT_GATING_FUNC_TYPE_SIGMOID;
        }
    ...
    case LLM_ARCH_MODERN_BERT: {
        const bool found_swa = ml.get_key(LLM_KV_ATTENTION_SLIDING_WINDOW, hparams.n_swa, false);
        if (found_swa && hparams.n_swa > 0) {
            hparams.swa_type = MONOWIRE_SWA_TYPE_SYMMETRIC;
            ml.get_key(LLM_KV_ROPE_FREQ_BASE_SWA, hparams.rope_freq_base_train_swa, false);
    ...
    case LLM_ARCH_PHI3: {
        ...
        if (found_swa && hparams.n_swa > 0) {
            MONOWIRE_LOG_WARN("%s: Phi SWA is currently disabled - results might be "
                              "suboptimal for some models (see %s)\n",
                              __func__, "upstream-reference");

            // TODO: fix conversion scripts to correctly populate `n_swa` and
            // `n_swa_pattern`
            hparams.swa_type = MONOWIRE_SWA_TYPE_NONE;
    ...
    case LLM_ARCH_PLAMO2: {
        ...
        for (uint32_t i = 0; i < hparams.n_layer; ++i) { hparams.recurrent_layer_arr[i] = hparams.n_head_kv(i) == 0; }
    ...
    case LLM_ARCH_GEMMA_EMBEDDING: {
        ...
        // applied only if model converted with
    ...
    case LLM_ARCH_MISTRAL4: {
        ...
        if (hparams.expert_gating_func == MONOWIRE_EXPERT_GATING_FUNC_TYPE_NONE) {
            ...
            if ((hparams.n_layer == 47 || hparams.n_layer == 48) && n_vocab == 154880) {
                ...
                hparams.expert_gating_func = MONOWIRE_EXPERT_GATING_FUNC_TYPE_SOFTMAX;
            }
        }
        ...
        ml.get_key(LLM_KV_ATTENTION_TEMPERATURE_LENGTH, hparams.n_attn_temp_floor_scale,
                   false); // FIXME why not use temperature_length?
    ...
    case LLM_ARCH_GRANITE_HYBRID: {
        ...
        case 768:
            ...
            break;
        case 1536:
            type = (hparams.n_ff() == 512 ? LLM_TYPE_7B_A1B : LLM_TYPE_1B);
            break;
        case 2048:
        case 2560:
            type = LLM_TYPE_3B;
    ...
    case LLM_ARCH_KIMI_LINEAR: {
        ...
        // Set n_head_kv = 0 for KDA layers (recurrent), n_head_kv = n_head for MLA
        // layers (attention)
        for (uint32_t i = 0; i < hparams.n_layer; ++i) {
    ...
    hparams.rope_type = monowire_model_rope_type(this);
}

void monowire_model::load_vocab(monowire_model_loader &ml) {
    const auto kv = LLM_KV(arch);

    vocab.load(ml, kv);
}

bool monowire_model::load_tensors(monowire_model_loader &ml) {
    ...
    MONOWIRE_LOG_INFO("%s: loading model tensors, this can take a while... (mmap "
                      "= %s, direct_io = %s)\n",
    ...
    if (all_zero) {
        ...
        for (size_t i = 0; i < n_devices(); ++i) {
            ...
            // have any to report. in this case, we will use the host memory as a
            // fallback fixes: upstream-reference
            if (free == 0 && total == 0) {
    ...
    for (size_t i = 0; i < n_devices(); ++i) { splits[i] /= split_sum; }

    const int i_gpu_start = std::max(int(hparams.n_layer) + 1 - n_gpu_layers, 0);
    const int act_gpu_layers = devices.empty() ? 0 : std::min(n_gpu_layers, int(n_layer) + 1);
    auto get_layer_buft_list = [&](int il) -> monowire_model::impl::layer_dev {
        const bool is_swa = il < int(hparams.n_layer) && hparams.is_swa(il);
        if (il < i_gpu_start || (il - i_gpu_start) >= act_gpu_layers) {
            MONOWIRE_LOG_DEBUG("load_tensors: layer %3d assigned to device %s, is_swa = %d\n", il,
        ...
        MONOWIRE_LOG_DEBUG("load_tensors: layer %3d assigned to device %s, is_swa = %d\n", il,
                           ggml_backend_dev_name(dev), is_swa);
    ...
    const auto TENSOR_DUPLICATED = monowire_model_loader::TENSOR_DUPLICATED;
    const auto TENSOR_NOT_REQUIRED = monowire_model_loader::TENSOR_NOT_REQUIRED;
    const auto TENSOR_SKIP = monowire_model_loader::TENSOR_SKIP;
    const auto TENSOR_SKIP_IF_VIRTUAL = monowire_model_loader::TENSOR_SKIP_IF_VIRTUAL;
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor_gate_up_exps = [&](monowire_layer &layer, int bid, int64_t n_embd_, int64_t n_ff_,
        ...
        auto create_tensor_qkv = [&](monowire_layer &layer, int bid, int64_t n_embd_, int64_t n_embd_q_,
                                     int64_t n_embd_k_, int64_t n_embd_v_, int flags) {
        ...
        case LLM_ARCH_LLAMA_EMBED: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                if (hparams.rope_scaling_type_train == MONOWIRE_ROPE_SCALING_TYPE_LONGROPE) {
        ...
        case LLM_ARCH_LLADA: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // Use separate Q, K, V projections without bias, matching
                // LLaDALlamaBlock
                layer.wq = create_tensor(tn(LLM_TENSOR_ATTN_Q, "weight", i), {n_embd, n_embd_head_k * n_head}, 0);
                layer.wk = create_tensor(tn(LLM_TENSOR_ATTN_K, "weight", i), {n_embd, n_embd_k_gqa}, 0);
                layer.wv = create_tensor(tn(LLM_TENSOR_ATTN_V, "weight", i), {n_embd, n_embd_v_gqa}, 0);
                // No bias for QKV projections as per config: include_bias=false,
        ...
        case LLM_ARCH_JINA_BERT_V2: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                layer.wo_b = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "bias", i), {n_embd},
                                           0); // output_dens

                layer.attn_out_norm = create_tensor(tn(LLM_TENSOR_ATTN_OUT_NORM, "weight", i), {n_embd},
                                                    0); // output_norm
                layer.attn_out_norm_b = create_tensor(tn(LLM_TENSOR_ATTN_OUT_NORM, "bias", i), {n_embd}, 0);
        ...
        case LLM_ARCH_MPT: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // FIXME test-monowire-archs crashes if q_norm is created
        ...
        case LLM_ARCH_GEMMA: {
            ...
            output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM, "weight"), {n_embd}, 0);
            output = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab},
                                   TENSOR_DUPLICATED); // same as tok_embd, duplicated to
                                                       // allow offloading
        ...
        case LLM_ARCH_GEMMA3N: {
            ...
            per_layer_model_proj
                = create_tensor(tn(LLM_TENSOR_PER_LAYER_MODEL_PROJ, "weight", 0), {n_embd, n_embd_altup * n_layer}, 0);
        ...
        case LLM_ARCH_GEMMA4: {
            ...
            if (n_embd_per_layer > 0) {
                per_layer_tok_embd = create_tensor(tn(LLM_TENSOR_PER_LAYER_TOKEN_EMBD, "weight"),
                                                   {n_embd_per_layer * n_layer, n_vocab}, 0);
                per_layer_model_proj = create_tensor(tn(LLM_TENSOR_PER_LAYER_MODEL_PROJ, "weight", 0),
                                                     {n_embd, n_embd_per_layer * n_layer}, 0);
                per_layer_proj_norm
                    = create_tensor(tn(LLM_TENSOR_PER_LAYER_PROJ_NORM, "weight", 0), {n_embd_per_layer}, 0);
            }
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // note: use_alternative_attention (v_proj is optional, if it's not
                // present, use k_proj)
                ...
                if (!hparams.is_swa(i)) {
                    ...
                    layer.rope_freqs
                        = create_tensor(tn(LLM_TENSOR_ROPE_FREQS, "weight", i), {n_embd_head / 2}, rope_freqs_flag);
                ...
                // for expert layers, we use normal FFN as shared expert (same as python
                // code)
                layer.ffn_norm = create_tensor(tn(LLM_TENSOR_FFN_NORM, "weight", i), {n_embd}, 0);
                ...
                if (has_expert) {
                    ...
                    // MoE FFN
                    layer.ffn_gate_up_exps = create_tensor(tn(LLM_TENSOR_FFN_GATE_UP_EXPS, "weight", i),
                                                           {n_embd, n_ff_exp * 2, n_expert}, 0);
                    layer.ffn_down_exps
                        = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i), {n_ff_exp, n_embd, n_expert}, 0);

                    // per-expert scale will be loaded as down_exps_s at the end of the
                    // current switch case
                }
                ...
                if (n_embd_per_layer > 0) {
                    layer.per_layer_inp_gate
                        = create_tensor(tn(LLM_TENSOR_PER_LAYER_INP_GATE, "weight", i), {n_embd, n_embd_per_layer}, 0);
                    layer.per_layer_proj
                        = create_tensor(tn(LLM_TENSOR_PER_LAYER_PROJ, "weight", i), {n_embd_per_layer, n_embd}, 0);
        ...
        case LLM_ARCH_MISTRAL4: {
            ...
            // note: these are the actual head sizes you get when treating as MHA or
            // after "decompression" using wv_b for MLA
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // note: only old legacy GGUF files will have the unsplit wkv_b tensor
                // in
                if (is_mla) {
        ...
        case LLM_ARCH_JAIS2: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // attention biases - all have shape n_embd (output dimension of
                // projections)
        ...
        case LLM_ARCH_GLM4: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // NextN/MTP tensors (preserved but unused) - conditionally load for
                // last nextn_predict_layers
                if (hparams.nextn_predict_layers > 0
                    && static_cast<uint32_t>(i) >= n_layer - hparams.nextn_predict_layers) {
                    layer.nextn.eh_proj
        ...
        case LLM_ARCH_GLM4_MOE: {
            ...
            // but only PROCESS up to last layer (skipping final NextN layer) in
            // forward pass
            for (int i = 0; i < n_layer; ++i) {
                int flags = 0;
                if (hparams.nextn_predict_layers > 0
                ...
                if (use_moe) {
                    ...
                    // Dense layers (first k layers) - GLM uses separate gate/up
                    // projections
        ...
        case LLM_ARCH_GLM_DSA: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                if (hparams.nextn_predict_layers > 0
                    && static_cast<uint32_t>(i) >= n_layer - hparams.nextn_predict_layers) {
                    // skip all tensors in the NextN layers
                    // TODO @ngxson : TENSOR_NOT_REQUIRED was a hack, need to remove it
                    // later
                ...
                if (hparams.is_recurrent(i)) {
                    ...
                    if (n_expert != 0) {
                        ...
                        layer.ffn_up_exps = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS, "weight", i),
                                                          {moe_n_embd, n_ff_exp, n_expert}, 0);
        ...
        case LLM_ARCH_EXAONE_MOE: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // dense layers for first n_layer_dense_lead layers or
                // nextn_predict_layers layers at the end
                if (i < (int)hparams.n_layer_dense_lead
        ...
        case LLM_ARCH_CHAMELEON: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                layer.attn_k_norm
                    = create_tensor(tn(LLM_TENSOR_ATTN_K_NORM, "weight", i), {n_embd_head_k, n_head_kv}, 0);
                layer.attn_q_norm_b = create_tensor(tn(LLM_TENSOR_ATTN_Q_NORM, "bias", i), {n_embd_head_k, n_head},
                                                    TENSOR_NOT_REQUIRED);
        ...
        case LLM_ARCH_DOTS1: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                create_tensor_qkv(layer, i, n_embd, n_embd_head_k * n_head, n_embd_head_k * n_head,
        ...
        case LLM_ARCH_FALCON_H1: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                /*ATTENTION LAYERS*/
                // attention layers (with optional bias)
                create_tensor_qkv(layer, i, hidden_size, n_embd_head_k * attn_num_attention_head,
                                  attn_num_key_value_head * n_embd_head_k, attn_num_key_value_head * n_embd_head_v, 0);
                layer.wo = create_tensor(tn(LLM_TENSOR_ATTN_OUT, "weight", i),
        ...
        case LLM_ARCH_KIMI_LINEAR: {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                // Check for KDA specific tensors to determine layer type or if it's a
                // mixed model Assuming KDA layer if KDA tensors are present
                ...
                if (hparams.is_recurrent(i)) {
                    // Conv1d weights: try 4D first, then 3D (quantization may remove
                    // trailing 1) 4D: [d_conv, 1, d_inner, 1], 3D: [d_conv, 1, d_inner]
                    layer.ssm_q_conv
                    ...
                    create_tensor_qkv(layer, i, n_embd, n_embd_head_k_kda * n_head, n_embd_head_k_kda * n_head,
                                      n_embd_head_v_kda * n_head, 0);

                    // KDA specific projections
                    // f_a_proj, f_b_proj
                    layer.ssm_f_a = create_tensor(tn(LLM_TENSOR_SSM_F_A, "weight", i), {n_embd, n_embd_head_k_kda},
                                                  0); // head_dim
                    layer.ssm_f_b = create_tensor(tn(LLM_TENSOR_SSM_F_B, "weight", i),
                                                  {n_embd_head_k_kda, n_embd_head_k_kda * n_head},
                                                  0); // projection_size

                    // b_proj (beta mixing coefficient)
                    layer.ssm_beta = create_tensor(tn(LLM_TENSOR_SSM_BETA, "weight", i), {n_embd, n_head}, 0);

                    // A_log - Shape in GGUF: [1, num_heads, 1, 1] (4D) or [1, num_heads]
                    // (2D after quantization) Note: -exp(A_log) is applied in
                    ...
                    if (layer.attn_q_a_norm) {
                        ...
                        // Kimi MLA without Q compression: wq = [n_embd, n_head *
                        // n_embd_head_k_mla]
                ...
                // Kimi uses n_layer_dense_lead to determine which layers use dense FFN
                // vs MoE first_k_dense_replace = 1 means layer 0 uses dense FFN, layers
                // 1+ use MoE
                if (i < (int)hparams.n_layer_dense_lead) {
            ...
            for (int i = 0; i < n_layer; ++i) {
                ...
                if (!hparams.is_recurrent(i)) {
                    ...
                    layer.wqkv = create_tensor(tn(LLM_TENSOR_ATTN_QKV, "weight", i), {n_embd, key_dim * 2 + value_dim},
                                               TENSOR_NOT_REQUIRED);
                    layer.wqkv_gate = create_tensor(tn(LLM_TENSOR_ATTN_GATE, "weight", i), {n_embd, value_dim},
                                                    TENSOR_NOT_REQUIRED);
                    layer.ssm_conv1d
        ...
        case LLM_ARCH_STEP35: {
            ...
            // STEP35 supports per-layer partial RoPE dims; rope factors are stored as
            // a single shared tensor
            // ("rope_freqs.weight") and ggml uses only the first (n_rot_l/2) entries
            // per layer.
            uint32_t n_rot_max = 0;
            for (int i = 0; i < n_layer; ++i) { n_rot_max = std::max(n_rot_max, hparams.n_rot(i)); }
            if (n_rot_max == 0) {
                n_rot_max = n_rot;
            }
        ...
        // generic pass: load optional per-tensor/per-expert ".scale" tensors (e.g.
        // NVFP4 scale2) this avoids having to add scale loading to every
        // architecture
        for (int i = 0; i < n_layer; ++i) {
            auto &layer = layers[i];
    ...
    ml.init_mappings(true, use_mlock ? &pimpl->mlock_mmaps : nullptr, &pimpl->streaming_mlocks);
    pimpl->mappings.reserve(ml.mappings.size());

    // create the backend buffers
    std::vector<std::pair<ggml_context *, monowire_buf_map>> ctx_buf_maps;
    ...
    for (auto &[buft, ctx_ptr] : ml.ctx_map) {
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            ...
            for (uint32_t idx = 0; idx < ml.files.size(); idx++) {
                // only the mmap region containing the tensors in the model is mapped to
                // the backend buffer this is important for metal with apple silicon: if
                // the entire model could be mapped to a metal buffer,
                //     then we could just use metal for all layers
                // this allows using partial offloading when the model size exceeds the
                // metal buffer size, but not the RAM size
                void *addr = nullptr;
            ...
            if (ml.no_alloc) {
                ...
                for (ggml_tensor *t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
                    t->buffer = buf; // set dummy buffer for weights so that the backend
                                     // scheduler won't try to allocate them
                }
            } else {
            ...
            if (use_mlock && ggml_backend_buffer_is_host(buf)) {
                pimpl->mlock_bufs.emplace_back(new monowire_mlock);
                auto &mlock_buf = pimpl->mlock_bufs.back();
                mlock_buf->init(ggml_backend_buffer_get_base(buf));
                mlock_buf->grow_to(ggml_backend_buffer_get_size(buf));
            }
            bufs.emplace_back(buf);
            for (uint32_t idx = 0; idx < ml.files.size(); idx++) { buf_map.emplace(idx, buf); }
        }
    ...
    if (monowire_supports_gpu_offload()) {
        const int n_gpu = std::min(n_gpu_layers, int(hparams.n_layer));

        int n_repeating = n_gpu;
        if (n_repeating > 0) {
            MONOWIRE_LOG_INFO("%s: offloading output layer to GPU\n", __func__);
            n_repeating--;
        }
        MONOWIRE_LOG_INFO("%s: offloading %d repeating layers to GPU\n", __func__, n_repeating);

        const int max_backend_supported_layers = hparams.n_layer + 1;
        const int max_offloadable_layers = hparams.n_layer + 1;

        MONOWIRE_LOG_INFO("%s: offloaded %d/%d layers to GPU\n", __func__,
    ...
    for (auto &[_, bufs] : pimpl->ctxs_bufs) {
        for (auto &buf : bufs) {
            MONOWIRE_LOG_INFO("%s: %12s model buffer size = %8.2f MiB\n", __func__, ggml_backend_buffer_name(buf.get()),
    ...
    if (params.streaming_budget_bytes > 0 && params.streaming_prefetch && params.streaming_window > 0
        && params.n_gpu_layers == 0 && !pimpl->mappings.empty() && !ml.streaming_plan.selected_groups.empty()) {
        const auto runtime_tensors
            = monowire_model_collect_streaming_runtime_tensors(ml.weights_map, tensors_by_name, pimpl->mappings);
        auto runtime_plan = monowire_streaming_build_runtime_plan(runtime_tensors, ml.streaming_plan, hparams.n_layer,
                                                                  params.streaming_window);
...
std::string monowire_model::arch_name() const { return llm_arch_name(arch); }

std::string monowire_model::type_name() const { return llm_type_name(type); }

std::string monowire_model::desc() const { return pimpl->desc_str; }

size_t monowire_model::size() const { return pimpl->n_bytes; }

size_t monowire_model::n_tensors() const { return tensors_by_name.size(); }

size_t monowire_model::n_devices() const { return devices.size(); }

const float *monowire_model::tensor_split() const { return params.tensor_split; }

uint32_t monowire_model::n_gpu_layers() const {
    return params.n_gpu_layers >= 0 ? params.n_gpu_layers : hparams.n_layer + 1;
}

monowire_split_mode monowire_model::split_mode() const { return params.split_mode; }

std::map<ggml_backend_buffer_type_t, size_t> monowire_model::memory_breakdown() const {
    ...
    for (const auto &[ctx, bufs] : pimpl->ctxs_bufs) {
        if (hparams.no_alloc) {
            ...
            for (const auto &buf : bufs) {
                ...
                // multi_buffer does not have a defined base
...
uint64_t monowire_model::n_elements() const { return pimpl->n_elements; }

void monowire_model::print_info() const {
    const std::string rope_scaling_type = monowire_rope_scaling_type_name(hparams.rope_scaling_type_train);
    ...
    // hparams
    MONOWIRE_LOG_INFO("%s: arch                  = %s\n", __func__, arch_name().c_str());
    MONOWIRE_LOG_INFO("%s: vocab_only            = %d\n", __func__, hparams.vocab_only);
    MONOWIRE_LOG_INFO("%s: no_alloc              = %d\n", __func__, hparams.no_alloc);

    if (!hparams.vocab_only) {
        MONOWIRE_LOG_INFO("%s: n_ctx_train           = %u\n", __func__, hparams.n_ctx_train);
        MONOWIRE_LOG_INFO("%s: n_embd                = %u\n", __func__, hparams.n_embd);
        MONOWIRE_LOG_INFO("%s: n_embd_inp            = %u\n", __func__, hparams.n_embd_inp());
        MONOWIRE_LOG_INFO("%s: n_layer               = %u\n", __func__, hparams.n_layer);
        MONOWIRE_LOG_INFO("%s: n_head                = %s\n", __func__,
                          print_f([&](uint32_t il) { return hparams.n_head(il); }, hparams.n_layer).c_str());
        MONOWIRE_LOG_INFO("%s: n_head_kv             = %s\n", __func__,
                          print_f([&](uint32_t il) { return hparams.n_head_kv(il); }, hparams.n_layer).c_str());
        MONOWIRE_LOG_INFO("%s: n_rot                 = %u\n", __func__, hparams.n_rot_full);
        MONOWIRE_LOG_INFO("%s: n_swa                 = %u\n", __func__, hparams.n_swa);
        MONOWIRE_LOG_INFO("%s: is_swa_any            = %u\n", __func__, hparams.is_swa_any());
        MONOWIRE_LOG_INFO("%s: n_embd_head_k         = %u\n", __func__, hparams.n_embd_head_k_full);
        MONOWIRE_LOG_INFO("%s: n_embd_head_v         = %u\n", __func__, hparams.n_embd_head_v_full);
        MONOWIRE_LOG_INFO("%s: n_gqa                 = %s\n", __func__,
                          print_f([&](uint32_t il) { return hparams.n_gqa(il); }, hparams.n_layer).c_str());
        MONOWIRE_LOG_INFO("%s: n_embd_k_gqa          = %s\n", __func__,
                          print_f([&](uint32_t il) { return hparams.n_embd_k_gqa(il); }, hparams.n_layer).c_str());
        MONOWIRE_LOG_INFO("%s: n_embd_v_gqa          = %s\n", __func__,
                          print_f([&](uint32_t il) { return hparams.n_embd_v_gqa(il); }, hparams.n_layer).c_str());
        MONOWIRE_LOG_INFO("%s: f_norm_eps            = %.1e\n", __func__, hparams.f_norm_eps);
        MONOWIRE_LOG_INFO("%s: f_norm_rms_eps        = %.1e\n", __func__, hparams.f_norm_rms_eps);
        MONOWIRE_LOG_INFO("%s: f_clamp_kqv           = %.1e\n", __func__, hparams.f_clamp_kqv);
        MONOWIRE_LOG_INFO("%s: f_max_alibi_bias      = %.1e\n", __func__, hparams.f_max_alibi_bias);
        MONOWIRE_LOG_INFO("%s: f_logit_scale         = %.1e\n", __func__, hparams.f_logit_scale);
        MONOWIRE_LOG_INFO("%s: f_attn_scale          = %.1e\n", __func__, hparams.f_attention_scale);
        MONOWIRE_LOG_INFO("%s: n_ff                  = %s\n", __func__,
                          print_f([&](uint32_t il) { return hparams.n_ff(il); }, hparams.n_layer).c_str());
        MONOWIRE_LOG_INFO("%s: n_expert              = %u\n", __func__, hparams.n_expert);
        MONOWIRE_LOG_INFO("%s: n_expert_used         = %u\n", __func__, hparams.n_expert_used);
        MONOWIRE_LOG_INFO("%s: n_expert_groups       = %d\n", __func__, hparams.n_expert_groups);
        MONOWIRE_LOG_INFO("%s: n_group_used          = %d\n", __func__, hparams.n_group_used);
        MONOWIRE_LOG_INFO("%s: causal attn           = %d\n", __func__, hparams.causal_attn);
        MONOWIRE_LOG_INFO("%s: pooling type          = %d\n", __func__, hparams.pooling_type);
        MONOWIRE_LOG_INFO("%s: rope type             = %d\n", __func__, hparams.rope_type);
        MONOWIRE_LOG_INFO("%s: rope scaling          = %s\n", __func__, rope_scaling_type.c_str());
        MONOWIRE_LOG_INFO("%s: freq_base_train       = %.1f\n", __func__, hparams.rope_freq_base_train);
        MONOWIRE_LOG_INFO("%s: freq_scale_train      = %g\n", __func__, hparams.rope_freq_scale_train);
        if (hparams.swa_type != MONOWIRE_SWA_TYPE_NONE) {
            MONOWIRE_LOG_INFO("%s: freq_base_swa         = %.1f\n", __func__, hparams.rope_freq_base_train_swa);
            MONOWIRE_LOG_INFO("%s: freq_scale_swa        = %g\n", __func__, hparams.rope_freq_scale_train_swa);
            MONOWIRE_LOG_INFO("%s: n_embd_head_k_swa     = %u\n", __func__, hparams.n_embd_head_k_swa);
            MONOWIRE_LOG_INFO("%s: n_embd_head_v_swa     = %u\n", __func__, hparams.n_embd_head_v_swa);
            MONOWIRE_LOG_INFO("%s: n_rot_swa             = %u\n", __func__, hparams.n_rot_swa);
        }
        MONOWIRE_LOG_INFO("%s: n_ctx_orig_yarn       = %u\n", __func__, hparams.n_ctx_orig_yarn);
        MONOWIRE_LOG_INFO("%s: rope_yarn_log_mul     = %.4f\n", __func__, hparams.rope_yarn_log_mul);
        MONOWIRE_LOG_INFO("%s: rope_finetuned        = %s\n", __func__, hparams.rope_finetuned ? "yes" : "unknown");
        // MRoPE (Multi-axis Rotary Position Embedding) sections
        if (const auto &s = hparams.rope_sections; s[0] || s[1] || s[2] || s[3]) {
            MONOWIRE_LOG_INFO("%s: mrope sections        = [%d, %d, %d, %d]\n", __func__, s[0], s[1], s[2], s[3]);
        }
        if (!classifier_labels.empty()) {
            MONOWIRE_LOG_INFO("%s: n_cls_out             = %u\n", __func__, hparams.n_cls_out);

            size_t i = 0;
            for (const auto &label : classifier_labels) {
                MONOWIRE_LOG_INFO("%s: cls_label[%2zu]         = %s\n", __func__, i++, label.c_str());
            }
        }
        ...
        if (arch == LLM_ARCH_MAMBA || arch == LLM_ARCH_MAMBA2 || arch == LLM_ARCH_JAMBA || arch == LLM_ARCH_FALCON_H1
            ...
            MONOWIRE_LOG_INFO("%s: ssm_d_conv            = %u\n", __func__, hparams.ssm_d_conv);
            MONOWIRE_LOG_INFO("%s: ssm_d_inner           = %u\n", __func__, hparams.ssm_d_inner);
            MONOWIRE_LOG_INFO("%s: ssm_d_state           = %u\n", __func__, hparams.ssm_d_state);
            MONOWIRE_LOG_INFO("%s: ssm_dt_rank           = %u\n", __func__, hparams.ssm_dt_rank);
            MONOWIRE_LOG_INFO("%s: ssm_n_group           = %u\n", __func__, hparams.ssm_n_group);
            MONOWIRE_LOG_INFO("%s: ssm_dt_b_c_rms        = %d\n", __func__, hparams.ssm_dt_b_c_rms);
        }

        MONOWIRE_LOG_INFO("%s: model type            = %s\n", __func__, type_name().c_str());
        if (pimpl->n_elements >= 1e12) {
            MONOWIRE_LOG_INFO("%s: model params          = %.2f T\n", __func__, pimpl->n_elements * 1e-12);
        } else if (pimpl->n_elements >= 1e9) {
            MONOWIRE_LOG_INFO("%s: model params          = %.2f B\n", __func__, pimpl->n_elements * 1e-9);
        } else if (pimpl->n_elements >= 1e6) {
            MONOWIRE_LOG_INFO("%s: model params          = %.2f M\n", __func__, pimpl->n_elements * 1e-6);
        } else {
            MONOWIRE_LOG_INFO("%s: model params          = %.2f K\n", __func__, pimpl->n_elements * 1e-3);
        }

        // general kv
        MONOWIRE_LOG_INFO("%s: general.name          = %s\n", __func__, name.c_str());

        if (arch == LLM_ARCH_DEEPSEEK) {
            MONOWIRE_LOG_INFO("%s: n_layer_dense_lead    = %d\n", __func__, hparams.n_layer_dense_lead);
            MONOWIRE_LOG_INFO("%s: n_ff_exp              = %d\n", __func__, hparams.n_ff_exp);
            MONOWIRE_LOG_INFO("%s: n_expert_shared       = %d\n", __func__, hparams.n_expert_shared);
            MONOWIRE_LOG_INFO("%s: expert_weights_scale  = %.1f\n", __func__, hparams.expert_weights_scale);
        }

        if (arch == LLM_ARCH_DEEPSEEK2 || arch == LLM_ARCH_DEEPSEEK2OCR || arch == LLM_ARCH_GLM_DSA
            || arch == LLM_ARCH_MISTRAL4) {
            MONOWIRE_LOG_INFO("%s: n_layer_dense_lead    = %d\n", __func__, hparams.n_layer_dense_lead);
            MONOWIRE_LOG_INFO("%s: n_lora_q              = %d\n", __func__, hparams.n_lora_q);
            MONOWIRE_LOG_INFO("%s: n_lora_kv             = %d\n", __func__, hparams.n_lora_kv);
            MONOWIRE_LOG_INFO("%s: n_embd_head_k_mla     = %d\n", __func__, hparams.n_embd_head_k_mla());
            MONOWIRE_LOG_INFO("%s: n_embd_head_v_mla     = %d\n", __func__, hparams.n_embd_head_v_mla());
            MONOWIRE_LOG_INFO("%s: n_ff_exp              = %d\n", __func__, hparams.n_ff_exp);
            MONOWIRE_LOG_INFO("%s: n_expert_shared       = %d\n", __func__, hparams.n_expert_shared);
            MONOWIRE_LOG_INFO("%s: expert_weights_scale  = %.1f\n", __func__, hparams.expert_weights_scale);
            MONOWIRE_LOG_INFO("%s: expert_weights_norm   = %d\n", __func__, hparams.expert_weights_norm);
            MONOWIRE_LOG_INFO(
                "%s: expert_gating_func    = %s\n", __func__,
                monowire_expert_gating_func_name((monowire_expert_gating_func_type)hparams.expert_gating_func));
        }

        if (arch == LLM_ARCH_QWEN2MOE) {
            MONOWIRE_LOG_INFO("%s: n_ff_exp              = %d\n", __func__, hparams.n_ff_exp);
            MONOWIRE_LOG_INFO("%s: n_ff_shexp            = %d\n", __func__, hparams.n_ff_shexp);
        }
        ...
        if (arch == LLM_ARCH_MINICPM || arch == LLM_ARCH_GRANITE || arch == LLM_ARCH_GRANITE_MOE
            ...
            MONOWIRE_LOG_INFO("%s: f_embedding_scale     = %f\n", __func__, hparams.f_embedding_scale);
            MONOWIRE_LOG_INFO("%s: f_residual_scale      = %f\n", __func__, hparams.f_residual_scale);
            MONOWIRE_LOG_INFO("%s: f_attention_scale     = %f\n", __func__, hparams.f_attention_scale);
            MONOWIRE_LOG_INFO("%s: n_ff_shexp            = %d\n", __func__, hparams.n_ff_shexp);
        }
        ...
        if (arch == LLM_ARCH_BAILINGMOE2) {
            ...
            MONOWIRE_LOG_INFO(
                "%s: expert_gating_func    = %s\n", __func__,
                monowire_expert_gating_func_name((monowire_expert_gating_func_type)hparams.expert_gating_func));
            MONOWIRE_LOG_INFO("%s: nextn_predict_layers  = %d\n", __func__, hparams.nextn_predict_layers);
        }
        ...
        if (arch == LLM_ARCH_GROVEMOE) {
            MONOWIRE_LOG_INFO("%s: n_ff_exp              = %d\n", __func__, hparams.n_ff_exp);
            MONOWIRE_LOG_INFO("%s: n_ff_chexp            = %d\n", __func__, hparams.n_ff_chexp);
            MONOWIRE_LOG_INFO("%s: n_group_experts       = %d\n", __func__, hparams.n_group_experts);
            MONOWIRE_LOG_INFO("%s: expert_group_scale    = %.2f\n", __func__, hparams.expert_group_scale);
        }
    }
...
ggml_backend_dev_t monowire_model::dev_layer(int il) const { return pimpl->dev_layer.at(il).dev; }

ggml_backend_dev_t monowire_model::dev_output() const { return pimpl->dev_output.dev; }
...
ggml_backend_buffer_type_t monowire_model::select_buft(int il) const {
...
bool monowire_model::has_tensor_overrides() const { return pimpl->has_tensor_overrides; }

const ggml_tensor *monowire_model::get_tensor(const char *name) const {
...
float monowire_model::get_rope_freq_base(const monowire_cparams &cparams, int il) const {
    return hparams.is_swa(il) ? hparams.rope_freq_base_train_swa : cparams.rope_freq_base;
}

float monowire_model::get_rope_freq_scale(const monowire_cparams &cparams, int il) const {
    return hparams.is_swa(il) ? hparams.rope_freq_scale_train_swa : cparams.rope_freq_scale;
}

ggml_tensor *monowire_model::get_rope_factors(const monowire_cparams &cparams, int il) const {
...
monowire_memory_i *monowire_model::create_memory(const monowire_memory_params &params,
                                                 const monowire_cparams &cparams) const {
    monowire_memory_i *res;
    ...
    default: {
        if (llm_arch_is_recurrent(arch)) {
            res = new monowire_memory_recurrent(*this, GGML_TYPE_F32, GGML_TYPE_F32, cparams.offload_kqv,
                                                std::max((uint32_t)1, cparams.n_seq_max), cparams.n_seq_max, nullptr);
        } else if (llm_arch_is_hybrid(arch)) {
            // The main difference between hybrid architectures is the
            // layer filters, so pick the right one here
            monowire_memory_hybrid::layer_filter_cb filter_attn = nullptr;
            monowire_memory_hybrid::layer_filter_cb filter_recr = nullptr;
            ...
            if (hparams.swa_type != MONOWIRE_SWA_TYPE_NONE) {
                ...
                res = new monowire_memory_hybrid_iswa(
                    /* model             */ *this,
                ...
                res = new monowire_memory_hybrid(
                    /* model             */ *this,
                    /* attn_type_k       */ params.type_k,
                    /* attn_type_v       */ params.type_v,
                    /* attn_v_trans      */ !cparams.flash_attn,
            ...
            monowire_memory_i::layer_reuse_cb reuse = nullptr;
            ...
            if (hparams.swa_type != MONOWIRE_SWA_TYPE_NONE) {
                ...
                res = new monowire_kv_cache_iswa(
                ...
                res = new monowire_kv_cache(*this, params.type_k, params.type_v, !cparams.flash_attn,
...
ggml_cgraph *monowire_model::build_graph(const llm_graph_params &params) const {
    ...
    case LLM_ARCH_GEMMA3: {
        if (hparams.swa_type == MONOWIRE_SWA_TYPE_STANDARD) {
...
std::unique_ptr<monowire_streaming_runtime_state> monowire_model::create_streaming_runtime() const {
    if (!params.streaming_prefetch || params.streaming_window == 0 || !pimpl->streaming_runtime_plan
        || pimpl->mappings.empty()) {
        return nullptr;
    }
...
monowire_model_params monowire_model_default_params() {
    monowire_model_params result = {
        /*.devices                     =*/nullptr,
...
const monowire_vocab *monowire_model_get_vocab(const monowire_model *model) { return &model->vocab; }

void monowire_free_model(monowire_model *model) { monowire_model_free(model); }

void monowire_model_free(monowire_model *model) { delete model; }

int32_t monowire_model_n_ctx_train(const monowire_model *model) { return model->hparams.n_ctx_train; }

int32_t monowire_model_n_embd(const monowire_model *model) { return model->hparams.n_embd; }

int32_t monowire_model_n_embd_inp(const monowire_model *model) { return model->hparams.n_embd_inp(); }

int32_t monowire_model_n_embd_out(const monowire_model *model) { return model->hparams.n_embd_out(); }

int32_t monowire_model_n_layer(const monowire_model *model) { return model->hparams.n_layer; }

int32_t monowire_model_n_head(const monowire_model *model) { return model->hparams.n_head(); }

int32_t monowire_model_n_head_kv(const monowire_model *model) { return model->hparams.n_head_kv(); }

int32_t monowire_model_n_swa(const monowire_model *model) { return model->hparams.n_swa; }

uint32_t monowire_model_n_cls_out(const struct monowire_model *model) { return model->hparams.n_cls_out; }

const char *monowire_model_cls_label(const struct monowire_model *model, uint32_t i) {
...
// deprecated
int32_t monowire_n_ctx_train(const monowire_model *model) { return monowire_model_n_ctx_train(model); }

// deprecated
int32_t monowire_n_embd(const monowire_model *model) { return monowire_model_n_embd(model); }

// deprecated
int32_t monowire_n_layer(const monowire_model *model) { return monowire_model_n_layer(model); }

// deprecated
int32_t monowire_n_head(const monowire_model *model) { return monowire_model_n_head(model); }

monowire_rope_type monowire_model_rope_type(const monowire_model *model) {
    switch (model->arch) {
    ...
    case LLM_ARCH_NEMOTRON_H_MOE:
    case LLM_ARCH_KIMI_LINEAR:
        return MONOWIRE_ROPE_TYPE_NONE;
    ...
    case LLM_ARCH_MAINCODER:
    case LLM_ARCH_GLM_DSA:
        return MONOWIRE_ROPE_TYPE_NORM;
    ...
    case LLM_ARCH_MIMO2:
    case LLM_ARCH_STEP35:
        return MONOWIRE_ROPE_TYPE_NEOX;

    case LLM_ARCH_QWEN2VL:
    case LLM_ARCH_PADDLEOCR:
        return MONOWIRE_ROPE_TYPE_MROPE;
    case LLM_ARCH_QWEN3VL:
    case LLM_ARCH_QWEN3VLMOE:
    case LLM_ARCH_QWEN35:
    case LLM_ARCH_QWEN35MOE:
        return MONOWIRE_ROPE_TYPE_IMROPE;

    case LLM_ARCH_GLM4:
        return model->hparams.use_mrope() ? MONOWIRE_ROPE_TYPE_MROPE : MONOWIRE_ROPE_TYPE_NORM;
    case LLM_ARCH_GLM4_MOE:
        return model->hparams.use_mrope() ? MONOWIRE_ROPE_TYPE_MROPE : MONOWIRE_ROPE_TYPE_NEOX;
...
float monowire_model_rope_freq_scale_train(const monowire_model *model) { return model->hparams.rope_freq_scale_train; }

int32_t monowire_model_meta_val_str(const monowire_model *model, const char *key, char *buf, size_t buf_size) {
...
int32_t monowire_model_meta_count(const monowire_model *model) { return (int)model->gguf_kv.size(); }

const char *monowire_model_meta_key_str(monowire_model_meta_key key) {
    switch (key) {
    case MONOWIRE_MODEL_META_KEY_SAMPLING_SEQUENCE:
        return "general.sampling.sequence";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_TOP_K:
        return "general.sampling.top_k";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_TOP_P:
        return "general.sampling.top_p";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_MIN_P:
        return "general.sampling.min_p";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_XTC_PROBABILITY:
        return "general.sampling.xtc_probability";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_XTC_THRESHOLD:
        return "general.sampling.xtc_threshold";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_TEMP:
        return "general.sampling.temp";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_PENALTY_LAST_N:
        return "general.sampling.penalty_last_n";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_PENALTY_REPEAT:
        return "general.sampling.penalty_repeat";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_MIROSTAT:
        return "general.sampling.mirostat";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_MIROSTAT_TAU:
        return "general.sampling.mirostat_tau";
    case MONOWIRE_MODEL_META_KEY_SAMPLING_MIROSTAT_ETA:
...
int32_t monowire_model_meta_key_by_index(const monowire_model *model, int i, char *buf, size_t buf_size) {
...
int32_t monowire_model_meta_val_str_by_index(const monowire_model *model, int32_t i, char *buf, size_t buf_size) {
    if (i < 0 || i >= (int)model->gguf_kv.size()) {
        if (buf_size > 0) {
            buf[0] = '\0';
        }
...
int32_t monowire_model_desc(const monowire_model *model, char *buf, size_t buf_size) {
    return snprintf(buf, buf_size, "%s", model->desc().c_str());
}

uint64_t monowire_model_size(const monowire_model *model) { return model->size(); }

const char *monowire_model_chat_template(const monowire_model *model, const char *name) {
    ...
    if (it == model->gguf_kv.end()) {
        ...
        monowire_vocab_pre_type pre_type = model->vocab.get_pre_type();
        if (!name && pre_type == MONOWIRE_VOCAB_PRE_TYPE_TEKKEN && model->layers.size() == 40) {
...
uint64_t monowire_model_n_params(const monowire_model *model) { return model->n_elements(); }

bool monowire_model_has_encoder(const monowire_model *model) {
    switch (model->arch) {
    case LLM_ARCH_T5:
        return true;
    case LLM_ARCH_T5ENCODER:
...
bool monowire_model_has_decoder(const monowire_model *model) {
    switch (model->arch) {
    case LLM_ARCH_T5ENCODER:
        return false;
    default:
...
monowire_token monowire_model_decoder_start_token(const monowire_model *model) {
    return model->hparams.dec_start_token_id;
}

bool monowire_model_is_recurrent(const monowire_model *model) { return llm_arch_is_recurrent(model->arch); }

bool monowire_model_is_hybrid(const monowire_model *model) { return llm_arch_is_hybrid(model->arch); }

bool monowire_model_is_diffusion(const monowire_model *model) { return llm_arch_is_diffusion(model->arch); }

const std::vector<std::pair<std::string, ggml_tensor *>> &
monowire_internal_get_tensor_map(const monowire_model *model) {
    return model->tensors_by_name;
}

int32_t monowire_model_n_expert(const struct monowire_model *model) { return model->hparams.n_expert; }

int32_t monowire_model_n_devices(const struct monowire_model *model) { return (int32_t)model->devices.size(); }

ggml_backend_dev_t monowire_model_get_device(const struct monowire_model *model, int i) {

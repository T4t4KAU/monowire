...
static std::string unicode_cpts_to_utf8(const std::vector<uint32_t> &cps) {
    std::string result;
    for (size_t i = 0; i < cps.size(); ++i) { result.append(unicode_cpt_to_utf8(cps[i])); }
    return result;
}
...
// "(?i:'s|'t|'re|'ve|'m|'ll|'d)|[^\\r\\n\\p{L}\\p{N}]?\\p{L}+|\\p{N}|
// ?[^\\s\\p{L}\\p{N}]+[\\r\\n]*|\\s*[\\r\\n]+|\\s+(?!\\S)|\\s+"
static std::vector<size_t> unicode_regex_split_custom_qwen2(const std::string &text,
                                                            const std::vector<size_t> &offsets) {
    std::vector<size_t> bpe_offsets;     // store the offset of each word
    bpe_offsets.reserve(offsets.size()); // Reserve memory for the approximate size
...
#ifdef _MSC_VER
    // Bypass bug in MSVC: upstream-reference
...
// [\p{Han}]+|[^\r\n\p{L}\p{N}]?[\p{Lu}\p{Lt}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]*[\p{Ll}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]+(?i:'s|'t|'re|'ve|'m|'ll|'d)?|[^\r\n\p{L}\p{N}]?[\p{Lu}\p{Lt}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]+[\p{Ll}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]*(?i:'s|'t|'re|'ve|'m|'ll|'d)?|\p{N}{1,3}|
// ?[^\s\p{L}\p{N}]+[\r\n]*|\s*[\r\n]+|\s+(?!\S)|\s+
    ...
    for (auto offset : offsets) {
        ...
        for (size_t pos = offset_ini; pos < offset_end; /*pos++*/) {
            ...
            if (unicode_cpt_is_han(cpt)) {
                while (unicode_cpt_is_han(_get_cpt(pos))) { pos++; }
                _add_token(pos);
                continue;
            }

            // Pattern 2 & 3: Letter words excluding Han characters with optional
            // contractions
            // [^\r\n\p{L}\p{N}]?[\p{Lu}\p{Lt}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]*[\p{Ll}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]+(?:'s|'t|'re|'ve|'m|'ll|'d)?
            // [^\r\n\p{L}\p{N}]?[\p{Lu}\p{Lt}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]+[\p{Ll}\p{Lm}\p{Lo}\p{M}&&[^\p{Han}]]*(?:'s|'t|'re|'ve|'m|'ll|'d)?
            // Check if current char is a letter OR if current char could be a leading
            // char and next char is a letter
            ...
            if (is_letter_pattern) {
                ...
                // Only proceed if we found letters (after potentially skipping leading
                // char)
                if (has_letters
                    ...
                    while (_get_flags(pos).is_letter && !unicode_cpt_is_han(_get_cpt(pos))) { pos++; }
...
// regex: [^\n]+|[\n]+
// splits text into runs of non-newline characters and runs of newline
// characters
static std::vector<size_t> unicode_regex_split_custom_newlines(const std::string &text,
                                                               const std::vector<size_t> &offsets) {
    std::vector<size_t> bpe_offsets;
    bpe_offsets.reserve(offsets.size());
...
static std::vector<size_t> unicode_regex_split_custom(const std::string &text, const std::string &regex_expr,
    ...
    if (regex_expr
        ...
        //   {"type": "Split", "pattern": {"Regex": "\\d{1,3}(?=(?:\\d{3})*\\b)"},
        //   "behavior": "Isolated"}
        // Splits digits into groups of 3 from the right (e.g., 1234567 -> 1, 234,
        // 567)
        // TODO: Revisit this regex, in case there are any subtle tokenization
        // differences with the original regex.
        bpe_offsets = unicode_regex_split_custom_afmoe(text, offsets);
    }
...
std::vector<uint32_t> unicode_cpts_from_utf8(const std::string &utf8) {
    ...
    while (offset < utf8.size()) {
        try {
            ...
            // Silently ignore invalid UTF-8 input to avoid leaking the exception
            // beyond monowire_tokenize
            ++offset;
...
uint32_t unicode_tolower(uint32_t cpt) {
    ...
    auto it = std::lower_bound(
        unicode_map_lowercase.begin(), unicode_map_lowercase.end(), cpt,
...
std::vector<std::string> unicode_regex_split(const std::string &text, const std::vector<std::string> &regex_exprs,
    ...
    static const std::map<int, std::string> k_ucat_map = {
        ...
        {unicode_cpt_flags::PUNCTUATION, "\x21-\x23\x25-\x2A\x2C-\x2F\x3A-\x3B\x3F-\x40\\\x5B-"
                                         "\\\x5D\x5F\\\x7B\\\x7D"},         // !-#%-*,-/:-;?-@\[-\]_\{\}
    ...
    // replaced by a single byte ref: upstream-reference

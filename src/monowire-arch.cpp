...
static const std::map<llm_arch, const char *> LLM_ARCH_NAMES = {
    {LLM_ARCH_CLIP, "clip"}, // dummy, only used by monowire-quantize
...
// - the layer in which the tensor is going to be used. this is needed in order
// to assign the correct buffer type for the weight
// - the operator which is going to use the weight. this is needed to determine
// if the respective backend supports the operator
//
// for example, input layers are usually assigned to CPU/host buffer types
//
// a mismatch between the declared information and the actual layer/op in which
// the tensor is used can lead to sub-optimal
//   assignment of the buffer types and extra overhead during computation
// example: upstream-reference
//
static const std::map<llm_tensor, llm_tensor_info> LLM_TENSOR_INFOS = {
    ...
    // Kimi KDA - Conv tensors are 4D [d_conv, 1, d_inner, 1], reshaped to 2D at
    // runtime
    ...
    // These tensors only exist in the last layer(s) and are treated as output
    // tensors
...
std::vector<llm_arch> llm_arch_all() {
    ...
    for (const auto &[arch, _] : LLM_ARCH_NAMES) { ret.push_back(arch); }
    return ret;
}
...
const llm_tensor_info &llm_tensor_info_for(llm_tensor tensor) { return LLM_TENSOR_INFOS.at(tensor); }

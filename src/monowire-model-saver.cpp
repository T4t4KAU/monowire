...
bool monowire_model_saver_supports_arch(llm_arch arch) {
    switch (arch) {
...
monowire_model_saver::monowire_model_saver(const struct monowire_model *model)
    : gguf_ctx(gguf_init_empty()), gguf_ctx_owned(true), model(model), llm_kv(model->arch) {
    GGML_ASSERT(monowire_model_saver_supports_arch(model->arch));
}

monowire_model_saver::monowire_model_saver(enum llm_arch arch, struct gguf_context *gguf_ctx)
    : gguf_ctx(gguf_ctx == nullptr ? gguf_init_empty() : gguf_ctx), gguf_ctx_owned(gguf_ctx == nullptr), model(nullptr),
      llm_kv(arch) {}

monowire_model_saver::~monowire_model_saver() {
    if (gguf_ctx_owned) {
        gguf_free(gguf_ctx);
    }
}

void monowire_model_saver::add_kv(const enum llm_kv key, const uint32_t value) {
    gguf_set_val_u32(gguf_ctx, llm_kv(key).c_str(), value);
}

void monowire_model_saver::add_kv(const enum llm_kv key, const int32_t value) {
    gguf_set_val_i32(gguf_ctx, llm_kv(key).c_str(), value);
}

void monowire_model_saver::add_kv(const enum llm_kv key, const float value) {
    gguf_set_val_f32(gguf_ctx, llm_kv(key).c_str(), value);
}

void monowire_model_saver::add_kv(const enum llm_kv key, const bool value) {
    gguf_set_val_bool(gguf_ctx, llm_kv(key).c_str(), value);
}

void monowire_model_saver::add_kv(const enum llm_kv key, const char *value) {
    gguf_set_val_str(gguf_ctx, llm_kv(key).c_str(), value);
}

[[noreturn]] void monowire_model_saver::add_kv(const enum llm_kv key, const char value) {
    GGML_UNUSED(key);
    GGML_UNUSED(value);
    GGML_ABORT("fatal error"); // this should never be called, only needed to make
                               // the template below compile
}

template <typename Container>
void monowire_model_saver::add_kv(const enum llm_kv key, const Container &value, const bool per_layer) {
...
template void monowire_model_saver::add_kv<std::vector<uint32_t>>(const enum llm_kv, const std::vector<uint32_t> &,
                                                                  const bool);

void monowire_model_saver::add_kv(const enum llm_kv key, const std::vector<std::string> &value) {
    std::vector<const char *> tmp(value.size());
    for (size_t i = 0; i < value.size(); ++i) { tmp[i] = value[i].c_str(); }
    gguf_set_arr_str(gguf_ctx, llm_kv(key).c_str(), tmp.data(), tmp.size());
}

void monowire_model_saver::add_tensor(const struct ggml_tensor *tensor) {
    if (!tensor) {
        return;
    }
    if (gguf_find_tensor(gguf_ctx, tensor->name) >= 0) {
        const std::string tensor_name = tensor->name;
        GGML_ASSERT(tensor_name == "rope_freqs.weight" || tensor_name == "rope_factors_long.weight"
...
void monowire_model_saver::add_kv_from_model() {
    const monowire_hparams &hparams = model->hparams;
    const monowire_vocab &vocab = model->vocab;
    ...
    if (vocab.get_type() != MONOWIRE_VOCAB_TYPE_NONE) {
        for (int32_t id = 0; id < n_vocab; ++id) {
            const monowire_vocab::token_data &token_data = vocab.get_token_data(id);
            ...
            case MONOWIRE_TOKEN_ATTR_UNKNOWN:
                token_types[id] = MONOWIRE_TOKEN_TYPE_UNKNOWN;
                break;
            case MONOWIRE_TOKEN_ATTR_UNUSED:
                token_types[id] = MONOWIRE_TOKEN_TYPE_UNUSED;
                break;
            case MONOWIRE_TOKEN_ATTR_NORMAL:
                token_types[id] = MONOWIRE_TOKEN_TYPE_NORMAL;
                break;
            case MONOWIRE_TOKEN_ATTR_CONTROL:
                token_types[id] = MONOWIRE_TOKEN_TYPE_CONTROL;
                break;
            case MONOWIRE_TOKEN_ATTR_USER_DEFINED:
                token_types[id] = MONOWIRE_TOKEN_TYPE_USER_DEFINED;
                break;
            case MONOWIRE_TOKEN_ATTR_BYTE:
                token_types[id] = MONOWIRE_TOKEN_TYPE_BYTE;
                break;
            // case MONOWIRE_TOKEN_ATTR_NORMALIZED:   ???
            // case MONOWIRE_TOKEN_ATTR_LSTRIP:       ???
            // case MONOWIRE_TOKEN_ATTR_RSTRIP:       ???
            case MONOWIRE_TOKEN_ATTR_UNDEFINED:
            default:
                token_types[id] = MONOWIRE_TOKEN_TYPE_UNDEFINED;
                break;
            }
        }
    }
    ...
    // add_kv(LLM_KV_ROPE_SCALE_LINEAR,                 rope_scaling_factor); //
    // old name
    add_kv(LLM_KV_ROPE_SCALING_TYPE, monowire_rope_scaling_type_name(hparams.rope_scaling_type_train));
    ...
    // FIXME monowire_token is type i32 but when reading in a GGUF file u32 is
    // expected, not an issue for writing though
...
void monowire_model_saver::add_tensors_from_model() {
    if (model->output != nullptr && std::string(model->output->name) != std::string(model->tok_embd->name)) {
        add_tensor(model->tok_embd); // some models use the same tensor for tok_embd
                                     // and output
    }
    ...
    for (const struct monowire_layer &layer : model->layers) {
...
void monowire_model_saver::save(const std::string &path_model) {
    gguf_write_to_file(gguf_ctx, path_model.c_str(), false);
}

void monowire_model_saver::save(FILE *file) { gguf_write_to_file_ptr(gguf_ctx, file, false); }
